...
`CHATTO_AUTH_ACCOUNT_CREATION_POLICY=invite_only`) to require an invitation for
...
Administrators with `invite.manage` create and revoke invitation links under
**Server Admin → Invitations**. New links default to one use and seven days;
...
Invitation links prove admission, not identity. Direct registration still
verifies the user's email, while an external provider still authenticates its
own subject. Expired, exhausted, revoked, and malformed invitations all appear
to the person signing up as unavailable.

<Aside type="caution">
  Keep `core.secret_key` stable across replicas and restores. Rotating it
  invalidates links already shared, although the durable invitation records
