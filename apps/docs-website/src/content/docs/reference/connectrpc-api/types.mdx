...
| `followed_rooms` | optional [`NotificationDeliveryMode`](#chatto-api-v1-NotificationDeliveryMode) | Delivery mode for top-level activity in rooms followed by the viewer. Servers without room-follow support apply it to all joined channel rooms. |
...
<a id="chatto-api-v1-ThreadInteraction"></a>
...
| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Channel room containing the thread. |
| `thread_root_event_id` | `string` | Event ID of the thread root message. |
| `causes` | repeated [`ThreadInteractionCause`](#chatto-api-v1-ThreadInteractionCause) | Post-time facts that established the relationship. |
| `last_activity_at` | `google.protobuf.Timestamp` | Latest visible activity in the thread. |

<a id="chatto-api-v1-ThreadInteractionCause"></a>

### ThreadInteractionCause

One post-time fact that established a thread interaction relationship.

| Field | Type | Description |
| --- | --- | --- |
| `type` | [`ThreadInteractionCauseType`](#chatto-api-v1-ThreadInteractionCauseType) | Relationship cause. |
| `source_message_event_id` | `string` | Message event that established the cause. |
| `created_at` | `google.protobuf.Timestamp` | Time when the source message was posted. |
...
<a id="chatto-api-v1-ThreadInteractionCauseType"></a>

### ThreadInteractionCauseType

Post-time reason that the current account is related to one thread.

| Name | Number | Description |
| --- | --- | --- |
| `THREAD_INTERACTION_CAUSE_TYPE_UNSPECIFIED` | `0` | The cause is absent or unknown. |
| `THREAD_INTERACTION_CAUSE_TYPE_ROOT_AUTHORED` | `1` | The current account authored the thread root. |
| `THREAD_INTERACTION_CAUSE_TYPE_DIRECT_MENTION` | `2` | Another account directly mentioned the current account. |
