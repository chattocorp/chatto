...
| Live Events                  | `corev1.Event`   | Transient  | `live.server.>` (NATS Core, fed by `SERVER_EVENTS` republish + direct publishes) and `live.evt.>` (fed by `SERVER_EVT` republish). The two roots coexist during the migration window. |
