...
export function macOSDistributionOptions(platform, environment) {
  ...
  const notaryValues = {
    appleApiIssuer: environment.CHATTO_MACOS_NOTARY_API_ISSUER_ID?.trim() || undefined,
    appleApiKey: environment.CHATTO_MACOS_NOTARY_API_KEY?.trim() || undefined,
    appleApiKeyId: environment.CHATTO_MACOS_NOTARY_API_KEY_ID?.trim() || undefined
  };
  const requestedNotarisation = Object.values(notaryValues).some(Boolean);

  if (requestedNotarisation && identity === '-') {
    throw new Error('CHATTO_MACOS_SIGN_IDENTITY is required when notarisation is configured.');
  }
  if (requestedNotarisation && Object.values(notaryValues).some((value) => !value)) {
    throw new Error(
