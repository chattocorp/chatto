...
| `evtstream.Publisher`           | [`publisher.go`](../../cli/internal/evtstream/publisher.go), [`subjects.go`](../../cli/internal/evtstream/subjects.go)                                           | Chatto adapter that owns the stable EVT subject vocabulary, validates durable `evtv1.Event` values, preserves their stable IDs, protobuf-encodes and decodes them above `EncodedEventLog`, and supplies exact-sequence reads for bounded timeline bucket reconstruction |
| `events.ProjectionHandle` / `events.Projector` / `events.ComponentizedProjection` | [`projector.go`](../../pkg/events/projector.go), [`component_projection.go`](../../pkg/events/component_projection.go), [`projector.go`](../../cli/internal/evtstream/projector.go) | Envelope-neutral typed model ownership, prepared reducers, one projector-owned apply barrier for related components, ordered replay, readiness, failure, projection snapshot cohorts, scalar snapshots, and checkpoints; `evtstream` supplies the Chatto event decoder and typed constructors |
...
| `RoomTimelineReadModel`          | [`room_timeline_read_model.go`](../../cli/internal/core/room_timeline_read_model.go), [`room_events.go`](../../cli/internal/core/room_events.go), [`room_timeline_projection.go`](../../cli/internal/core/room_timeline_projection.go) | Operation-level room/thread timeline read API with actor membership checks, broad or interaction-scoped message authorization, thread-root validation, filtered room roots, projection-backed page/window selection, and process-local reconstruction of encrypted bodies from exact EVT sequence bucket recipes |
