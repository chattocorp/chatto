...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="Bearer sessions renew without another login">
    ...
    realtime in place without reopening OAuth or losing the current route. It warns before the
    fixed maximum requires a deliberate reconnect. The bundled same-origin client prefers its
    safer HttpOnly cookie and persists the direct-login bearer only when a cookie probe cannot
    authenticate, removing that fallback after a later successful cookie request.
  </ReleaseFeatureCard>
