...
test('the coordinator calls ordinary and agentic tasks as tools in one workflow', async () => {
  ...
  const researcher = {
    runOutcome: vi.fn(
      async (ctx: WorkflowContext<unknown>, _prompt: string, options?: AgentRunOptions) => {
  ...
  await vi.waitFor(() =>
    expect(post.mock.calls.some((call) => call[1] === 'Research complete')).toBe(true)
  );
  expect(post.mock.calls.some((call) => call[1] === 'Which palette?')).toBe(false);
  sent.resolve();
  await vi.waitFor(() =>
    expect(post.mock.calls.some((call) => call[1] === 'Which palette?')).toBe(true)
  );
  await bot.route(
    {
      start: async () => {
  ...
  expect(
    events.filter((event) => event.type === 'step.started').map((event) => event.label)
  ).toEqual(['Chatto coordinator demo', 'Investigate: Find theme code', 'Ask Chatto user']);
...
test.each(['delivered', 'rejected', 'parallel'])(
  ...
  async (mode) => {
      ...
      runOutcome: vi.fn(async (ctx: WorkflowContext<unknown>) => {
        ...
        await Promise.all(
          Array.from({ length: mode === 'parallel' ? 2 : 1 }, (_, i) =>
            tool.execute(`research-${i}`, { question: 'Find theme code' }, ctx.signal)
          )
        );
        return report('Done');
      }),
    ...
    await vi.waitFor(() =>
      expect(researcher.runOutcome).toHaveBeenCalledTimes(mode === 'parallel' ? 2 : 1)
    );
    await bot.route(
      {
        start: async () => {
    ...
    await vi.waitFor(() =>
      expect(coordinator.steer).toHaveBeenCalledWith('Also consider accessibility')
    );
    if (mode === 'parallel') expect(researcher.steer).not.toHaveBeenCalled();
    else {
      await vi.waitFor(() =>
        expect(researcher.steer).toHaveBeenCalledWith('Also consider accessibility')
      );
    }
    expect(post.mock.calls.some((call) => call[1].startsWith('Passed your message'))).toBe(false);
    await bot.route(
      { start: async () => ({ id: 'unused' }) },
      delivery('/cancel', 'cancel', 'root')
    );
    await failed;
...
test('implementation requires approval of the displayed plan and cannot run twice', async () => {
  ...
  const coordinator = {
    runOutcome: vi.fn(async (ctx: WorkflowContext<unknown>) => {
      ...
      await expect(tool.execute('duplicate', { plan: 'Another plan' }, ctx.signal)).rejects.toThrow(
        'already pending or was attempted'
      );
  ...
  await vi.waitFor(() =>
    expect(post.mock.calls.some((call) => call[1].includes('First proposed plan'))).toBe(true)
  );
  expect(prepare).not.toHaveBeenCalled();
  await expect(
    registered
      .get('implement')
      .execute('parallel', { plan: 'Concurrent plan' }, new AbortController().signal)
  ...
  await vi.waitFor(() =>
    expect(post.mock.calls.some((call) => call[1].includes('Revised exact plan'))).toBe(true)
  );
  expect(prepare).not.toHaveBeenCalled();
  await bot.route(
    { start: async () => ({ id: 'unused' }) },
    delivery('/implement', 'approval', 'root')
  );
  await run;
  ...
  expect(
    messages.filter(
      (text) => text === 'Preparing an isolated worktree and installing dependencies.'
    )
  ).toHaveLength(1);
  expect(messages).toContain('Running project checks and tests (attempt 1/3).');
  expect(
    messages.indexOf('Preparing an isolated worktree and installing dependencies.')
  ).toBeLessThan(messages.indexOf('Running project checks and tests (attempt 1/3).'));
  ...
  const coordinator = {
    runOutcome: vi.fn(async (ctx: WorkflowContext<unknown>) => {
      const result = await tools
        .get('implement')
        .execute('approve', { plan: 'Small fix' }, ctx.signal);
