...
const runtimeWith = (prompt: string, reports: AgentResult[], answers: string[]) => {
...
describe('plan workflow', () => {
  test('interviews until the agent can complete an implementation plan', async () => {
    ...
    await expect(
      plan(createWorkflowContext(), {
        directory: runtime.f.cwd ?? '/project',
        prompt: 'Add deployment support'
      })
    ).resolves.toEqual({
      summary: 'Plan deployment support',
      details: '## Plan\n\n1. Add the deployment adapter.\n2. Test it.',
  ...
  test('asks for the initial request when no CLI prompt was provided', async () => {
    ...
    expect(runtime.questions).toEqual(['What would you like to build or change?']);
  ...
  test('fails clearly when the planning agent fails', async () => {
    ...
    ).rejects.toThrow('Planning failed: Could not inspect the repository');
    expect(runtime.disposed).toBe(true);
  });
});
