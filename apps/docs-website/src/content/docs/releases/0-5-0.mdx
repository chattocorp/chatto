...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="Notifications that stay useful after you read them">
    ...
    you no longer want to retain it; per-cause controls distinguish mentions,
...
- Notifications 2.0 replaces the previous notification service and stored records. Existing
  ...
  starts with an empty notification list and the new per-cause defaults. Integrations must
  ...
  `NotificationOccurrence.target` is a typed oneof whose initial variant is `room_message`;
  clients must not infer navigation for unsupported variants and may show them as generic,
  dismissible activity. The bundled client uses binary protobuf for this forward-compatible
  fallback. ProtoJSON integrations must ignore unknown fields or regenerate before a future
  target variant is exposed.
