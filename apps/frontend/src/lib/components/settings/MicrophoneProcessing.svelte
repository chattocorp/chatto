...
<div class="flex flex-col gap-5">
  ...
  <details>
    <summary class="cursor-pointer font-medium">{m('voice.preferences.processing')}</summary>
    <div class="mt-4 flex flex-col gap-4">
      ...
      <Checkbox
        ...
        disabled={unavailable}
        bind:checked={
          () => preferences.effects.equalizer,
          (value) => preferences.setEffects({ equalizer: value })
        }
      />
      ...
      <RangeField
        ...
        bind:value={
          () => preferences.effects.amount,
          (value) => preferences.setEffects({ amount: value ?? 50 })
        }
