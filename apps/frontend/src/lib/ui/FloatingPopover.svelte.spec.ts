...
it('keeps the full menu inside the viewport while its entrance is scaled', async () => {
  ...
  expect(parseFloat(popover.style.left) + popover.offsetWidth).toBeLessThanOrEqual(
    window.innerWidth - 8
  );
  expect(parseFloat(popover.style.top) + popover.offsetHeight).toBeLessThanOrEqual(
