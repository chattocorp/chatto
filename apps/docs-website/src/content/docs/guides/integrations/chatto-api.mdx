...
Discovery is also the right place for custom clients to learn public server identity and compatibility before starting OAuth or login flows. Typed, versioned capability fields describe protocol support; they are separate from server configuration and the authenticated viewer's permissions.
