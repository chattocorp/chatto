...
Users can attach files to messages — images, videos, documents — via drag-and-drop, paste, or file picker. Images are dimensioned and resizable on the fly via signed URLs. When video processing is enabled, videos are transcoded into adaptive HLS streams with MP4 compatibility variants; animated GIFs retain the MP4 loop path.
...
- New processed-video manifests contain a signed HLS master-playlist URL when HLS packaging succeeds. The player prefers HLS for adaptive, seekable playback and falls back to the highest MP4 variant if HLS is absent or fails. Existing processed videos are not backfilled and continue through the MP4 path.
...
### 5. Processed videos use HLS with MP4 compatibility variants

**Decision:** Transcoding produces multiple H.264 MP4 variants whose target resolutions are derived from the source display resolution. A 1080p source might yield 720p and 480p; a 480p source skips the higher tiers. Encoders force aligned six-second keyframes, then ffmpeg packages each MP4 into a VOD HLS media playlist and MPEG-TS segments without another encode. The terminal manifest records a master playlist, rendition metadata, and every playlist/segment asset ID. Clients prefer HLS and retain the MP4 variants as a compatibility and failure fallback. Animated GIF loops continue using MP4 directly.
**Why:** Bounded HLS segments provide duration, seeking, and adaptive rendition switching even when the asset backend can only stream complete objects. Keeping MP4 makes the additive API safe for older clients and lets playback survive an HLS packaging or runtime failure.
**Tradeoff:** Each processed upload stores MP4 variants plus playlists and segmented copies, increasing object count and storage. HLS packaging failure degrades to a successful MP4-only result. Historical processed videos are not backfilled; their manifests omit HLS and clients continue using MP4. During mixed-version operation an older replica can ignore HLS child IDs while deleting a source; the durable asset-cleanup consumer on a newer replica re-reads the source manifest and tombstones any still-live HLS children. Videos whose meaningful content occupies only part of the encoded canvas retain that empty space because the player does not guess which parts are safe to crop.
...
**Decision:** Public attachment APIs expose attachment media as stable asset paths plus per-user access tickets: `/assets/files/{assetId}?access={ticket}` for originals and `/assets/files/{assetId}/image/{width}x{height}/{fit}?access={ticket}` for image derivatives. HLS uses a domain-separated, source-video-scoped ticket on `/assets/hls/{assetId}/master.m3u8`; Chatto rewrites stored playlist entries to authorised child routes and checks every playlist and segment against the durable derivative manifest. Attachment, thumbnail, video thumbnail, HLS master, and variant URLs expose the ticket expiry so the client can refresh before or after a lazy-load miss. Every fetch verifies the signed user is still a member of the asset's room.
