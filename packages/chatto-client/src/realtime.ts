...
export async function consumeRealtime(
  ...
  options: ConsumeRealtimeOptions
): Promise<void> {
  ...
  async function session(): Promise<Outcome> {
    ...
    try {
      socket = (factory ?? ((url) => new WebSocket(url)))(url.href);
    } catch {
      return { error: new TerminalError('Cannot create the realtime transport') };
    }
    ...
    const finish = (result: Outcome = {}) => {
      ...
      try {
        ...
        /* A connecting socket can reject close. */
      }
      wake?.();
    };
      ...
      while (!ended) {
        ...
        switch (frame.frame.case) {
          case 'event': {
            ...
            if (event.event.case !== undefined) {
              try {
                ...
                throw new TerminalError('Realtime event handler failed');
              }
            }
          ...
          case 'caughtUp': {
            ...
            if (
              !cursor ||
              (recovery !== RealtimeRecovery.RESUMED && recovery !== RealtimeRecovery.LIVE_ONLY)
            ) {
            ...
            status({
              state: 'ready',
              recovery: recovery === RealtimeRecovery.RESUMED ? 'resumed' : 'live-only',
          ...
          case 'close': {
            ...
            if (
              !close.reconnect ||
              ![
                RealtimeCloseCode.TEMPORARILY_UNAVAILABLE,
                RealtimeCloseCode.RESYNC_REQUIRED,
                RealtimeCloseCode.PRIVILEGED_MODE_EXPIRED
              ].includes(close.code)
            )
              throw new TerminalError('Realtime subscription was rejected or ended');
          ...
          default:
            throw new TerminalError('Unsupported realtime frame');
        }
      }
    } catch (error) {
      outcome = {
        error:
          error instanceof TerminalError ? error : new TerminalError('Realtime processing failed')
      };
    } finally {
      ended = true;
      clearTimeout(timer);
