...
// DirectImage contains bounded image bytes returned directly by the preview
// URL. The caller promotes them through the normal pending-attachment path;
...
func (f *Fetcher) Fetch(ctx context.Context, rawURL string) (*FetchResult, error) {
	...
	if detectedContentType != "application/octet-stream" {
		...
		return &FetchResult{DirectImage: directImage}, nil
	}
...
func (f *Fetcher) readDirectImage(reader io.Reader, contentType string) (*DirectImage, error) {
	...
	return &DirectImage{Data: imageData, ContentType: contentType}, nil
}
