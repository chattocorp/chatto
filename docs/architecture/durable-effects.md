...
| Video derivative processing | `AssetProcessingStartedEvent`; `AssetProcessingSucceededEvent` or `AssetProcessingFailedEvent` is terminal | A process-local callback starts bounded ffmpeg work, writes MP4/thumbnail derivatives, packages HLS playlists and segments, then publishes the terminal manifest; partial HLS uploads are tombstoned and deleted best-effort | Each enabled video service scans the asset projection at boot for unmanifested video attachments and resubmits them; terminal event OCC prevents later manifest replacement; an HLS failure still commits an MP4-only success | Partially recoverable; scheduling is durable, but ownership is process-local and duplicate derivative work is possible across replicas |
...
| Asset binary and transform-cache deletion | `AssetDeletedEvent` makes projected reads and signed asset resolution reject the asset; the asset ID locates the canonical aggregate's durable creation metadata | Message deletion, attachment removal, account cleanup, pending-upload expiry, and derivative cleanup delete NATS/S3 bytes and cached transforms after recording deletion | The elected `asset_cleanup` worker incrementally consumes canonical `asset_deleted` facts, loads storage metadata from the matching `asset_created` fact, and retries idempotent binary/cache deletion after failure, restart, or lease handover. A source-video tombstone also re-reads its durable HLS manifest and tombstones any still-live HLS children, repairing deletion by an older HLS-unaware replica; beta room-scoped facts without a canonical creation aggregate are skipped | Recoverable for canonical message-owned asset deletion facts, including mixed-version HLS cleanup; beta room-scoped cleanup remains best-effort |
