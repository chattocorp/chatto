...
| -------------------------------------------------------------------------------------------- | -------------------------------- | ------------------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `/api/connect/chatto.api.v1.AdminService/ListUsers`                                          | `AdminService`                   | `ListUsers`                     | Admin API token from that token entry's allowed CIDRs; served only on the dedicated Admin API listener. | Operator-only paginated user listing for CLI/bootstrap automation.                            |
| `/api/connect/chatto.api.v1.AdminService/GetUser`                                            | `AdminService`                   | `GetUser`                       | Admin API token from that token entry's allowed CIDRs; served only on the dedicated Admin API listener. | Operator-only user lookup by ID or login.                                                     |
| `/api/connect/chatto.api.v1.AdminService/CreateUser`                                         | `AdminService`                   | `CreateUser`                    | Admin API token from that token entry's allowed CIDRs; served only on the dedicated Admin API listener. | Create a user as the system actor, optionally with password, verified email, and roles.       |
| `/api/connect/chatto.api.v1.AdminService/UpdateUser`                                         | `AdminService`                   | `UpdateUser`                    | Admin API token from that token entry's allowed CIDRs; served only on the dedicated Admin API listener. | Update a user's login and/or display name as the system actor.                                |
| `/api/connect/chatto.api.v1.AdminService/SetUserPassword`                                    | `AdminService`                   | `SetUserPassword`               | Admin API token from that token entry's allowed CIDRs; served only on the dedicated Admin API listener. | Set a user's password as the system actor and advance their auth generation.                  |
| `/api/connect/chatto.api.v1.AdminService/DeleteUser`                                         | `AdminService`                   | `DeleteUser`                    | Admin API token from that token entry's allowed CIDRs; served only on the dedicated Admin API listener. | Permanently delete a user through the normal account-deletion model.                          |
| `/api/connect/chatto.api.v1.AdminService/AddUserVerifiedEmail`                               | `AdminService`                   | `AddUserVerifiedEmail`          | Admin API token from that token entry's allowed CIDRs; served only on the dedicated Admin API listener. | Add an already-verified email address to a user as the system actor.                          |
| `/api/connect/chatto.api.v1.AdminService/AssignUserRole`                                     | `AdminService`                   | `AssignUserRole`                | Admin API token from that token entry's allowed CIDRs; served only on the dedicated Admin API listener. | Assign a server role to a user as the system actor.                                           |
| `/api/connect/chatto.api.v1.AdminService/RevokeUserRole`                                     | `AdminService`                   | `RevokeUserRole`                | Admin API token from that token entry's allowed CIDRs; served only on the dedicated Admin API listener. | Revoke a server role from a user as the system actor.                                         |
