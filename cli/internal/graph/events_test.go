...
func TestMessagePostedEventResolver_Reactions(t *testing.T) {
	...
	event, err := env.core.PostMessage(env.ctx, "channel", env.testRoom.Id, env.testUser.Id, "Hello world", nil, "", "", nil, false)
	if err != nil {
	...
	t.Run("returns reactions after adding one", func(t *testing.T) {
		_, err := env.core.AddReaction(env.ctx, "channel", env.testRoom.Id, event.Id, "thumbsup", env.testUser.Id)
		if err != nil {
...
func TestMessagePostedEventResolver_Body(t *testing.T) {
	...
	event, err := env.core.PostMessage(env.ctx, "channel", env.testRoom.Id, env.testUser.Id, "Test body content", nil, "", "", nil, false)
	if err != nil {
		t.Fatalf("failed to post message: %v", err)
	}
...
func TestMessagePostedEventResolver_ReplyCount(t *testing.T) {
	...
	t.Run("root message with no replies returns 0", func(t *testing.T) {
		event, err := env.core.PostMessage(env.ctx, "channel", env.testRoom.Id, env.testUser.Id, "Root msg", nil, "", "", nil, false)
		if err != nil {
			t.Fatalf("failed to post message: %v", err)
		}
		msgEvent := event.Event.(*corev1.Event_MessagePosted).MessagePosted
