...
it.each([100, 180, 260])('preserves moderate voiced input at AWESOME (%s Hz)', async (fundamental) => {
  const rate = 48000;
  ...
  try {
    ...
    expect(20 * Math.log10(gain)).toBeGreaterThan(-3);
    expect(20 * Math.log10(gain)).toBeLessThan(3);
    for (const harmonic of [2, 3]) {
      const relative = magnitude(output, fundamental * harmonic) /
        magnitude(input, fundamental * harmonic) / gain;
      expect(Math.abs(20 * Math.log10(relative))).toBeLessThan(2);
    }
    // The input has no fifth harmonic: avoid adding audible waveshaping distortion.
    expect(magnitude(output, fundamental * 5) / magnitude(output, fundamental)).toBeLessThan(0.01);
  } finally {
    graph.destroy();
    gate.disconnect();
    polish.disconnect();
