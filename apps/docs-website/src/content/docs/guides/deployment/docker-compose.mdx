...
The Compose example uses LiveKit's embedded TURN/UDP server; you do not need a
separate coturn service for a standard deployment. The generated configuration
also sets `rtc.use_external_ip: true` for internet-facing deployments. LiveKit
uses outbound STUN requests to discover the public IP it advertises to clients,
so allow UDP egress from the LiveKit container to its configured or default STUN
servers. This is separate from the inbound UDP 3478 listener used by the
embedded TURN/UDP relay.

If the deployment has a stable, known public IP, you can avoid that discovery
traffic by setting the following in the LiveKit config:
...
Use this only when the advertised address and media port forwarding are stable.
For NATed or dynamic deployments, keep `use_external_ip: true`.
...
**Calls don't connect** — Verify the LiveKit API key/secret in `.env` matches the selected LiveKit config (`livekit.generated.yaml` or `livekit.yaml`). Ensure `CHATTO_LIVEKIT_URL` uses the public `wss://livekit.*` subdomain (browsers connect to it directly). Check that TCP 7881 and UDP 3478 and 50000-50200 are open in your firewall. When `rtc.use_external_ip: true`, also allow outbound UDP from LiveKit to its configured or default STUN servers. For a LAN-only host without an external IP address, set `rtc.use_external_ip: false` in `livekit.generated.yaml` before starting or restarting LiveKit.
