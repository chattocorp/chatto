...
| `notification_read_boundary.{userId}.{roomId}[.{threadRootEventId}]` | Two big-endian EVT stream sequences: the latest room/thread timeline target read and the reaction projection horizon observed by that read action. Creation and read reconciliation use direct KV handshakes so cross-replica watcher lag cannot leave covered activity unread. Every replica also reconciles boundaries against unread projected occurrences at startup and in the background, completing an interrupted KV-to-`NOTIFICATIONS` handshake idempotently. The two coordinates keep reactions that arrive after a read new until the next read. The key expires 90 days after its latest update and account deletion removes it. |
