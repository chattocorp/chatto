...
Use the **Microphone processing** panel to adjust these optional effects:

- **Low-cut filter** removes low-frequency rumble below 80 Hz.
- **Equaliser** adjusts bass, mids, and treble by up to 6 dB in either direction.
  Boosts reserve headroom, so the overall volume can decrease.
- **Compressor** reduces the difference between loud and quiet speech.
  Increase **Amount** for stronger compression. The browser compressor also raises quiet input.

All three effects start disabled. They apply immediately to your call and
microphone test. Processing runs locally in this order: low-cut filter,
noise gate, equaliser, compressor. It does not affect camera, screen-share,
or received audio. **Reset processing** disables the gate and effects and
restores their default values. Your device choices stay unchanged.
