...
This blocks normal members and named roles that only have less-specific posting grants. Owners can post, and a named role with its own room-level `message.post` grant can post too.
...
<Aside type="caution" title="Authorization behavior changed">
  When upgrading from the older literal deny-wins resolver, review configurations that combine an `everyone` deny with a named-role or direct-user allow. Same-scope or nearer allows can now widen access, while a nearer `everyone` deny can contain broader named grants. For suspensions, clear nearer decisions on the same user or restriction role.
</Aside>
