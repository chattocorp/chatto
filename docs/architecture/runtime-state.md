...
| `link_preview.{urlHash}` | Cached link preview metadata (protobuf `CachedLinkPreview`) keyed by SHA-256 of the normalized URL. Successful previews use per-key 24-hour TTL; failed fetches use per-key 1-hour TTL. Legacy Bluesky snapshots without the provider-neutral canonical post URL are treated as cache misses and replaced on the next fetch. |
