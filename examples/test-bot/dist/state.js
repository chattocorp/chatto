...
export async function loadTestBotState(stateFile) {
    let raw;
    try {
    ...
    return {
        ...
        processedEventIds: candidate.processedEventIds.slice(-MAX_PROCESSED_EVENT_IDS),
    };
}
/** Atomically retain a cursor and a bounded event-ID deduplication window. */
export async function saveTestBotState(stateFile, state) {
...
export function rememberProcessedEvent(state, eventId) {
    ...
    if (state.processedEventIds.length > MAX_PROCESSED_EVENT_IDS) {
        state.processedEventIds.splice(0, state.processedEventIds.length - MAX_PROCESSED_EVENT_IDS);
    }
    return true;
}
