...
handles use typed `RUNTIME_STATE` `session.{hmac}` records with sliding expiry
and periodic browser-handle rotation.
...
the revocation authority for every access generation. Successful access-token
validation never extends either expiry. Raw access and refresh credentials are
not stored.
