...
export function createChattoInputDemo({ post, timeout = 30 }: DemoOptions) {
  ...
  return createChattoWebhook({
    ...
    async run(ctx, _delivery, destination) {
      const name = await input(ctx, `What is your name? Please reply within ${timeout} seconds.`, {
        timeout
      });
      const topic = await input(
        ctx,
        `What would you like to work on? Please reply within ${timeout} seconds.`,
        { timeout }
      );
      await post(
        destination,
        `Thanks! Your name: ${name}\nYour topic: ${topic}`,
        messageSignal(ctx)
      );
      return { name, topic };
    }
