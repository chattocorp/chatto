...
- Each content key is wrapped with the author's per-user key encryption key (KEK).
...
The per-user KEK is generated on demand and stored in a dedicated `ENCRYPTION_KEYS` NATS KV bucket. Authentication tags are verified on every read; tampered ciphertext or ciphertext copied into the wrong event context is rejected with `ErrDecryptionFailed`.
