...
{#each groups as group}
  ...
  <button
    ...
    aria-label={group.messages.length > 1
      ? `${group.messages.length} task messages`
      : `Message: ${first.from} → ${first.to}. ${first.status}`}
    title={group.messages.length > 1
      ? `${group.messages.length} messages — click to inspect`
      : `${first.status}: ${first.payload.slice(0, 100)}`}
    onclick={() => onselect(first.id)}
  >
    <span
      class="flex h-4 items-center gap-1 rounded-sm bg-linear-to-b from-yellow-200 to-yellow-300 px-1 text-yellow-950 group-hover:brightness-105"
    >
      ...
      {/if}
      {#if group.messages.length > 1}<span
          class="pl-0.5 text-[10px] leading-none font-medium tabular-nums"
