...
Room-relevant permissions such as `message.read`, `message.read.interactions`, `message.post`, `message.attach`, `message.react`, `message.manage`, `room.join`, and `room.ban-member` can be configured at room/group/server scope. Server-only permissions such as `server.manage`, `role.manage`, and `admin.view-audit` are configured only at server scope.

Permission identifiers contain two or more dot-separated components. A dotted
prefix does not automatically grant a longer permission. Chatto defines one
explicit inclusion: an effective `message.read` allow also allows
...
does not restrict a separate narrow allow.
