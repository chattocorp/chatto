...
<EnvVar name="CHATTO_CORE_PROJECTION_SNAPSHOTS" toml="core.projection_snapshots" default="false">
  Persist encrypted `ThreadProjection` snapshots to validate snapshot storage and restoration. This canary does not reduce total startup time because the shared replay still starts at the beginning of `EVT`. Chatto treats `EVT` as authoritative and automatically replays it when a snapshot is missing, incompatible, corrupt, or unavailable. While enabled, one elected replica periodically removes unreferenced snapshot objects after a 24-hour grace period. S3 credentials therefore require permission to list the configured bucket prefix in addition to reading, writing, and deleting objects.
</EnvVar>
