...
each process reads the sender's choice once per typing event, and only when a
local member other than the sender can receive it. The hub does not hold its
