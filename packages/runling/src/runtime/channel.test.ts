...
import { ChannelClosedError, ChannelFullError, createChannel } from './channel.ts';
...
test('queues FIFO values before a reader starts, including undefined', async () => {
  ...
  await Promise.all([channel.send(1), channel.send(undefined), channel.send(3)]);
  channel.close();
...
test('enforces a single consumer and one pending next call', async () => {
  ...
  expect(() => channel[Symbol.asyncIterator]()).toThrow('already has a consumer');
