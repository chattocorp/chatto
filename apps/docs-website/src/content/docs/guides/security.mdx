...
The CIDR check uses the direct TCP peer address seen by Chatto, not `X-Forwarded-For` or another forwarded client header. If Chatto sits behind a reverse proxy on loopback, public requests forwarded to Chatto can appear to come from `127.0.0.1` or `::1`; do not publicly route `/api/connect/chatto.api.v1.AdminService/*` unless that is intentional. Prefer a local-only operator path, a private sidecar, or proxy-level denial for AdminService routes.

AdminService actions are attributed to Chatto's system actor, not to an in-app user or RBAC role. Keep admin tokens out of browser clients, logs, and shell history where possible, and prefer a private localhost/sidecar network path over exposing them through a public reverse proxy.
