...
func TestOAuthClientAuthorizationPolicyAndTokenRevocation(t *testing.T) {
	...
	token, err := c.CreateOAuthAccessTokenForClient(ctx, member.Id, clientID, generation, time.Time{})
	if err != nil {
	...
	if _, err := c.CreateOAuthAccessTokenForClient(ctx, member.Id, clientID, generation, time.Time{}); !errors.Is(err, ErrOAuthClientBlocked) {
...
func TestOAuthClientAuthorizationCodeFailureDoesNotRecordClient(t *testing.T) {
	...
	if code, err := c.CreateOAuthClientAuthorizationCode(ctx, request, "https://code-failure.example/callback", GenerateCodeChallenge("verifier"), "S256", mustCurrentAuthGeneration(t, c, member.Id), time.Time{}); err == nil || code != "" {
...
func TestOAuthClientAuthorizationRecordFailureDiscardsCode(t *testing.T) {
	...
	if code, err := c.CreateOAuthClientAuthorizationCode(ctx, request, "https://record-failure.example/callback", GenerateCodeChallenge("verifier"), "S256", mustCurrentAuthGeneration(t, c, member.Id), time.Time{}); !errors.Is(err, ErrInvalidArgument) || code != "" {
...
func TestOAuthClientAuthorizationPostCommitWaitFailureKeepsCode(t *testing.T) {
	...
	code, err := c.createOAuthClientAuthorizationCode(ctx, request, "https://wait-failure.example/callback", GenerateCodeChallenge("verifier"), "S256", mustCurrentAuthGeneration(t, c, member.Id), time.Time{}, func(context.Context, events.StreamPosition) error {
		return waitErr
	})
	if err != nil || code == "" {
...
func TestOAuthClientBlockEventInvalidatesTokenOnAnotherReplicaBeforeCleanup(t *testing.T) {
	...
	token, err := first.CreateOAuthAccessTokenForClient(ctx, member.Id, clientID, generation, time.Time{})
	if err != nil {
