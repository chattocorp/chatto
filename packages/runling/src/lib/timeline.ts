...
export interface TaskMessage {
  ...
  from: string;
  to: string;
  status: 'Queued' | 'Read by task' | 'Consumed by agent' | 'Not consumed by agent';
}
...
export function isActivityActive(activity: Activity): boolean {
  if (
    activity.segments?.some((segment) => segment.kind === 'input' && segment.status === 'running')
  )
    return false;
  const hasRunningDescendant = (node: Activity): boolean =>
    node.children.some((child) => child.status === 'running' || hasRunningDescendant(child));
  return (
    activity.status === 'running' && activity.kind !== 'input' && !hasRunningDescendant(activity)
  );
}

export function buildTimeline(events: RunlingEvent[], runStatus: RunStatus): Activity[] {
  ...
  for (const [index, event] of events.entries()) {
    ...
    if (event.type === 'conversation.started' && event.activityId)
      conversations.add(event.activityId);
    ...
    if (event.type === 'agent.action') {
      ...
      if (node) {
        ...
        if (
          ...
          !/^(?:Agent started|Agent finished|Turn \d+|Tokens:|Token usage:)/.test(event.action)
        ) {
  ...
  const links = new Map(
    events.flatMap((event) =>
      event.type === 'task.linked' ? [[event.channelId, event.taskId] as const] : []
    )
  );
  const reads = new Set(
    events.flatMap((event) => (event.type === 'message.read' ? [event.id] : []))
  );
  const receipts = new Map(
    events.flatMap((event) =>
      event.type === 'message.receipt' ? [[event.id, event.consumed] as const] : []
    )
  );
  ...
  for (const id of conversations) {
    const node = nodes.get(id);
    if (!node) continue;
    node.segments = node.children.filter(
      (child) => child.kind === 'agent' || child.kind === 'input'
    );
  ...
  const aggregate = (node: Activity): void => {
    ...
    if (node.kind === 'step')
      node.usage = mergeUsage(
        [...node.children, ...(node.segments ?? [])].flatMap((child) =>
...
export function findActivity(nodes: Activity[], id: string): Activity | undefined {
