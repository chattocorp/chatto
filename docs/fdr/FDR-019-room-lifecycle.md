...
- **Manage members** — `room.manage` holders can list, inspect, add, or remove human members of channel rooms, including when they are not themselves members or eligible to join. Adding can bring a user into a private room even when that user could not self-join through `room.join`. Bot accounts are not ordinary room members and use their separate room-approval model. Active room bans still block adding; the user must be unbanned first. DM membership remains visible only to its participants.
