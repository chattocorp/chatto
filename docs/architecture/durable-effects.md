...
| Client-sync privacy cleanup after account deletion | `client_sync.{userId}.deletion_preparing` exclusively fences the command before commit; post-commit `deletion_pending` identifies retry work; retained `deleted` records committed deletion | The command creates the preparation, commits the account event, creates the retry and retained markers, then purges preferences and the known-server directory | After projection catch-up, every replica scans only committed retry markers for cleanup and separately revision-purges abandoned preparations; a post-write fence check handles racing mutations | Recoverable and idempotent; expiring a preparation cannot discard committed retry work, pending is removed after success, and deleted is retained |
