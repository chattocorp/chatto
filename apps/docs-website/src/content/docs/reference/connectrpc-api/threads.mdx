...
| `unread_only` | `bool` | Return only threads with replies that the viewer has not read. The page total then counts only these threads. Older servers ignore this field and return all followed threads. |
