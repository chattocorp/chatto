...
<EnvVar name="CHATTO_CLIENT_SYNC_ENABLED" toml="client_sync.enabled" default="false">
  Let authenticated users choose this server as their home server and synchronise portable settings plus their public server list through it. Enabling this stores two caller-scoped records per participating user in `RUNTIME_STATE`; passwords, sessions, and bearer tokens are never included. The public discovery API advertises the capability so clients only offer eligible servers.
</EnvVar>
