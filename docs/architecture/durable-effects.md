...
| Video derivative processing | `AssetProcessingStartedEvent`; `AssetProcessingSucceededEvent` or `AssetProcessingFailedEvent` is terminal. Failed events record the derivative IDs created by that attempt; losing terminal attempts and ambiguous derivative creation use dedicated cleanup/reconciliation facts; each success attempt is preceded by an exact-event reconciliation guard | A process-local callback starts bounded ffmpeg work, creates temporary MP4 renditions, uploads a thumbnail and HLS segments, then publishes the terminal manifest; animated GIF loops upload one MP4 derivative. Failure publication has an independent bounded context and precedes prompt cleanup | Each enabled video service scans the asset projection at boot for assets without a completed/failed outcome, including `STARTED`-only attempts, and resubmits them. Terminal event OCC prevents later manifest replacement. The elected asset-cleanup worker consumes failed outcomes and losing-generation cleanup requests. A success guard waits through a grace period, retains output if its exact event ID committed, or records failure and cleans the output otherwise. Ambiguous derivative creation retains canonical storage metadata so recovery can clean both committed and uncommitted writes. Normal deletion recovery removes bytes idempotently after timeout, restart, or lease handover | Recoverable scheduling, ambiguous commits, losing generations, and partial-derivative cleanup; processing ownership remains process-local, so duplicate work remains possible across replicas |
...
| Asset binary and transform-cache deletion | `AssetDeletedEvent` makes projected reads and signed asset resolution reject the asset; the asset ID locates the canonical aggregate's durable creation metadata | Message deletion, attachment removal, account cleanup, pending-upload expiry, and derivative cleanup delete NATS/S3 bytes and cached transforms after recording deletion | The elected `asset_cleanup` worker consumes canonical failed-generation, losing-generation, ambiguous-commit, and deletion lanes. It validates cleanup IDs against immutable child `asset_created` facts, appends tombstones, then loads storage metadata from those facts and retries idempotent binary/cache deletion. A source-video tombstone also re-reads its durable HLS manifest and tombstones any still-live HLS children, repairing deletion by an older HLS-unaware replica; beta room-scoped facts without a canonical creation aggregate are skipped | Recoverable for canonical message-owned asset deletion facts, all durably signalled video-generation cleanup, and mixed-version HLS cleanup; beta room-scoped cleanup remains best-effort |
...
coverage; video processing covers boot resubmission, ambiguous terminal commits,
terminal manifest races, and replayed cleanup of failed generations;
