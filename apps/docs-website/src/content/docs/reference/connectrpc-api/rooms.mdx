...
| `page` | [`PageInfo`](/reference/connectrpc-api/types/#chatto-api-v1-PageInfo) | Page metadata. |
| `latest_pin_event_id` | `string` | Opaque identity of the latest pin fact observed for this room. The value remains stable when pins are removed so clients can detect new pins without comparing wall-clock timestamps. |


<a id="chatto-api-v1-RoomService-BatchGetPinnedMessages"></a>

### BatchGetPinnedMessages

Gets current pins for a bounded set of messages. Room membership is
required; unknown and unpinned messages are omitted.

```http
POST /api/connect/chatto.api.v1.RoomService/BatchGetPinnedMessages
```

<a id="chatto-api-v1-BatchGetPinnedMessagesRequest"></a>

#### Input: BatchGetPinnedMessagesRequest
...
| --- | --- | --- |
| `room_id` | `string` | Required channel room ID. |
| `message_event_ids` | `repeated string` | Required canonical message event IDs. Unknown and unpinned messages are omitted from the response. |


<a id="chatto-api-v1-BatchGetPinnedMessagesResponse"></a>

#### Result: BatchGetPinnedMessagesResponse

Current pins found for a bounded set of messages.

| Field | Type | Description |
| --- | --- | --- |
| `pinned_messages` | repeated [`PinnedMessage`](/reference/connectrpc-api/types/#chatto-api-v1-PinnedMessage) | Pinned messages in first-seen request order, with repeated IDs de-duplicated. |
