...
| `unread_only` | `optional bool` | When true, returns only threads with unread replies. Filtering is applied before pagination. |
...
| `unread_only` | `optional bool` | Confirms whether unread-only filtering was applied by the server. Older servers omit this field so clients can fall back to local filtering. |
