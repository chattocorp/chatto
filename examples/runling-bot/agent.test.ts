...
async function fixture(
  run: (emit: (event: object) => void, restart: () => Promise<void>) => Promise<object>,
  ...
  const createAgent: typeof agent = async (options) => {
    ...
    await (extension as (api: AgentExtensionAPI) => unknown)({
      ...
      setActiveTools() {
        assert.fail("Do not override Runling's tool availability on any prompt");
      },
...
for (const mode of ['missing', 'invalid', 'exception']) {
  test(`${mode} report hides intermediate text and sends only an error`, async () => {
    ...
    await result.sender.notifyFailure();
    assert.deepEqual(result.posted, ["Sorry, I couldn't generate a reply. Please try again."]);
  });
}
