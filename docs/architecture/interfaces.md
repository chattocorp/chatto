...
| Browser authentication | `GET /auth/browser/csrf`, `POST /auth/browser/login`, `POST /auth/browser/register/complete`, `POST /auth/browser/logout`, `POST /auth/browser/session/renew`, `POST /auth/browser/revoke-bearer-session` | Bound CSRF-proof repair, cookie-only password/registration authentication, logout, stable-handle session renewal, and removal of stored origin bearer authority | Every mutation requires JSON, the browser-auth mode header, and an exact same-origin request. Renewal and logout also require signed double-submit CSRF proof. The safe CSRF route requires a valid cookie session. These routes do not return bearer credentials. |
