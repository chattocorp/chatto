...
- Pre-0.5 browser authentication cookies are not accepted by the SCS-backed
  `chatto_auth_*` path. Affected users must sign in once after the upgrade. After
  that sign-in, active browser sessions renew automatically without user action.
