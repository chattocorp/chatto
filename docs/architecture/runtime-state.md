...
| `push_subscription_account_deleted.{userId}` | One-byte, 24-hour account-deletion fence. The durable `UserAccountDeletedEvent` is the permanent authority; this bounded runtime marker cheaply rejects and reconciles a subscription registration that was already authorised when deletion committed and completed after the durable cleanup worker's first pass. |
