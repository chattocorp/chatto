...
- Chatto generates a fresh per-message data encryption key (DEK).
- The body is encrypted with **XChaCha20-Poly1305** using that DEK.
- The DEK is wrapped with the author's per-user key encryption key (KEK).
- The encrypted body is authenticated with its event context — event ID, room ID, author ID, and message-body event type — as Additional Authenticated Data (AAD).
...
2. **Deletes the user's KEK**, rendering remaining encrypted body bytes and wrapped per-message DEKs permanently unreadable.
