...
<EnvVar name="CHATTO_ADMIN_API_ENABLED" toml="admin_api.enabled" default="false">
  Enable the operator-only ConnectRPC AdminService. Keep disabled unless you need token-based automation through `chatto admin ...` or another trusted operator client.
</EnvVar>

<EnvVar name="CHATTO_ADMIN_API_LISTENER_ENABLED" toml="admin_api.listener.enabled" default="false">
  Serve AdminService on its dedicated HTTP listener. Required when `admin_api.enabled = true`; AdminService is never mounted on the public web listener.
</EnvVar>

<EnvVar name="CHATTO_ADMIN_API_LISTENER_BIND_ADDRESS" toml="admin_api.listener.bind_address" default="127.0.0.1">
  Address to bind the dedicated AdminService listener. Keep this on localhost unless a trusted private network or sidecar needs direct access.
</EnvVar>

<EnvVar name="CHATTO_ADMIN_API_LISTENER_PORT" toml="admin_api.listener.port" default="4021">
  Port for the dedicated AdminService listener.
</EnvVar>

<EnvVar name="CHATTO_ADMIN_API_TOKENS_<index>_NAME" toml="admin_api.tokens[].name">
  Counted token name for an AdminService bearer token. Indexes start at `0` and must not have gaps.
</EnvVar>

<EnvVar name="CHATTO_ADMIN_API_TOKENS_<index>_TOKEN" toml="admin_api.tokens[].token">
  Bearer token accepted by the AdminService for this named token entry. Tokens must be at least 32 characters. At least one token entry is required when `admin_api.enabled = true`. Treat each token like a root operator secret.
</EnvVar>

<EnvVar name="CHATTO_ADMIN_API_TOKENS_<index>_ALLOWED_CIDRS" toml="admin_api.tokens[].allowed_cidrs" default="127.0.0.1/32,::1/128">
  Comma-separated CIDR ranges allowed to use this AdminService token. The server checks the direct TCP peer address, not forwarded headers.
</EnvVar>
