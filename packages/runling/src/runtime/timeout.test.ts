...
test('workflow timeout includes input waits and preserves usage and its first abort reason', async () => {
  ...
  const run = runWorkflow(
    async (context) => {
      ...
      try {
        await input(ctx, 'Question', { timeout: 100 });
      } finally {
        expect(() => ctx.abort('later reason')).toThrow(TimeoutError);
      }
    },
    {
      input: undefined,
  ...
  expect(await run).toMatchObject({
    ok: false,
    error: 'Workflow timed out after 2 seconds',
    usage: { input: 5, cost: 0.1 }
  });
...
test('input timeout is catchable without failing the workflow', async () => {
  ...
  const run = runWorkflow(
    async (ctx) => {
      try {
        return await input(ctx, 'Question', { timeout: 1 });
      } catch (error) {
        if (error instanceof TimeoutError) return 'default';
        throw error;
      }
    },
...
test('concurrent workflow deadlines are isolated', async () => {
  vi.useFakeTimers();
  const slow = runWorkflow((ctx) => input(ctx, 'Wait'), {
    input: undefined,
    timeout: 1,
    onInput: () => new Promise(() => {})
  });
  const answer = Promise.withResolvers<string>();
  const other = runWorkflow((ctx) => input(ctx, 'Other'), {
    input: undefined,
    timeout: 10,
    onInput: () => answer.promise
  });
  await vi.advanceTimersByTimeAsync(1000);
...
test.each([-1, NaN, Infinity, 2_147_483.648])(
  'invalid timeout %s rejects before user work',
  async (timeout) => {
    const work = vi.fn();
