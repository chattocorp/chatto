...
func TestMigrateUsersToES_SeedsEncryptedUserAggregateAndReplays(t *testing.T) {
	...
	require.IsType(t, &corev1.Event_UserContentKeyGenerated{}, eventsBySeq[0].GetEvent())
	require.IsType(t, &corev1.Event_UserAccountCreated{}, eventsBySeq[1].GetEvent())
	require.IsType(t, &corev1.Event_UserPasswordHashChanged{}, eventsBySeq[2].GetEvent())
	require.IsType(t, &corev1.Event_UserAvatarSet{}, eventsBySeq[3].GetEvent())
	...
	contentKey := eventsBySeq[0].GetUserContentKeyGenerated()
	require.Equal(t, "U1", contentKey.GetUserId())
	require.EqualValues(t, 1, contentKey.GetEpoch())
	require.NotEmpty(t, contentKey.GetEncryptedContentKey())
	require.NotEmpty(t, contentKey.GetContentKeyNonce())
	require.NotEmpty(t, contentKey.GetWrappingKeyRef())
	unwrappedContentKey, err := unwrapMigrationContentKey(ctx, keyWrapper, contentKey)
	require.NoError(t, err)

	account := eventsBySeq[1].GetUserAccountCreated()
	...
	require.NotContains(t, string(eventsBySeq[1].GetUserAccountCreated().GetEncryptedLogin().GetEncryptedValue()), "Alice")
	login := decryptImportedUserString(t, unwrappedContentKey, eventsBySeq[1].GetId(), "U1", events.EventUserAccountCreated, "login", account.GetEncryptedLogin())
	require.Equal(t, "Alice", login)
	displayName := decryptImportedUserString(t, unwrappedContentKey, eventsBySeq[1].GetId(), "U1", events.EventUserAccountCreated, "display_name", account.GetEncryptedDisplayName())
	require.Equal(t, "Alice A.", displayName)

	email := eventsBySeq[4].GetUserVerifiedEmailAdded()
	require.NotNil(t, email.GetEncryptedEmail())
	emailPlaintext := decryptImportedUserString(t, unwrappedContentKey, eventsBySeq[4].GetId(), "U1", events.EventUserVerifiedEmailAdded, "email", email.GetEncryptedEmail())
	require.Equal(t, "Alice@Example.com", emailPlaintext)
	require.True(t, eventsBySeq[4].GetCreatedAt().AsTime().Equal(verifiedAt))
...
func TestMigrateUsersToES_AppendsEncryptedRepairForPlaintextUserEVTPrefix(t *testing.T) {
	...
	eventsBySeq := readUserMigrationEvents(t, ctx, stream, 4)
	require.Nil(t, eventsBySeq[0].GetUserAccountCreated().GetEncryptedLogin())
	require.IsType(t, &corev1.Event_UserContentKeyGenerated{}, eventsBySeq[1].GetEvent())
	...
	contentKey, err := unwrapMigrationContentKey(ctx, keyWrapper, eventsBySeq[1].GetUserContentKeyGenerated())
	require.NoError(t, err)
	account := eventsBySeq[2].GetUserAccountCreated()
	require.Equal(t, "Alice", decryptImportedUserString(t, contentKey, eventsBySeq[2].GetId(), "U1", events.EventUserAccountCreated, "login", account.GetEncryptedLogin()))
	email := eventsBySeq[3].GetUserVerifiedEmailAdded()
	require.Equal(t, "Alice@Example.com", decryptImportedUserString(t, contentKey, eventsBySeq[3].GetId(), "U1", events.EventUserVerifiedEmailAdded, "email", email.GetEncryptedEmail()))
