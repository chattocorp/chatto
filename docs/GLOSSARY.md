...
**App Preferences** — User choices owned by the current app and applied across every registered server, such as appearance, language, message editor, and send-key behavior. They are opened from the Application Header, use a sidebar with Appearance, Language, and Composer pages, and do not sync to another browser or device. See [FDR-022](fdr/FDR-022-user-profile.md) and [FDR-032](fdr/FDR-032-message-formatting.md).

**User Preferences** — A user's choices for one server, such as time and region or notification behavior. A preference may be server-synced or stored by the app with a per-server key; the term describes its user and server scope, not its storage mechanism. The User Preferences group is part of the unified Settings sidebar. See [FDR-012](fdr/FDR-012-notifications.md) and [FDR-022](fdr/FDR-022-user-profile.md).

**Server Configuration** — Permission-gated configuration that affects a server or its managed resources, as distinct from one user's User Preferences. The Server Configuration group shares the unified Settings sidebar with User Preferences. See [FDR-020](fdr/FDR-020-server-branding-and-configuration.md) and [FDR-021](fdr/FDR-021-admin-dashboard.md).
...
**Server Sidebar** — The wider sidebar to the right of the Server Gutter. It normally owns a server-scoped pane's chrome (positioning, mobile slide, resize, and current-user footer), while App Preferences reuses the same shell without a server-scoped footer. Its contents are typically the server banner and room list, the unified Settings navigation, or the App Preferences navigation. Implemented in `apps/frontend/src/lib/components/ServerSidebar.svelte`.
