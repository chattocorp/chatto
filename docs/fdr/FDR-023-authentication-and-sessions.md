...
- **Password and account lifecycle revocation** — password resets and password changes add a cutoff marker to `auth_revoked_before.{userId}` before the new password hash event is appended, then leave the marker behind only when the append succeeds. Cookie sessions, bearer tokens, and OAuth authorization-code exchange compare their authentication time to the maximum cutoff marker, so credentials authenticated before the cutoff are rejected even if their runtime records are created after a revoke scan starts. Revoke-all scans still delete matching `cookie_session.*` and `session.*` records as cleanup. Account deletion also best-effort deletes a user's cookie sessions and bearer tokens.
...
**Decision:** Cross-origin clients (multi-instance frontend, CLI tools) authenticate via opaque bearer tokens stored in `RUNTIME_STATE` under HMAC-derived `session.{hmac}` keys. Tokens are validated by KV lookup plus the per-user `auth_revoked_before.{userId}` maximum cutoff marker; single-token revocation is one delete, and user-wide cleanup scans `session.*` for records owned by that user.
