...
- Server member reads moved from `ServerMemberService` to `ServerService`: replace
  `ServerMemberService.ListMembers`, `GetMember`, and `BatchGetMembers` with
  `ServerService.ListMembers`, `GetMember`, and `BatchGetMembers`. User directory
  reads were also folded into `ServerService`: replace `UserDirectoryService.GetUser`
  with `ServerService.GetMember` and `UserDirectoryService.BatchGetUsers` with
  `ServerService.BatchGetMembers`.
...
- Message attachment URL refreshes were folded into message reads: replace
  `MessageService.RefreshMessageAttachmentUrls` with `MessageService.GetMessage` and
  `MessageService.BatchRefreshMessageAttachmentUrls` with
  `MessageService.BatchGetMessages`. Attachment URLs are returned on the message's
  `RoomTimelineAttachment` fields.
