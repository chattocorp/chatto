...
Use `user get --login` to find one account by its login. This helps a script recover the user ID after an interrupted run. Supply either a user ID or `--login`, not both.
