...
Typed, versioned protocol capabilities are distinct from server feature
...
optional capability should disable or degrade the affected feature, not make
the whole server unusable. New required client behaviour must be negotiated or
accompanied by an explicit minimum bundled-client version.
