...
| `/api/connect/chatto.api.v1.ExternalIdentityService/DisconnectExternalIdentity`                | `ExternalIdentityService`        | `DisconnectExternalIdentity`    | Authenticated self-service; cannot remove the last sign-in method.              | Disconnect a provider identity from the current account.                                      |
...
| `/api/connect/chatto.api.v1.AccountService/SetPassword`                                      | `AccountService`                 | `SetPassword`                   | Authenticated self-service.                                                      | Add the first password credential, or change an existing password with current-password proof. |
...
| `/api/connect/chatto.api.v1.AdminUserManagementService/SetUserPassword`                      | `AdminUserManagementService`     | `SetUserPassword`               | Authenticated; self-target or requires `role.assign`.                          | Set a user's password as an admin action and revoke their runtime credentials.                |
...
| `external_identity_create.{hmac}` | Pending external provider identity create-flow JSON. Created only for unlinked provider callbacks where that provider has `auto_provision = true`; uses per-key 15-minute TTL and is deleted on account creation or cancellation. |
| `external_identity_link.{hmac}` | Pending external provider identity link-flow JSON bound to the authenticated user who started provider linking. Uses per-key 15-minute TTL and is deleted on link or cancellation. |
| `external_identity_link_start.{hmac}` | Pending provider-link browser handoff JSON created by authenticated `StartExternalIdentityLink`. Uses per-key 15-minute TTL and is consumed before the OAuth/OIDC provider redirect begins. |
| `session.{hmac}` | Opaque bearer auth token JSON with the user auth generation it was issued against. Uses per-key `auth.token_ttl` (default 90 days); successful validation refreshes the key with a new per-key TTL for sliding-window expiry. Password resets, password changes, and account deletion revoke all older bearer tokens by advancing the user's auth generation through durable user events; adding the first password to a passwordless account marks the password event to preserve existing credentials. Scans of `session.*` delete matching records as cleanup. |
