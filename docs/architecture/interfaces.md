...
| Incoming bot webhook | `POST /webhooks/bots/{botId}` | JSON root text-message command; write-only and at-least-once | Bot API key must identify the path bot; `messages.write`, active owner authority, and explicit channel installation are rechecked at commit |
