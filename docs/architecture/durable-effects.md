...
| Client-sync privacy cleanup after account deletion | Retained `client_sync.{userId}.deleted` fence blocks later writes; `client_sync.{userId}.deletion_pending` identifies unfinished cleanup | Account deletion creates both markers and purges preferences and the known-server directory | Every replica periodically scans only pending markers; a post-write fence check purges mutations that raced deletion | Recoverable and idempotent; the pending marker is removed after success while the fence is retained |
