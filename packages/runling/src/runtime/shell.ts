...
import { execa, ExecaError, type Options, type TemplateExpression } from 'execa';
...
type ShellExpression =
  string | number | boolean | null | undefined | { raw: string } | readonly ShellExpression[];
...
export class Command implements PromiseLike<ShellOutput> {
  ...
  constructor(private readonly launch: (options: Options) => PromiseLike<ProcessResult>) {}
  cwd(path: string) {
  ...
  then<TResult1 = ShellOutput, TResult2 = never>(
    fulfilled?: ((value: ShellOutput) => TResult1 | PromiseLike<TResult1>) | null,
...
export function createShell(options: CreateShellOptions = {}) {
  return function (...args: ShellArguments) {
    ...
    const command = strings
      .map(
        (part, index) =>
          part + (index < expressions.length ? formatExpression(expressions[index]) : '')
      )
      .join('');
    return track(
...
export function createExec(options: CreateShellOptions = {}) {
  return function (strings: TemplateStringsArray, ...expressions: TemplateExpression[]) {
    const preview = strings
      .map(
        (part, index) =>
          part + (index < expressions.length ? JSON.stringify(expressions[index]) : '')
      )
      .join('');
    return track(
...
function track(command: Command, formattedCommand: string, options: CreateShellOptions) {
  ...
  logCommand(id, `${log.highlight('Running', COMMAND_COLOR)} ${formattedCommand}`);
...
function formatCommand(...[strings, ...expressions]: ShellArguments) {
  const command = strings
    .map((part, index) => {
      ...
      return expression === undefined ? part : `${part}${formatExpression(expression)}`;
    })
    .join('')
    .trim()
    .replaceAll('\n', ' ');
...
function formatExpression(expression: ShellExpression): string {
  ...
  if (typeof expression === 'object' && expression !== null && 'raw' in expression) {
