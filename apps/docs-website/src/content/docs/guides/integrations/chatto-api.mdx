...
| MCP | Configured canonical `/mcp` resource on a separate listener | Experimental agent tool discovery and bounded user-scoped operations. |
...
member changes also arrive as authoritative projection operations. A live
notification replacement can carry optional transition metadata for effects
such as sounds; bootstrap, resume reconciliation, and reset omit that metadata.
Mention and new-DM attention use notification occurrences, not separate
transient hints. The remaining transient envelopes are typing, presence, and
session termination.
...
| React to bot activations | `notification_occurrences_replace` and `NotificationService.ListNotificationOccurrences` |
