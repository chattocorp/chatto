...
export class RunlingDashboard implements Component {
  ...
  renderHeader(width: number): string[] {
    ...
    const status = this.execution
      ? this.execution.ok
        ? paint('limegreen', '✓')
        : paint('crimson', '✗')
      : paint('dodgerblue', this.spinner());
    return [fit(`${bold(paint('dodgerblue', '◆ Runling'))}  ${this.title}  ${status}`, columns)];
  }
  ...
  renderTranscript(width: number): string[] {
    ...
    if (result !== undefined && result !== null) {
      const markdown =
        result.details === undefined ? result.summary : `${result.summary}\n\n${result.details}`;
  ...
  renderFooter(width: number): string[] {
    const columns = Math.max(1, Math.floor(width));
    const elapsed = formatElapsed(this.execution?.durationMs ?? performance.now() - this.startedAt);
    return [dim('─'.repeat(columns)), fit(this.footer(elapsed), columns)];
  }

  private footer(elapsed: string): string {
    const steps = this.nodes.filter((node): node is StepNode => node.type === 'step');
    const completed = steps.filter(({ status }) => status === 'completed').length;
    const failed = steps.filter(({ status }) => status === 'failed').length;
    ...
    const parts = [status, elapsed, `${tokens.toLocaleString('en-US')} tokens`];

    const cacheableInput = this.usage.input + this.usage.cacheRead;
    if (this.usage.cacheRead > 0 && cacheableInput > 0) {
      parts.push(`${Math.round((this.usage.cacheRead / cacheableInput) * 100)}% cached`);
    }
    ...
    switch (node.type) {
      ...
      case 'command': {
        ...
        if (node.status === 'failed' && node.output !== undefined) {
          for (const outputLine of failureOutput(node.output)) {
            lines.push(
              ...this.wrapNode(`${indent}  ${paint('crimson', '│')} `, dim(outputLine), width)
            );
          }
        }
        break;
      ...
      case 'agent': {
        ...
        if (node.outcome === undefined) {
          lines.push(...this.wrapNode(`${indent}  ${paint(node.color, '↳')} `, node.action, width));
        }
        break;
      }
      case 'input': {
        ...
        if (node.status === 'waiting' && editor !== undefined) {
          ...
          const editorLines = editor.render(Math.max(1, width - visibleWidth(prefix)));
          lines.push(
          ...
          event.level === 'success'
            ...
            : event.level === 'error'
              ...
              : event.level === 'debug'
                ? '·'
                : '●';
        lines = this.wrapNode(`${indent}${paint(event.color, marker)} `, event.message, width);
        break;
      }
    }
  ...
  private spinner(): string {
    return SPINNER[Math.floor((performance.now() - this.startedAt) / 80) % SPINNER.length]!;
  }
}
...
const fit = (line: string, width: number): string => truncateToWidth(line, width, '');
...
const formatCost = (cost: number): string =>
  cost === 0 ? '$0.00' : cost < 0.0001 ? '<$0.0001' : `$${cost.toFixed(cost < 0.01 ? 4 : 2)}`;

const COMMAND_FAILURE_LINES = 8;

const failureOutput = ({ stdout, stderr }: { stdout: string; stderr: string }): string[] => {
