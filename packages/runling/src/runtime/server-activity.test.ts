...
test('concurrent runs and nested tasks retain distinct stable prefixes without payloads', () => {
  ...
  expect(
    vi.mocked(serverLog).mock.calls.map((call) => [call[2]?.runReference, call[2]?.taskReference])
  ).toEqual([
    ['bright-waves-1234', 'task-1'],
    ['bright-waves-1234', 'task-2'],
    ['happy-toes-5678', 'task-1'],
    ['bright-waves-1234', 'task-2'],
    ['bright-waves-1234', 'task-1'],
    ['bright-waves-1234', 'task-2']
  ]);
...
test('throttles agent activity but always reports tool failures and completion', () => {
  ...
  expect(vi.mocked(serverLog).mock.calls.map((call) => [call[0], call[2]?.activity])).toEqual([
    ['info', 'Agent started · model'],
    ['error', 'Tool read failed'],
    ['error', 'Task failed · 5 ms']
  ]);
...
test('summarizes successful calls, reports failures immediately, and associates host milestones', () => {
  ...
  expect(vi.mocked(serverLog).mock.calls.map((call) => call[2]?.activity)).toEqual([
    'Agent started · test/model',
    'Activity · 100 read, 1 search',
    'Tool preparePullRequest failed',
    'Validation failed'
  ]);
  expect(vi.mocked(serverLog).mock.calls[1]?.[2]).toMatchObject({
...
test('reports silence without claiming progress and stops reminders after agent completion', () => {
  ...
  try {
    ...
    expect(vi.mocked(serverLog).mock.calls.at(-1)).toMatchObject([
      'warn',
      'run.activity',
      {
        activity: 'No recorded agent activity for 2 min; agent still running',
        agentLabel: 'implement'
      }
    ]);
    write({
    ...
    expect(
      vi
        ...
        .mock.calls.filter((call) =>
          String(call[2]?.activity).startsWith('No recorded agent activity')
        )
    ).toHaveLength(1);
    write({
      type: 'agent.finished',
