...
| `bot_api_key.{botId}` | The bot's single indefinite API-key record containing its bot ID, issuance time, and a domain-separated HMAC verifier. The raw `cht_BK...` key is shown only when rotated and is never stored. Revision-conditional replacement immediately invalidates the previous key; explicit revocation and account deletion remove the record. |
