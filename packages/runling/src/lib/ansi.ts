...
export function ansiTokens(text: string) {
  return parseAnsiSequences(text).map((token) => {
    const styles: string[] = [];
    let foreground = token.foreground ? palette.value(token.foreground) : undefined;
    let background = token.background ? palette.value(token.background) : undefined;
    if (token.decorations.has('reverse'))
      [foreground, background] = [background ?? '#ffffff', foreground ?? '#24324b'];
