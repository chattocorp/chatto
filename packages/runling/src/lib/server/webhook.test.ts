...
test('accepts schema-free routing functions and preserves webhook names', async () => {
  ...
  const response = await handleWebhook('custom', request('{"event":"reply"}'), {
    config: custom,
    start
  });

  expect(await response.json()).toEqual({ runs: [] });
  expect(routed).toHaveBeenCalledWith(expect.objectContaining({ start: expect.any(Function) }), {
    event: 'reply'
  });
  expect(start).not.toHaveBeenCalled();
  expect(await describeWebhook('custom', { config: custom }).json()).toEqual({
    input: {},
    output: {}
  });
});
...
test('a router can select tasks, map input, and fan out sequentially or concurrently', async () => {
  const h = host();
  const number = task(
    { name: 'Number', input: Type.Number(), output: Type.Number() },
    (_ctx, input) => input * 2
  );
  const custom = defineWebConfig({
    webhooks: {
  ...
  const response = await handleWebhook('fanout', request('{"name":"Ada","count":3}'), {
    config: custom,
    start: h.start
  });
...
test('the convenience router also supports ordinary tasks without schemas', async () => {
  ...
  const response = await handleWebhook('plain', request('{"name":"Ada"}'), {
    config,
    start: h.start
  });
  expect(response.status).toBe(202);
...
test('accepts Valibot schema exports', async () => {
  ...
  expect((await h.executions[0])?.output).toBe('hello');
  expect(await describeWebhook('echo', { config }).json()).toMatchObject({
    input: { type: 'string' },
    output: { type: 'string' }
  });
});
...
test.each([
  {
    schema: Type.Object({ count: Type.Integer({ minimum: 1 }) }),
    value: { count: 2 },
    invalid: { count: 0 }
  },
  ...
  const h = host();
  expect(
    (await handleWebhook('typed', request(JSON.stringify(value)), { config, start: h.start }))
      .status
  ).toBe(202);
  const response = await handleWebhook('typed', request(JSON.stringify(invalid)), {
    config,
    start: h.start
  });
...
test('custom router metadata validates asynchronously but leaves payload parsing to the router', async () => {
  const route = Object.assign(
    vi.fn<WebhookRouter<string>>(async () => {}),
    {
      input: z
        .string()
        .refine(async (value) => value === 'hello', 'Expected hello')
        .transform((value) => value.length)
    }
  );
  const config = defineWebConfig({ webhooks: { custom: route } });
  const h = host();
  expect((await handleWebhook('custom', request('"bad"'), { config, start: h.start })).status).toBe(
    400
  );
...
test.each(['missing', 'constructor', '__proto__', 'toString'])(
  'rejects unknown or inherited name %s',
  async (name) => {
    expect(describeWebhook(name, { config }).status).toBe(404);
    expect((await handleWebhook(name, request(), { config, start: host().start })).status).toBe(
      404
    );
  }
);
...
test.each([null, [], {}, { task: echo }, { task: echo, route: async () => {} }, true])(
  'rejects non-function webhook definitions',
  (value) => {
...
test('rejects invalid schema metadata and unexportable schema tasks', () => {
  ...
  const local = task(
    { name: 'Local', input: v.string(), output: v.string() },
    (_ctx, input) => input
  );
