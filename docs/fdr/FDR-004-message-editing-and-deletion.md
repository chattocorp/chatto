...
- Every authorized edit, attachment removal, and preview removal rechecks mutable authority inside a global-EVT mutation attempt. Any intervening EVT fact forces a retry before commit, so global role and permission revocations already committed to EVT are strict for these edits. Deletions still recheck mutable authority on each room-OCC attempt and remain eventually consistent with an in-flight global revocation.
...
- Creating or removing a channel echo through an edit commits atomically with the parent edit. Echo creation also rechecks `message.echo` and `message.post` authority on each global-EVT attempt.
...
**Decision:** Authorized edits use the global EVT tail through ADR-068's reusable mutation executor. Every attempt waits for current room, group, RBAC, actor, and message state, then rechecks room archive state, membership, current message identity and authorship, the exact author edit-window boundary, and applicable permissions. It rebuilds from the latest committed body and atomically commits the body, semantic edit, and any edit-driven echo change. Internal linked-message propagation stays room-scoped. Deletions continue to use room-wide OCC. Neither path advances the global authorization fence.
**Why:** Reusing a body prepared before an OCC conflict could restore an attachment or preview removed by another mutation, while guarding edit facts independently could let a late body resurrect a deleted message. The global edit boundary closes both those lifecycle races and the prior cross-aggregate authorization race without another fence event. Atomic echo reconciliation prevents partial success. The narrower deletion path is retained as an explicit proof-of-concept limit. See ADR-016, ADR-033, ADR-034, ADR-040, and ADR-068.
**Tradeoff:** Any EVT activity can force an authorized edit to retry, even when unrelated to its room, and five consecutive conflicts fail the operation. Deletions can still overlap a global role or permission revocation until the serving replica projects it. The public API does not currently expose a client revision token, so concurrent full-text replacements resolve in commit order; the later successful edit supplies the visible text while retaining independently committed metadata changes.
