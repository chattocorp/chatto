...
| `evt.user.{userId}.bot_api_key_rotated`                    | `BotApiKeyRotatedEvent`; replace-all HMAC verifier and rotation timestamp. A targeted revocation of a rollback-visible key uses its optional revoked-key ID and an unissued verifier as an old-binary rollback fence |
