...
| Field | Type | Description |
| --- | --- | --- |
| `id` | `string` | Stable event ID of the pin operation. |
| `message` | [`Message`](#chatto-api-v1-Message) | Current renderable message. Retracted messages are omitted from pin lists. |
| `actor` | [`User`](#chatto-api-v1-User) | Message author, when the account is still available. |
| `pinned_by` | [`User`](#chatto-api-v1-User) | User who pinned the message, when the account is still available. |
| `pinned_at` | `google.protobuf.Timestamp` | Time when the message was pinned. |
