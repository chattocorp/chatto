...
async function executeWebFetch(
  ...
  return tool.execute('tool-call', { url }, undefined, undefined, undefined as never);
}
...
test('fetches textual public content with response metadata', async () => {
  ...
  assert.match(result.content[0]?.type === 'text' ? result.content[0].text : '', /Status: 200 OK/);
  assert.match(
...
test('pins each redirect to its validated addresses without a second DNS read', async () => {
  ...
  const tool = loadWebFetchTool(
    ...
    async (hostname) => {
      ...
      if (resolved.filter((value) => value === hostname).length > 1) return ['127.0.0.1'];
...
test('pinned lookup rejects unsafe inputs and unsupported address families', () => {
  for (const addresses of [[], ['127.0.0.1'], ['240.0.0.1'], ['93.184.216.34', '::1']]) {
    assert.throws(() => pinnedPublicLookup(addresses), /public destination addresses/);
  }
...
test('rejects non-HTTP URLs before resolving or fetching', async () => {
  ...
  await assert.rejects(executeWebFetch(tool, 'file:///etc/passwd'), /only supports HTTP and HTTPS/);
...
test('blocks a redirect to a non-public destination', async () => {
  ...
  const tool = loadWebFetchTool(
    ...
    async (hostname) => (hostname === 'public.example' ? ['93.184.216.34'] : [hostname])
  );
