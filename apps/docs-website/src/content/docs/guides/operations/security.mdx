...
| Runtime credential verifiers | `RUNTIME_STATE` (`session.{hmac}` and `renewable_session.{hmac}`, HMAC-keyed, with TTL) | Yes (not raw access tokens, refresh credentials, or cookie handles) |
