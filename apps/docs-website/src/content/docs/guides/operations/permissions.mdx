...
Permission identifiers are dot-separated paths. For example,
`message.post` is a descendant of `message`.

- An **Allow** for `message` also allows every registered `message` descendant.
- A **Deny** for `message` affects only `message`. It does not deny
  `message.post` or another descendant.
- A direct setting for a descendant still takes precedence for that descendant.

Chatto registers every parent path. The permissions page can show parent paths
such as `message` when you need to grant a complete registered permission
family. Chatto has no wildcard permission syntax.
...
Room-relevant permissions such as `message.read`, `message.post`, `message.attach`, `message.react`, `message.manage`, `room.join`, and `room.ban-member` can be configured at room/group/server scope. Server-only permissions such as `server.manage`, `role.manage`, and `admin.view-audit` are configured only at server scope.
...
<Aside type="caution" title="Existing servers need an explicit read grant">
  ...
  scope to preserve the previous channel-room membership-based behavior.
  Chatto does not migrate, backfill, or reconcile existing RBAC state. This
  permission does not control DM reads.
</Aside>
