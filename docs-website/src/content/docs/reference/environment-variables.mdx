...
<EnvVar name="CHATTO_MANAGEMENT_SOCKET_PATH" toml="management.socket_path" default=".chatto/admin.sock">
  Unix socket path for the private management API. Relative config-file paths resolve relative to the `chatto.toml` directory. The server creates the parent directory with private permissions and binds the socket with mode `0600`; existing parent directories must not be accessible by group or other users.
</EnvVar>
