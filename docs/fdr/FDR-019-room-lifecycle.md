...
- **Manage members** — `room.manage` holders can list, inspect, add, or remove human members and bot installations in channel rooms, including when they are not themselves members or eligible to join. Adding can bring a human into a private room even when that user could not self-join through `room.join`. Universal rooms allow explicit bot installation and removal while human membership remains implicit and cannot be edited. Installation never gives a bot room-wide read access. Active room bans still block adding; the user must be unbanned first. DM membership remains visible only to its participants.
