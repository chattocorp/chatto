...
| Network MCP | `/mcp` on the public HTTP listener; `GET /.well-known/oauth-protected-resource/mcp` publishes RFC 9728 metadata when `[mcp].enabled = true` | MCP `2026-07-28` stateless Streamable HTTP with the bounded `list_rooms` tool; the canonical resource uses the `webserver.url` origin | Resource-bound OAuth bearer with `chatto:rooms:read`, or a current bot API key; every tool call also uses the normal operation authorization model |
