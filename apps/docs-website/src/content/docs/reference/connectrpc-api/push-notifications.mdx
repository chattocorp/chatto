...
| `client_host` | `optional string` | URL host of the Chatto server that supplied the installed web app. The sending server combines this with its own hostname to reconstruct the client route. A host can include a port but no scheme, path, or user info. When omitted, notification clicks open this server's bundled /chat/- client. |
...
| `client_host_stored` | `bool` | True when the server persisted client_host. New clients use this acknowledgement before trusting a remote server to reconstruct the installed web app's notification route. |
