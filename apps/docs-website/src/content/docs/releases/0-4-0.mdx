...
- Message posting no longer has a server-side large-mention confirmation challenge.
  ...
  `MessageService.CreateMessage`. `CreateMessageResponse`, `GetMessageResponse`, and
  `BatchGetMessagesResponse` now return only message event data; hydrate user/member
  details through `ServerService.GetMember` or `BatchGetMembers` when needed.
