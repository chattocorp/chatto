...
**Client Preferences** — User choices owned by the current client and applied across every registered server, such as theme, language, message editor, and send-key behavior. They are opened from the Application Header and do not sync to another browser or device. See [FDR-022](fdr/FDR-022-user-profile.md) and [FDR-032](fdr/FDR-032-message-formatting.md).

**Server Preferences** — A user's choices for one server, such as time and region or notification behavior. A preference may be server-synced or stored by the client with a per-server key; the term describes its user and server scope, not its storage mechanism. The Server Preferences group is part of the unified Settings sidebar. See [FDR-012](fdr/FDR-012-notifications.md) and [FDR-022](fdr/FDR-022-user-profile.md).

**Server Settings** — Permission-gated configuration that affects a server or its managed resources, as distinct from one user's Server Preferences. The Server Settings group shares the unified Settings sidebar with Server Preferences. See [FDR-020](fdr/FDR-020-server-branding-and-configuration.md) and [FDR-021](fdr/FDR-021-admin-dashboard.md).
