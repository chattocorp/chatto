<script lang="ts">
  ...
  function handleTabKey(event: KeyboardEvent) {
    ...
    tab = event.key === "Home" ? "log" : event.key === "End" ? "timeline" : tab === "log" ? "timeline" : "log";
...
<section class="flex h-full min-h-0 min-w-0 flex-col" aria-label="Run details">
  <header class="shrink-0 border-b border-base-300 px-4 pt-4 pb-3 sm:px-6">
    <div class="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
      <div class="min-w-0 flex-1">
        <h1 class="m-0 wrap-anywhere text-xl font-semibold tracking-tight sm:text-2xl">{run.workflow}</h1>
        <div class="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-base-content/55 tabular-nums">
          <span>{run.source === "web" ? "Started from web" : run.source === "source" ? `Source ${run.sourceName ?? run.webhook}` : `Webhook /${run.webhook}`}</span>
          <time datetime={new Date(run.startedAt).toISOString()}>{new Date(run.startedAt).toLocaleString()}</time>
      ...
      <div class="flex shrink-0 items-center gap-2">
        ...
        {#if run.status === "running"}
          <button
            ...
            onclick={cancelRun}
          >
            <span class={cancelling ? "icon-[lucide--loader-circle] size-3.5 animate-spin motion-reduce:animate-none" : "icon-[lucide--square] size-3.5"} aria-hidden="true"></span>
    ...
    <div class="mt-2 flex min-w-0 items-center gap-1 text-xs text-base-content/60">
      ...
      <button class="btn btn-ghost btn-square btn-xs shrink-0" onclick={copyReference} aria-label="Copy run reference" title="Copy run reference">
    ...
    <details class="mt-2 text-sm">
      <summary class="w-fit cursor-pointer text-xs text-base-content/65 hover:text-base-content focus-visible:outline-2 focus-visible:outline-primary">Run details</summary>
  ...
  <div class="tabs tabs-border shrink-0 border-b border-base-300 px-4 sm:px-6" aria-label="Run views" role="tablist">
    <button class="tab text-sm" class:tab-active={tab === "log"} role="tab" aria-selected={tab === "log"} aria-controls="run-log-panel" tabindex={tab === "log" ? 0 : -1} onclick={() => (tab = "log")} onkeydown={handleTabKey}>Log</button>
    <button class="tab text-sm" class:tab-active={tab === "timeline"} role="tab" aria-selected={tab === "timeline"} aria-controls="run-timeline-panel" tabindex={tab === "timeline" ? 0 : -1} onclick={() => (tab = "timeline")} onkeydown={handleTabKey}>Timeline</button>
  </div>
  {#if tab === "log"}
  ...
  {:else}
    <div id="run-timeline-panel" class="min-h-0 flex-1 overflow-auto px-4 py-5 sm:px-6" role="tabpanel" aria-label="Timeline">
      ...
      {#if nodes.length}
        ...
        />
        <p class="mt-3 text-xs leading-relaxed text-base-content/60">Select a block to inspect its activity. Collapse a step to focus the timeline.</p>
      {:else}
        <div class="rounded-lg border border-dashed border-base-300 px-5 py-9 text-center text-sm text-base-content/60">
          {run.status === "running" ? "Waiting for the first workflow event…" : "No activity events were recorded for this run."}
        </div>
      {/if}
