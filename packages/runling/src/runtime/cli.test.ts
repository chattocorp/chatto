...
function harness() {
  ...
  return {
    run,
    serve,
    parse: (args: string[]) => cli.parseAsync(args, { from: 'user' }),
    stdout: () => stdout,
    stderr: () => stderr
  };
}
...
describe('CLI commands', () => {
  ...
  test('rejects a prompt combined with JSON input', async () => {
    const h = harness();
    await expect(h.parse(['run', 'workflow.ts', 'prompt', '--input', '{}'])).rejects.toThrow(
      'Use either a prompt or --input'
    );
    expect(h.run).not.toHaveBeenCalled();
  ...
  test.each(['nope', '0', '65536', '1.5', '1e3', '', '-1'])(
    'rejects invalid port %j before starting the server',
    async (port) => {
      const h = harness();
      await expect(h.parse(['serve', `--port=${port}`])).rejects.toThrow(
  ...
  test.each([
    ['run'],
    ['run', 'workflow.ts', 'too', 'many'],
    ['run', 'workflow.ts', '--unknown'],
    ['serve', '--json'],
    ['web'],
    ['workflow.ts'],
    ['--port', '3000'],
  ...
  test.each([
    ...
    ['help', 'run'],
    ['--version']
  ])('shows help or version for %j without executing', async (...args) => {
    const h = harness();
    try {
      await h.parse(args);
    } catch (error) {
