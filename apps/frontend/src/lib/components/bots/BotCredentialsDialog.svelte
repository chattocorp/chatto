...
for generation, rotation, or revocation, followed by the show-once secret after
generation or rotation. Bot creation can open the show-once step directly with
`initialSecret`.
-->
<script lang="ts">
  ...
  let {
    ...
    bot: BotAccount;
    action: 'generate' | 'rotate' | 'revoke' | 'show';
  ...
  let stage = $state<'confirm' | 'show'>(untrack(() => (action === 'show' ? 'show' : 'confirm')));
...
{#if stage === 'confirm'}
  <ConfirmDialog
    ...
    tone={action === 'revoke' ? 'danger' : 'info'}
    actionLabel={action === 'generate'
      ? m('bots.credentials.generate')
      : action === 'rotate'
        ? m('bots.credentials.rotate')
        : m('bots.credentials.revoke')}
    actionIcon={action === 'generate'
  ...
  <Dialog visible title={m('bots.credentials.title', { bot: bot.displayName })} size="md" {onclose}>
    <div class="flex flex-col gap-4">
      ...
      <div class="flex items-center gap-2 rounded-lg bg-surface-emphasized p-3">
        <code
          class="min-w-0 flex-1 text-left text-sm break-all select-all"
