...
`refresh_request_id`. Persist that request ID before sending and reuse it until
the complete response has been persisted; this lets an exact retry recover a
lost successful response. Serialize refresh attempts. An `invalid_grant`
response requires explicit authentication, while network failures and `5xx`
responses may be retried with the same request ID.
