...
- S3 asset storage must keep custom S3-compatible endpoints first-class. When changing S3 client setup, preserve automatic path-style addressing for non-AWS endpoints and cover omitted `path_style` configs with integration tests against a custom endpoint.
