...
- **Named roles decide before `everyone`.** Direct-user and explicitly assigned role decisions are combined first. A deny from any of those subjects wins; otherwise an allow wins.
- **`everyone` is the baseline.** Chatto consults `everyone` only when the user and their named roles have no decision. This lets a named role opt into something denied to ordinary members.
...
| **admin**     | Adds broad server administration defaults, including room administration, role management, user administration, audit access, and `message.manage`. System diagnostics remain owner-only. |
...
4. **Combine named subjects.** Any deny among those decisions wins; otherwise any allow grants the permission. Role position is not considered.
5. **Fall back to `everyone`.** Only if no direct-user or named-role decision applies, Chatto uses `everyone`'s nearest value.
6. **Default deny.** If nothing decides, the action is denied.

For example, `admin: allow` plus `everyone: deny` resolves to **allow**. `admin: allow` plus `suspended: deny` resolves to **deny**.

<Aside type="tip">
  Prefer room permissions for local policy. Use `everyone` for ordinary-member defaults, named roles for reusable access, and a direct-user or restriction-role deny for a hard restriction.
</Aside>
...
This blocks normal members who have no named posting decision. Owners can post, and a named role with its own `message.post` grant can post too.
...
This works as long as no named role or direct-user decision denies `message.manage`. An `everyone` deny does not block the grant because `everyone` is only the fallback baseline.
...
- **Admins** receive broad default grants, but they are still ordinary non-owner users for permission resolution. A direct-user deny or a deny from another named role can restrict an admin. An `everyone` deny alone cannot.
...
<Aside type="caution" title="Authorization behavior changed">
  When upgrading from the older literal deny-wins resolver, review configurations that combine an `everyone` deny with a named-role or direct-user allow. Those allows now win over the baseline and can widen access. Put hard restrictions on the affected user or on a named restriction role such as `suspended`.
</Aside>
