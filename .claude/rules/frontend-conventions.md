...
- Keep visible form action buttons, including compact popover and settings form actions, in one horizontal row with `justify-end gap-2` and `flex-nowrap` where needed. Prefer smaller button sizes, a wider container, or shorter labels over stacking actions vertically; reserve vertical stacks for deliberate mobile/bottom-sheet layouts.
