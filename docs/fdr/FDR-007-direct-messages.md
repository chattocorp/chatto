...
- A user can archive a non-empty DM from the trailing action revealed when its sidebar row is hovered, or from the row's context menu. The hover action replaces any unread or notification indicator for as long as the row is hovered. Archiving removes the conversation only from that user's sidebar; it remains available through direct navigation and the quick switcher.
