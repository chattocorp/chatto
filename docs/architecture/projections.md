...
| Server/user config | Server Config        | `evt.config.>`, selected user cleanup/preference facts     | `ConfigModel`; server config, branding refs, deprecated per-server time preferences for older clients, explicit per-signal-class server/room notification delivery modes, blocked usernames; historical coarse notification-level facts decode but have no projected effect |
