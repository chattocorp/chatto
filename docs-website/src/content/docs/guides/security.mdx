...
- Chatto generates a per-user, purpose-scoped message-body DEK epoch and records the wrapped key in the user event stream as a `UserDEKGeneratedEvent`.
- The body is encrypted with **XChaCha20-Poly1305** using the author's active message-body DEK epoch.
- Each DEK is wrapped through Chatto's KMS boundary with the event's opaque wrapping key reference.
...
The built-in KMS runs in-process and stores KEKs in a dedicated `ENCRYPTION_KEYS` NATS KV bucket under opaque `kek.*` references. DEK events record their purpose, epoch, and wrapping key ref, which keeps Chatto's durable event shape close to external KMS systems such as Vault, AWS KMS, or HSM-backed providers. Authentication tags are verified on every read; tampered ciphertext or ciphertext copied into the wrong event context is rejected with `ErrDecryptionFailed`.
...
2. **Shreds the user's KMS key refs**, rendering remaining encrypted body bytes, durable PII bytes, and wrapped DEK epochs permanently unreadable.
...
Encryption keys are exported and imported separately with `chatto keys export` / `chatto keys import`, again age-encrypted. Restores need those keys to rebuild encrypted profile indexes as well as message bodies. The same passphrase format means a standalone `age` CLI can inspect or re-encrypt any Chatto archive.
...
| ----------------------------------- | ------------------------------------------- | -------------- |
| Per-user KEKs for wrapped DEKs      | `ENCRYPTION_KEYS` (NATS KV)                 | No (export separately) |
