...
test('tool announcements reach the conversation thread before work starts', async () => {
  ...
  try {
    ...
    expect(post).toHaveBeenCalledWith(
      { roomId: 'dm', threadRootId: 'root', inReplyTo: 'reply' },
      "I'll compare those pages in the source.",
...
test('queued replies retain their prompting message and unmentioned replies reach the active inbox', async () => {
  ...
  expect(post.mock.calls.map(([destination]) => destination.inReplyTo)).toEqual([
    'first',
    'second',
    undefined
  ]);
...
test.each(['assistant-first', 'announcement-first', 'paraphrase'])(
  'deduplicates concurrent announcements: %s',
  async (order) => {
    const entered = Promise.withResolvers<void>();
    const release = Promise.withResolvers<void>();
      ...
      async task(ctx, _prompt, options) {
        if (order !== 'announcement-first') {
          ...
          await options.announce(
            order === 'paraphrase' ? "I'm going to check the source now." : "I'll investigate.",
            ctx.signal
          );
        } else {
