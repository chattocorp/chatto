...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="My Threads becomes a conversation inbox">
    ...
    without opening the thread. The Unread filter uses the existing thread read cursor. Mark
    read and Unfollow are available on each conversation.
  </ReleaseFeatureCard>
...
- Regenerate custom clients that read `ThreadViewerState`. The
  `has_unread` field is now `has_unread_replies`, and the new
  `attention_level` field reports notification attention separately. Protobuf
  tag `2` keeps its Boolean wire type, but the source and ProtoJSON names
  change. Deploy 0.5 clients and servers together. Stored thread follows, read
  cursors, and notification state need no migration.
