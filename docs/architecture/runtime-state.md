...
| `push_subscription.{userId}.{endpointHash}` | Web Push subscription record (protobuf `PushSubscription`) for a user's browser/device and service-worker registration. The optional client host identifies the Chatto server that supplied the installed app; the sending server combines it with its own hostname to reconstruct the click route. Client-host-aware records put the provider URL in the additive `delivery_endpoint` field and leave the legacy `endpoint` field empty, so every older sender skips them during a rolling upgrade. Legacy records without a client host use the sending server's bundled-client route. The endpoint hash keeps multiple devices and per-server scoped subscriptions per user while deduplicating the same browser subscription. A record is deliverable only while its revision matches the endpoint's active owner claim. |
| `push_endpoint_owner.{sha256(endpoint)}` | JSON Web Push endpoint owner claim containing the active user ID, exact `push_subscription` KV revision, and (for current writes) the provider endpoint used to verify the referenced record. Saves transfer the claim with KV OCC; revision-matched deletes prevent stale logout, expiry cleanup, and subscription rotation races from releasing a newer claim. The ordinary user/revision fields keep endpoint-based `Unsubscribe` compatible with older replicas. Every new replica reconciles at startup and once per minute, removing claims orphaned when an older replica account-deletes a client-host-aware record whose legacy endpoint field is empty. Legacy subscription records without a claim remain inert until the browser re-registers. |
