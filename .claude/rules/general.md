...
- **E2E tests run locally without any external stack.** Each Playwright test spawns its own `chatto` binary with embedded NATS on a random port (see `e2e/fixtures/server.ts`); they do NOT need `mise dev`, Docker, OrbStack, or a separate test database. Use `mise test-e2e` for the full suite, or `mise x -- pnpm exec playwright test <path>` for a subset (the `build-e2e-server` dependency is already wired into `test-e2e`). After backend changes, rebuild the test binary first with `mise build-e2e-server` — running `playwright test` directly will silently use the stale binary.
