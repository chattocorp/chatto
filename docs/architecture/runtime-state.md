...
| `projection_snapshot_pointer.{opaqueLocator}` | Encrypted current/previous generation IDs for one snapshot projection, projection-local cursor lineage, and key epoch. The opaque locator is versioned when cutoff semantics change, so pointers from an older cursor model cannot block publication. Uses KV revision OCC so stale snapshot writers cannot regress newer history within a lineage. |
...
| `link_preview.{urlHash}` | Versioned cached link preview metadata (protobuf `CachedLinkPreview`) keyed by SHA-256 of the normalized URL. Successful previews use per-key 24-hour TTL; failed fetches use per-key 1-hour TTL. Legacy Bluesky snapshots without the provider-neutral canonical post URL are treated as cache misses. Pre-v1 negative entries refresh once after validated multi-address dialing was added; pre-v1 Mastodon-shaped generic entries also refresh for federated proxy discovery. Current failures and generic fallbacks retain their normal TTL. |
