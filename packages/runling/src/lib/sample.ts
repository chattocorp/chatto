...
export function sample(value: unknown, depth = 0): unknown {
  ...
  if (Array.isArray(branches) && branches[0]) return sample(branches[0], depth + 1);
  if (schema.type === 'object' || schema.properties) {
    return Object.fromEntries(
      Object.entries((schema.properties ?? {}) as Record<string, Record<string, unknown>>).map(
        ([key, value]) => [key, sample(value, depth + 1)]
      )
    );
  }
  if (schema.type === 'array')
    return Array.from({ length: Math.min(5, Number(schema.minItems) || 0) }, () =>
