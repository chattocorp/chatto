...
<a id="chatto-api-v1-AdminUser"></a>
...
| Field | Type | Description |
| --- | --- | --- |
| `user_id` | `string` | Stable user ID. |
| `login` | `string` | Login name used for password sign-in and mentions. |
| `display_name` | `string` | Display name shown in clients. |
| `created_at` | `google.protobuf.Timestamp` | When the account was created, if known. |
| `role_names` | `repeated string` | Explicit server roles assigned to the user. The implicit "everyone" role is not included. |
| `verified_emails` | repeated [`AdminVerifiedEmail`](#chatto-api-v1-AdminVerifiedEmail) | Verified email addresses attached to this user. |

<a id="chatto-api-v1-AdminVerifiedEmail"></a>
...
| `verified_at` | `google.protobuf.Timestamp` | When the email address was verified. |
