...
const pages = Object.fromEntries(
  entries.map(({ id, data }) => [
    ...
    {
      title: data.title,
      description: data.description ?? 'Self-hosting documentation for Chatto.'
    }
  ])
);
...
const formatTitle = (title: string) => {
  ...
  const brandedTitle = spacedTitle.startsWith('Chatto') ? spacedTitle : `Chatto · ${spacedTitle}`;
...
const formatDescription = (description: string) => {
  ...
  const displayDescription =
    description.length > shortened.length ? `${shortened.slice(0, breakAt).trimEnd()}…` : shortened;
