...
| `message.post`            | Post root messages. A human with this permission at DM scope can also start DMs. A bot cannot start a DM. Explicit thread creation also requires `message.post-in-thread`. |
| `message.post-in-thread`  | Post messages inside channel-room or DM threads and, together with `message.post`, explicitly establish a root as a thread. |
