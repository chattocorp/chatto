...
3. The user's browser connects directly to the LiveKit server using the token and a per-room E2EE key from Chatto.
...
Chatto records call join/leave facts durably and reconciles active participants against LiveKit webhooks, so call indicators recover after server restarts. LiveKit media is end-to-end encrypted by current Chatto clients; older clients that do not support this call encryption cannot decode encrypted media.
