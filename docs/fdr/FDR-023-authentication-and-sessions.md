...
- **External provider login** — operators configure repeated `[[auth.providers]]` entries or equivalent counted `CHATTO_AUTH_PROVIDERS_<index>_<field>` environment variables. Supported provider types are `openid-connect`, `github`, `gitlab`, `google`, and `discord`. The login page renders buttons from `Server.authProviders`, and provider flows use `/auth/providers/{providerID}` plus `/auth/providers/{providerID}/callback`.
