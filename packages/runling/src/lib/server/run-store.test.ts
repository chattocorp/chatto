...
test('live activity logs include run and task prefixes and cancellation matches history', async () => {
  ...
  try {
    ...
    expect(info.mock.calls.some(([line]) => String(line).includes(`[${reference} / task-2]`))).toBe(
      true
    );
    ...
    expect(
      JSON.stringify([...info.mock.calls, ...warn.mock.calls, ...errors.mock.calls])
    ).not.toContain('sensitive dynamic label');
  } finally {
    info.mockRestore();
    warn.mockRestore();
    errors.mockRestore();
...
test('run references survive restart and avoid collisions across concurrent starts', async () => {
  ...
  const names = vi
    .fn()
    .mockReturnValueOnce('brave-otters-4821')
    .mockReturnValueOnce('brave-otters-4821')
    .mockReturnValue('calm-badgers-7392');
  ...
  expect(new Set(history.list().map((run) => run.reference))).toEqual(
    new Set(['brave-otters-4821', 'calm-badgers-7392'])
  );
  const restored = new RunStore(
    original.directory,
    vi.fn().mockReturnValueOnce('brave-otters-4821').mockReturnValue('bright-foxes-2846')
  );
  await restored.init();
  for (const run of runs)
    expect((await restored.get(run.id))?.reference).toBe((await history.get(run.id))?.reference);
...
test('shutdown keeps Node alive until cooperative cleanup and journal flush finish', async () => {
  ...
  const records = (await readFile(resolve(original.directory, file!), 'utf8'))
    .trim()
    .split('\n')
...
test('streams ordered nested events and restores the completed run', async () => {
  ...
  expect(
    records.some((record) => record.type === 'event' && record.event.type === 'task.state')
  ).toBe(true);
  const restored = new RunStore(original.directory);
  await restored.init();
...
test('records failures and keeps concurrent token totals separate', async () => {
  ...
  const release = Promise.withResolvers<void>();
  const slow = task({ name: 'Slow', input: Type.String(), output: Type.String() }, async (ctx) => {
  ...
  const fast = task({ name: 'Fast', input: Type.String(), output: Type.String() }, (ctx) => {
...
test('recovers a truncated journal as interrupted and saves the recovery', async () => {
  ...
  expect(buildTimeline((await recovered.get(run.id))!.events, 'interrupted')[0]?.status).toBe(
    'completed'
  );
...
test('loads completed details on demand without retaining event arrays', async () => {
  ...
  const workflow = task(
    ...
    (ctx, input) => {
      log.info('A retained journal event');
      return input;
    }
  );
...
test('cancels only the selected run, keeps cleanup and usage, and restores cancellation', async () => {
  ...
  await ready.promise;
  const other = await history.start(
    'other',
    task({ name: 'Other', input: Type.String(), output: Type.String() }, (ctx) => {
