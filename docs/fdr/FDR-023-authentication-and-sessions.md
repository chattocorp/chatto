...
- **CSRF protection** — cookie-session unsafe requests to `/api/graphql` and `/auth/logout` must include an `X-CSRF-Token` header matching the readable `chatto_csrf` cookie and the token stored in the signed session. GraphQL requests may also use the non-simple `X-REQUEST-TYPE: GraphQL` header as a same-origin request proof. Bearer-only requests are exempt because bearer credentials are not ambient browser cookies.
