...
Owners and permissioned admins use the admin UI for daily operations. Depending on their permissions, they can inspect members, roles, permissions, server configuration, room layout, room suspensions, event-log diagnostics, and system health.
...
The Bleve directory contains plaintext-derived message content. Keep it on a protected local volume and exclude it from normal backups. Chatto rebuilds a missing index or a recognized old index format from retained `EVT` history. Unknown or unreadable indexes require [operator recovery](/guides/operations/search/#rebuild-the-search-index). See [Enabling Search](/guides/operations/search/) for language configuration, standalone deployment, index lifecycle, and troubleshooting.
...
<Aside type="danger">
  Treat operator socket access like root access to Chatto. Do not mount the
  socket into untrusted containers or expose it over TCP.
</Aside>
...
<Aside type="caution" title="Upgrading from 0.4 to 0.5">
  ...
  frontend with the server replicas because realtime protocol 4 is also a
  coordinated compatibility boundary. Users whose browsers still carry the
  retired pre-typed cookie-session format are signed out once and must sign in
...
<CardGrid>
  <LinkCard
    ...
    description="Backup, restore, key export, and disaster recovery procedures."
  />
  <LinkCard
    title="Operator CLI"
    href="/guides/operations/operator-cli/"
    description="Local recovery and bootstrap commands."
  />
  <LinkCard
    title="Enabling Search"
    href="/guides/operations/search/"
    description="Configure local message indexing and language analyzers."
  />
  <LinkCard
    title="High Availability"
    href="/guides/infrastructure/high-availability/"
    description="Run Chatto with replicated infrastructure."
  />
  <LinkCard
    title="Horizontal Scaling"
    href="/guides/infrastructure/horizontal-scaling/"
    description="Scale server processes and externalize runtime dependencies."
  />
  <LinkCard
    title="Environment Variables"
    href="/reference/environment-variables/#metrics"
    description="Metrics, exporter, and operator API configuration reference."
  />
</CardGrid>
