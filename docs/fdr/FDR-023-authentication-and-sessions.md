...
- **External provider login** — operators configure repeated `[[auth.providers]]` entries. Supported provider types are `openid-connect`, `github`, `gitlab`, `google`, `discord`, and `microsoftonline`. The login page renders buttons from `Server.authProviders`, and provider flows use `/auth/providers/{providerID}` plus `/auth/providers/{providerID}/callback`.
