...
| `events.ProjectionHandle` / `events.Projector` / `events.ComponentProjection` | [`projector.go`](../../pkg/events/projector.go), [`component_projection.go`](../../pkg/events/component_projection.go), [`projector.go`](../../cli/internal/evtstream/projector.go) | Envelope-neutral typed model ownership, prepared reducers, one apply barrier for related components, ordered replay, readiness, failure, component snapshot cohorts, scalar snapshots, and checkpoints; `evtstream` supplies the Chatto event decoder and typed constructors |
...
| `projectionSnapshotWorker`       | [`projection_snapshot_worker.go`](../../cli/internal/core/projection_snapshot_worker.go) | Optional per-pass elected post-boot and daily publication of encrypted scalar generations and complete `ServerContentView` component cohorts; a separate cluster-wide cooldown limits bounded S3 age expiry when Chatto owns lifecycle cleanup |
