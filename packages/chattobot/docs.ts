...
function docsUrl(value: string, base = DOCS_HOME): URL {
  ...
  if (url.origin !== new URL(DOCS_HOME).origin || url.username || url.password || url.search) {
    throw new Error(
      'Only HTTPS Chatto documentation URLs without credentials or query strings are allowed'
    );
  }
  url.hash = '';
  return url;
  ...
  for (let redirects = 0; ; redirects++) {
    ...
    try {
      response = await request(url, {
        ...
        signal: bounded,
        credentials: 'omit',
        headers: { accept: 'text/html', 'user-agent': 'ChattoBot-docs/1.0' }
      });
    } catch {
