...
| `account_kind` | [`UserAccountKind`](#chatto-api-v1-UserAccountKind) | Whether this identity represents a person or a bot. |
...
| `api_key_created_at` | `google.protobuf.Timestamp` | Time the initial API key was created. |
| `api_key_rotated_at` | `optional google.protobuf.Timestamp` | Most recent API-key rotation time, when it has been rotated. |

<a id="chatto-api-v1-BotPermissionCell"></a>
...
| Field | Type | Description |
| --- | --- | --- |
| `permission` | `string` | Stable permission key. |
| `scope_id` | `string` | Matrix scope ID. |
| `configured` | [`BotPermissionDecision`](#chatto-api-v1-BotPermissionDecision) | Explicit decision stored at this exact scope. |
| `delegated` | [`BotPermissionDecision`](#chatto-api-v1-BotPermissionDecision) | Bot decision after its own scope inheritance, before the owner ceiling. |
| `owner_granted` | `bool` | Whether the owner currently has this permission at the same scope. |
| `effective_granted` | `bool` | Whether the bot can currently exercise the permission. |
...
| `applicable_permissions` | `repeated string` | Permission rows. |
| `scopes` | repeated [`BotPermissionMatrixScope`](#chatto-api-v1-BotPermissionMatrixScope) | Scope columns. |
| `cells` | repeated [`BotPermissionCell`](#chatto-api-v1-BotPermissionCell) | Sparse permission cells. |
...
| `parent_group_id` | `string` | Parent room-group ID for room scopes. |
...
| `followed_rooms` | optional [`NotificationDeliveryMode`](#chatto-api-v1-NotificationDeliveryMode) | No field description provided. |
...
| `kind.followed_room_activity` | [`FollowedRoomActivity`](#chatto-api-v1-FollowedRoomActivity) | No field description provided. |
...
<a id="chatto-api-v1-UserAccountKind"></a>
...
| Name | Number | Description |
| --- | --- | --- |
| `USER_ACCOUNT_KIND_UNSPECIFIED` | `0` | Account kind was not reported; treat as HUMAN for compatibility. |
| `USER_ACCOUNT_KIND_HUMAN` | `1` | Account represents a person. |
| `USER_ACCOUNT_KIND_BOT` | `2` | Account represents an automated bot. |
...
| `BOT_PERMISSION_DECISION_NONE` | `3` | No explicit decision; clears the stored state on writes. |
...
| `NOTIFICATION_DELIVERY_MODE_OFF` | `1` | Matching activity does not create a notification occurrence. |
| `NOTIFICATION_DELIVERY_MODE_SILENT` | `2` | Matching activity appears in the notification list without interruptive delivery. |
| `NOTIFICATION_DELIVERY_MODE_ALERT` | `3` | Matching activity appears in the notification list and may trigger sound or push. |
