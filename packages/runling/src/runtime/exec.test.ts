...
describe('createExec', () => {
  test('passes arguments literally without requiring shell syntax', async () => {
    ...
    expect(
      ...
      })`${process.execPath} -e ${printArgs} ${values}`.json()
    ).toEqual(values);
  });
  ...
  test('joins adjacent interpolations into one argument', async () => {
    expect(
      await createExec({
        cwd: import.meta.dirname
      })`${process.execPath} -e ${printArgs} refs/${'feature a'}:${'target b'}`.json()
  ...
  test('preserves output buffers, newlines, and nonzero exit codes', async () => {
    ...
    const command = "console.log('stdout'); console.error('stderr'); process.exit(7)";
    await observeRunlingEvents(
      (event) => events.push(event),
      async () => {
        const result = await createExec({
          cwd: import.meta.dirname
        })`${process.execPath} -e ${command}`.nothrow();
  ...
  test('reports missing executables as failed even in nothrow mode', async () => {
    ...
    await observeRunlingEvents(
      ...
      async () => {
        const result = await createExec({
          cwd: import.meta.dirname
        })`runling-command-that-does-not-exist`.nothrow();
  ...
  test('keeps explicit directories isolated across concurrent commands', async () => {
    const directories = await Promise.all(
      [1, 2].map(() => mkdtemp(join(tmpdir(), 'runling-exec-')))
    );
    try {
      ...
      await expect(exec`${process.execPath} -e ${script}`).rejects.toThrow(
