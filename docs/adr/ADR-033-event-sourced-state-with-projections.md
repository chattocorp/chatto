...
- **Snapshots were deferred but accommodated.** The projection interface includes `Snapshot()` and `Restore()` methods from day one. Initial releases replay from the beginning of the stream; [ADR-049](ADR-049-ephemeral-encrypted-projection-snapshots.md) defines the later optional snapshot mechanism without changing this projection contract or the authority of EVT.
...
- **Startup cost grows with stream length.** Without a usable compatible snapshot, a long-running instance pays projection-rebuild time on restart. ADR-049 permits snapshots only as an optional accelerator, so complete replay remains the correctness baseline and fallback.
