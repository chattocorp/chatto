...
describe('executeWorkflow', () => {
  ...
  test('logs accumulated token usage from agent interactions', async () => {
    ...
    expect(
      logs.some((line) =>
        line.includes('Total token usage: in 150, out 45, cache read 1,050, cache write 25')
      )
    ).toBe(true);
    expect(logs.some((line) => line.includes('Finished in '))).toBe(true);
  });
  ...
  test('resets token usage totals between executions', async () => {
    createWorkflowContext().recordUsage({
      input: 999,
      output: 999,
      cacheRead: 999,
      cacheWrite: 999
    ...
    expect(logs.some((line) => line.includes('Total token usage: in 10, out 5'))).toBe(true);
...
describe('runWorkflow', () => {
  ...
  test('runs headlessly with host-provided input and event handling', async () => {
    ...
    const running = runWorkflow(joke, {
      input: 'Make me laugh',
      onInput: (request) => {
...
describe('normalizeWorkflowResult', () => {
  test('rejects non-JSON values even when a summary is present', () => {
    ...
    for (const output of [
      ...
      expect(() => normalizeWorkflowResult(output)).toThrow('Workflow output must be valid JSON');
    }
  });
...
test('isolates workflow contexts and usage events across concurrent executions', async () => {
  ...
  expect(
    events.map((list) => list.filter((e) => e.type === 'usage.updated').map((e) => e.usage.input))
  ).toEqual([
    [10, 20],
    [100, 200]
  ]);
...
test('emits context usage updates from callbacks created outside the run', async () => {
  ...
  try {
    ...
    expect(
      events.filter((event) => event.type === 'usage.updated').map((event) => event.usage)
...
test('host handlers can be overridden by child contexts without changing the parent', async () => {
  ...
  const workflow = task(async (ctx) => {
    ...
    const answers = await Promise.all([child(override), askInput(ctx, 'parent question')]);
...
test('passes text handlers through nested tasks and isolates concurrent runs', async () => {
  const first: string[] = [];
  const second: string[] = [];
  const leaf = task(async (ctx, text: string) => {
    await ctx.onText?.(text);
  });
...
test('awaits text delivery and reports handler failures through normal task errors', async () => {
  ...
  const failure = await runWorkflow(root, {
    input: undefined,
    onText: async () => {
