...
function realtimeUrlFromGraphQLEndpoint(url: string): string {
  if (url.startsWith('/')) {
    const origin = typeof window !== 'undefined' ? window.location.origin : 'http://localhost';
    return httpToWsUrl(new URL('/api/realtime', origin).toString());
  }
  const parsed = new URL(url);
  return httpToWsUrl(new URL('/api/realtime', parsed.origin).toString());
}
...
export class GraphQLClient {
  ...
  constructor(config: GraphQLClientConfig) {
    ...
    this.#realtimeUrl = realtimeUrlFromGraphQLEndpoint(url);
    this.#token = token;
