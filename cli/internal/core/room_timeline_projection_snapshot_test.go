	...
	for _, legacy := range []bool{false, true} {
		t.Run(fmt.Sprintf("legacy=%v", legacy), func(t *testing.T) {
			...
			for _, projection := range []*RoomTimelineProjection{p, restored} {
				reply, found := projection.Get("REPLY")
				require.True(t, found)
				require.Equal(t, "ROOT", reply.ThreadRootEventID)
				require.Equal(t, "ROOT", reply.InThreadEventID)
				echo, found := projection.Get("ECHO")
				require.True(t, found)
				require.Equal(t, "ROOT", echo.ThreadRootEventID)
				require.Equal(t, "REPLY", echo.EchoOfEventID)
