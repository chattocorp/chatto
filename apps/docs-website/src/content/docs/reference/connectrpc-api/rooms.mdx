...
| `user_id` | `string` | Required. Human user or bot installation to add as an explicit room member. |
...
<a id="chatto-api-v1-RoomService-RemoveBotThreadAccess"></a>
...
```http
POST /api/connect/chatto.api.v1.RoomService/RemoveBotThreadAccess
```

<a id="chatto-api-v1-RemoveBotThreadAccessRequest"></a>

#### Input: RemoveBotThreadAccessRequest

Request revocation of one bot's mention-derived access to a thread.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Channel containing the shared thread. |
| `thread_root_event_id` | `string` | Required. Root message identifying the shared thread. |
| `bot_id` | `string` | Required. Bot whose contextual access should be removed. |


<a id="chatto-api-v1-RemoveBotThreadAccessResponse"></a>

#### Result: RemoveBotThreadAccessResponse

Result of revoking a bot's contextual thread access.

| Field | Type | Description |
| --- | --- | --- |
| `removed` | `bool` | True when an active grant was removed by this call. |
