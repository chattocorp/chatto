...
- Reconnect catch-up in the web client refreshes the currently viewed room window from `RoomTimelineProjection` after wake/reconnect. When the user is at the bottom it fetches the latest page; when scrolled up it uses `eventsAround` for the visible anchor event and preserves scroll by event ID. Server-scoped stores also refetch projected state after event-bus reconnect/resubscribe gaps so notifications, unread/sidebar state, room layout, server profile/settings, and active-call indicators do not depend on replayed subscription events. `Subscription.myEvents` is live-only and does not expose a replay cursor.
