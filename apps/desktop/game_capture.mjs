...
export const gameCaptureCancelListSourcesChannel = 'chatto:game-capture:cancel-list-sources';
...
export function supportsMacOSGameCapture(systemVersion) {
  ...
  return Number.isSafeInteger(majorVersion) && majorVersion >= minimumMacOSGameCaptureMajorVersion;
}
...
export function parseMacOSGameCaptureSourceId(sourceId) {
  ...
  if (!match) throw new Error('The selected native screen-share source is invalid.');
...
export class MacOSGameCaptureSourceOffers {
  ...
  replace(response) {
    ...
    return {
      ...
      sources: response.sources.map((source) => {
        ...
        this.#offers.set(id, {
          source: nativeSource,
          expectedBundleIdentifier: source.kind === 'window' ? source.bundleIdentifier : undefined,
  ...
  consume(id) {
    ...
    if (!offer || offer.expiresAt <= this.#now()) {
      throw new Error('The selected native screen-share source has expired. Choose it again.');
    }
    return {
      ...offer.source,
...
export function parseGameCapturePublisherStatus(line) {
  ...
  if (value?.protocolVersion !== 1 || value?.kind !== 'started') {
    throw new Error('The capture helper returned an unsupported publisher status.');
  }
  if (
