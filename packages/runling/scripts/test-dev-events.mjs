...
try {
  ...
  assert.deepEqual(
    run.events.filter((event) => event.type !== 'log').map((event) => event.type),
  ...
  const send = (input) =>
    endpoint.POST({
      params: { name: 'probe' },
      request: new Request('http://localhost/api/runs/start/probe', {
        method: 'POST',
  ...
  for (
    let attempts = 0;
    attempts < 100 && (await hostStore.get(acceptedId)).status === 'running';
    attempts++
  ) {
