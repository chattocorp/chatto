...
| `asset_upload.{uploadId}` | JSON room-scoped attachment upload session with author, an operator-session marker when created through the private socket, declared size/SHA-256, committed offset, chunk keys, status, and expiry. Public uploads cannot use marked sessions. Open sessions use a 15-minute TTL; completed sessions expire with the 24-hour pending-attachment window. |
...
| `grant.{hmac}` | OAuth authorization code JSON bound to user ID, optional client ID, optional exact OAuth resource and scopes, exact redirect URI, PKCE challenge, and the user auth generation it was issued against. New codes set `auth_generation_recorded`. Uses per-key 5-minute TTL and is claimed with a revision-matched delete before exchange validation and token issuance. |
...
| `external_identity_link.{hmac}` | Previously issued pending link confirmation JSON containing provider identity and optional verified-email/profile hints, bound to the authenticated user. New provider callbacks complete explicit links directly; the confirmation API continues to accept existing records until expiry. The KV key is HMAC-derived from the raw capability token, which is never stored; the record uses a 15-minute TTL. |
