/** Observe child runs, retain bounded task state, and route selected owner notices. */
...
export type AgentTaskData =
  null | boolean | number | string | AgentTaskData[] | { [key: string]: AgentTaskData };
...
export function observeAgentTasks(
  ctx: WorkflowContext<any, any>,
  {
    ...
    maxToolFailures,
    notifyActivity = true,
    toolFailureNoticeThreshold = 2
  }: {
    progressIntervalMs?: number;
    maxTasks?: number;
    /** Notify after repeated failures; permission errors notify immediately. Defaults to 2. */ toolFailureNoticeThreshold?: number;
    /** Keep routine activity in snapshots without waking the owner when false. */ notifyActivity?: boolean;
    /** Stop a child after this many failures of an operation without success. Disabled by default. */ maxToolFailures?: number;
  } = {}
) {
  if (
  ...
  if (
    maxToolFailures !== undefined &&
    (!Number.isSafeInteger(maxToolFailures) || maxToolFailures < 1)
  )
    throw new Error('Invalid tool failure limit');
  if (!Number.isSafeInteger(toolFailureNoticeThreshold) || toolFailureNoticeThreshold < 1)
    throw new Error('Invalid tool failure notification threshold');
  type Entry = {
    state: AgentTaskState;
    handle: Run<string, AgentTaskUpdate, unknown>;
  ...
  const snapshot = (entry: Entry): AgentTaskState => ({
    ...entry.state,
    output: entry.state.output.map((item) => ({ ...item })),
    ...(entry.state.state
      ? {
          state: structuredClone(entry.state.state),
          stateAgeMs: Math.max(0, Date.now() - entry.state.stateAt!)
        }
      : {}),
    ...(entry.state.provider ? { provider: { ...entry.state.provider } } : {}),
    ...(entry.state.progressAt !== undefined
      ? { progressAgeMs: Math.max(0, Date.now() - entry.state.progressAt) }
      : {}),
    ...(entry.state.activity
      ? {
          activity: { ...entry.state.activity },
          activityAgeMs: Math.max(0, Date.now() - entry.state.activityAt!)
        }
      : {}),
  ...
  const notify = (entry: Entry, type: string, text?: string) => {
    ...
    if (
      notices.get(entry.state.id)?.type === 'task.reply' &&
  ...
  const observer = {
    ...
    start<Input, Update extends AgentTaskUpdate = AgentTaskUpdate>(
      name: string,
      run: (ctx: WorkflowContext<string, Update>, input: Input) => Promise<string>,
      input: Input
    ): AgentTaskState {
      ...
      return observer.observe(
        name,
        ctx.spawn((ctx: WorkflowContext<string, Update>) => {
                ...
                emit: async (update) => {
                  if (typeof update !== 'string' && update.type === 'state') {
                    ...
                    if (!encoded || encoded.length > 16_000)
                      throw new Error('Task state exceeds the JSON size limit');
                    // Copy before queuing so subsequent producer mutations cannot change the published state.
                    await ctx.emit({
                      ...(update as Extract<AgentTaskUpdate, { type: 'state' }>),
          ...
          for await (const text of handle.output) {
            ...
            if (typeof text !== 'string' && text.type === 'state') {
              ...
              if (text.activity)
                emitRunlingEvent({
                  type: 'task.activity',
                  channelId: handle.id,
                  message: text.activity.slice(0, 200),
            ...
            if (typeof text !== 'string' && text.type !== 'finding') {
              if (text.type === 'tool') {
                ...
                if (
                  ...
                  state.lastToolFailure?.operation === text.operation
                ) {
                ...
                if (text.phase === 'failed') {
                  ...
                  if (maxToolFailures !== undefined && text.failures >= maxToolFailures) {
                    ...
                    text.failures >= toolFailureNoticeThreshold ||
                    text.error === 'permission_denied'
                  )
                    notify(entry, 'task.tool_failed');
                } else if (!entry.timer)
                  entry.timer = setTimeout(() => {
                    entry.timer = undefined;
                    if (state.status === 'running' && (entry.progress || notifyActivity))
                      notify(
                        entry,
                        entry.progress ? 'task.progress' : 'task.activity',
                        entry.progress
                      );
                  }, progressIntervalMs);
                continue;
              ...
              state.provider =
                text.type === 'retrying'
                  ? {
                      ...
                      maxAttempts: text.maxAttempts,
                      delayMs: text.delayMs
                    }
                  : text.type === 'blocked'
                    ? { type: 'blocked', reason: 'provider_error' }
                    : { type: 'working' };
              if (text.type === 'working' && notices.get(state.id)?.type === 'task.retrying')
                notices.delete(state.id);
              clearTimeout(entry.timer);
              entry.timer = undefined;
              ...
              if (state.provider.type !== 'working' && previousProvider !== state.provider.type)
                notify(entry, `task.${state.provider.type}`);
              continue;
            }
            entry.finding = typeof text !== 'string';
            entry.progress = (typeof text === 'string' ? text : text.text).slice(0, 4_000);
            remember(
              entry.finding ? 'finding' : 'output',
            ...
            if (!entry.timer)
              entry.timer = setTimeout(() => {
                entry.timer = undefined;
                if (state.status === 'running' && (entry.progress || notifyActivity))
                  notify(entry, entry.progress ? 'task.progress' : 'task.activity', entry.progress);
              }, progressIntervalMs);
          }
        } catch {
      ...
      entry.settled = (async () => {
        try {
          ...
          try {
            ...
            text = '[Result cannot be represented as JSON]';
          }
          state.result =
            text.length > 64_000 ? `${text.slice(0, 64_000)}\n[Task result truncated]` : text;
    ...
    get active() {
      return (
        notices.size > 0 || [...tasks.values()].some((entry) => entry.state.status === 'running')
      );
    },
    dispose
  };
...
export function agentTasksExtension(tasks: AgentTasks) {
  ...
  return defineAgentExtension((pi) => {
    pi.registerTool({
      name: 'task_send',
      label: 'Steer task',
      description: "Send a relevant user clarification to a running task's inbox.",
      parameters: Type.Object({
        id: Type.String(),
        message: Type.String({ minLength: 1, maxLength: 16_000 })
      }),
      async execute(_id, input) {
        await tasks.send(input.id, input.message);
    ...
    pi.registerTool({
      name: 'task_cancel',
      label: 'Cancel task',
      description: 'Cancel one owned task without ending the conversation.',
      parameters: Type.Object({ id: Type.String() }),
      async execute(_id, input) {
        tasks.cancel(input.id);
        return result(tasks.get(input.id));
      }
    });
  });
}
