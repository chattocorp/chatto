...
| ---- | ----------------- |
| **Required** | Every new root message automatically gets a thread. Reply actions open that thread, and the server rejects flat replies to roots. |
| **Encouraged** | The primary reply action opens the thread, while **Reply in room** remains available with a small reminder. Creating a thread with a new root stays opt-in. |
