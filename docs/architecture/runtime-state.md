...
| ----------------------------- | ------- | -------- | ----------------------------------------------- |
| `RUNTIME_STATE`               | File    | Yes      | Persisted latest-value runtime/user state, including temporary notification work and boundaries, push subscriptions, auth/workflow tokens, wrapped app DEK records, and encrypted snapshot pointers |
...
| `notification_work.{triggerEventId}` | Temporary protobuf `NotificationMaterializationWork` containing the complete exact recipient-and-signal decision prepared before one existing message/reaction source fact commits. Every command OCC retry replaces this one value, so stale recipients cannot survive a conflict. The shared durable EVT consumer loads it by trigger ID, appends deterministic signal facts to `NOTIFICATIONS`, and deletes it after success. Failed source appends may leave an untriggered value; the same absolute 90-day TTL bounds it. |
