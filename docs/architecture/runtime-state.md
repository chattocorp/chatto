...
Direct JPEG, PNG, GIF, and static WebP responses use the same pending attachment
storage and durable asset facts. The SSRF-safe link-preview fetcher reads at
most 5 MB, while `AssetUploadModel` performs the authoritative decoded-image
validation. Direct-image results bypass the shared link-preview cache. A
hashed `RUNTIME_STATE` reservation deduplicates repeated composer fetches and
bounds storage before any remote bytes are downloaded.
