...
func (s *Storage) Consent(ctx context.Context, id string) (ConsentRequest, error) {
	...
	return ConsentRequest{ID: state.ID, ClientName: state.ClientName, ClientHost: state.ClientHost, RedirectOrigin: redirect.Scheme + "://" + redirect.Host, Scopes: append([]string(nil), state.Scopes...)}, nil
}
