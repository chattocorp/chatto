...
- **External identity matching and confirmation** — provider identities are linked through durable `EVT` user facts. OIDC providers use the verified issuer URL plus subject; OAuth-only providers use the configured provider ID plus Goth `UserID`. Email claims are optional profile data and are not used as the primary account match key. If a provider callback returns an identity that is not linked to a Chatto account, Chatto creates a short-lived pending external-identity flow only when the provider is eligible for the requested action: account creation requires `auto_provision = true`, while account linking requires an already authenticated user who created a provider link-start URL. The user must explicitly confirm create, link, or cancel through ConnectRPC; there is no silent email-based merge. Provider avatar URLs are transient profile hints: Chatto imports them as managed avatar assets during account creation or first matched login when no avatar is already set, and does not persist the remote URL.
- **Password set/change** — users can set or change their own password from account settings through authenticated ConnectRPC. Adding the first password to a passwordless account does not require current-password proof and preserves existing sessions because it adds a new sign-in method. Changing an existing password requires the current password, applies the same password length policy as registration/reset, writes the password hash as a durable user event, and revokes existing runtime credentials. Server admins can set a user's password through `AdminUserManagementService`; admin-set passwords do not require the old password and revoke the target user's runtime credentials.
