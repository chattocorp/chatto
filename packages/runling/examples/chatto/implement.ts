import { agent, connectAgent, type AgentOptions, type RunlingAgent } from 'runling/agents';
...
export interface ImplementationOptions {
  ...
  validate?: (ctx: WorkflowContext<unknown>, directory: string) => Promise<string | null>;
  hasChanges?: (ctx: WorkflowContext<unknown>, directory: string) => Promise<boolean>;
}
const git = (ctx: WorkflowContext<unknown>, directory: string, ...args: string[]) =>
  execa('git', args, {
    cwd: directory,
      ...
      try {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          ...
          const report = await step(
            attempt === 1 ? 'Implement approved plan' : `Repair implementation (${attempt}/3)`,
