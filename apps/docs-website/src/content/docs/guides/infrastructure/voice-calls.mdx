...
Select **Start microphone test** to view the input level. You can then record
up to ten seconds and play the recording through your selected speaker.
The recording stays in browser memory and is discarded when you leave the
page. This local test does not connect to LiveKit or test the network.
Leave the selected server's call before running the test.

Select **Show cameras** if the browser has not yet disclosed camera choices.
This requests camera access, then stops capture immediately.
