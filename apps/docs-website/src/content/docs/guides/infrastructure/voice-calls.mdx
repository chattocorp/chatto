...
The glow on your participant card shows microphone activity. If it does not
appear when you speak, use the call toolbar's gear to open Voice & video
preferences and test your input device.
...
Select **Start microphone test** to see the input level while the browser
records a local sample. The speaker stays silent during recording. Select
**Stop and play**, or wait 10 seconds, to end capture and hear the sample through
the selected speaker. Select **Play again** to repeat it. The sample stays in
browser memory until you start another test, change the microphone, or leave
the page. The test does not connect to LiveKit. Leave the selected server's call
before running it.
