...
<a id="chatto-admin-v1-BotRoomMembership"></a>
...
| Field | Type | Description |
| --- | --- | --- |
| `joined` | `bool` | Whether the bot is currently an effective member of the room. |
| `automatic` | `bool` | Whether membership follows the universal room's join eligibility. |
| `can_join` | `bool` | Whether the caller can add the bot now. |
| `can_leave` | `bool` | Whether the caller can remove the bot now. |
...
| `bot_membership` | [`BotRoomMembership`](#chatto-admin-v1-BotRoomMembership) | Present only for channel-room scopes in a bot's user permission matrix. |
