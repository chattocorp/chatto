...
const implementationPreview = task(async function implementationMode(
  _ctx,
  plan: string,
  revision: string
) {
...
export function createChattoPlanDemo({
  ...
  return createChattoWebhook({
    ...
    async run(ctx, delivery, destination, inbox) {
      // Check at invocation so an unused demo cannot prevent the console starting.
      if (!directory.trim()) {
        throw new Error(
          'Set CHATTO_WORKING_COPY in packages/runling/.env before running this demo'
        );
      }
      ...
      try {
        ...
        if (busyDirectories.has(root)) {
          await say(
            'I am already planning another change in this checkout. Please send a new DM after that conversation ends.'
          );
        ...
        acquired = true;
        await say(
          'I’ll update my Chatto checkout and investigate your request. Send additional thoughts at any time. Use /cancel to stop; /implement accepts a completed plan (demo only).'
        );
        const revision = await withChattoTyping(ctx, destination, typing, () =>
          refresh(ctx, root!)
        );
        const planner = await withChattoTyping(ctx, destination, typing, () =>
          createPlanner(root!)
        );
        try {
          ...
          const feedback = async () => {
            ...
            while (true) {
              ...
              await say(
                'Please wait for a completed plan, then send /implement to accept it. That command was not saved as approval.'
              );
          ...
          while (true) {
            ...
            const report = await runChattoAgent(ctx, planner, message, {
              ...
              post,
              typing,
              reservedCommands: ['/implement']
            });
            if (report.outcome === 'failed') throw new Error(`Planning failed: ${report.summary}`);
            const during = await feedback();
            if (during.length) {
              await say(
                'I received your additional messages and will include them before posting the next question or plan.'
              );
            ...
            while (true) {
              ...
              await say(
                'There is no completed plan to accept yet. Please answer the planning question first.'
              );
            }
            ...
            if (answer.trim() === '/implement' && extra.length) {
              await say(
                'I’ll review your additional feedback first. Please approve the updated plan when it is ready.'
              );
            }
            if (answer.trim() === '/implement' && !extra.length) {
              const result = await implementationPreview(ctx, plan, revision);
              await say(
                'Implementation mode reached. This is a demo stub: the plan is accepted, but no implementation files were changed.'
              );
              return result;
            }
        ...
        if (!(error instanceof TimeoutError) && !ctx.signal.aborted) {
          await say(
            'Planning stopped because an operation failed. Check this run in Runling for details, then send a new DM to retry.'
          ).catch(() => {});
        }
        throw error;
      } finally {
...
export default createChattoPlanDemo({
  directory: process.env.CHATTO_WORKING_COPY ?? '',
