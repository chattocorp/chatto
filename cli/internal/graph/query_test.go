...
func TestQueryResolver_RoomEvents(t *testing.T) {
	...
	_, err := env.core.PostMessage(env.ctx, "channel", env.testRoom.Id, env.testUser.Id, "Test message", nil, "", "", nil, false)
	if err != nil {
...
func TestQueryResolver_RoomEventsAround(t *testing.T) {
	...
	var eventIDs []string
	for i := 0; i < 20; i++ {
		event, err := env.core.PostMessage(env.ctx, "channel", env.testRoom.Id, env.testUser.Id,
...
func TestQueryResolver_RoomEventsForward(t *testing.T) {
	...
	// Post 10 messages
	for i := 0; i < 10; i++ {
		_, err := env.core.PostMessage(env.ctx, "channel", env.testRoom.Id, env.testUser.Id,
...
func TestQueryResolver_RoomEventByEventID(t *testing.T) {
	...
	event, err := env.core.PostMessage(env.ctx, "channel", env.testRoom.Id, env.testUser.Id, "Test message for single event", nil, "", "", nil, false)
	if err != nil {
		t.Fatalf("Failed to post message: %v", err)
	}
