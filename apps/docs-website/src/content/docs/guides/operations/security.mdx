...
short-lived verifier record expires. The server checks the exact human
credential before a realtime subscription starts and once per minute after
that. A bearer socket also cannot continue past the access token's fixed
expiry.

The `chatto_auth` browser authentication cookie has these options:
...
and explicit expiry live in the server-side `session.{hmac}` record.
