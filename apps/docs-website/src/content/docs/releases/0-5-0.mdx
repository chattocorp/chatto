...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="A more practical permission baseline">
    ...
    channel-room message access from membership.
  </ReleaseFeatureCard>
...
- Before or during the upgrade, grant `message.read` to every existing role or
  ...
  threads. A server-level `everyone` grant for both permissions preserves broad
  access and enables the narrow mode. Chatto never migrates, backfills, or
  reconciles existing RBAC state, while fresh servers receive both grants
