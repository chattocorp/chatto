...
- Notification-alert pushes set the Web Push provider TTL to the remaining portion of the occurrence's immutable two-minute, source-time delivery window. Queue retry or backup restore cannot extend how long private content remains eligible at the provider.
...
- **ADRs:** ADR-074 (deterministic notification occurrences), ADR-075 (persistent notification list)
