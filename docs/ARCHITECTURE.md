...
- **Cross-cutting architectural choices** (NATS as primary store, protobuf/wire and GraphQL API boundaries, per-user encryption, etc.) live in the [Architecture Decision Records](adr/INDEX.md).
...
Chatto is a real-time chat application with a protobuf WebSocket API for the bundled web client, a GraphQL compatibility gateway, and a NATS/JetStream backend. Durable domain state is event-sourced in the `EVT` stream and served from projections; `RUNTIME_STATE` holds persisted latest-value runtime state such as notifications, push subscriptions, and auth tokens.
...
Key files: [`cli/internal/core/core.go`](../cli/internal/core/core.go), [`cli/internal/events/`](../cli/internal/events/), [`cli/internal/http_server/wire.go`](../cli/internal/http_server/wire.go), [`cli/internal/http_server/wire_admin_permissions.go`](../cli/internal/http_server/wire_admin_permissions.go), [`cli/internal/http_server/attachment_upload.go`](../cli/internal/http_server/attachment_upload.go), [`frontend/src/lib/state/server/serverConnection.svelte.ts`](../frontend/src/lib/state/server/serverConnection.svelte.ts), [`frontend/src/lib/state/server/wireEventBus.svelte.ts`](../frontend/src/lib/state/server/wireEventBus.svelte.ts), [`frontend/src/lib/wire/client.ts`](../frontend/src/lib/wire/client.ts), [`frontend/src/lib/wire/activeServerClient.ts`](../frontend/src/lib/wire/activeServerClient.ts), [`frontend/src/lib/chatTypes.ts`](../frontend/src/lib/chatTypes.ts), [`frontend/src/lib/pb/`](../frontend/src/lib/pb/), [`proto/chatto/core/v1/`](../proto/chatto/core/v1/), [`proto/chatto/wire/v1/`](../proto/chatto/wire/v1/), [`proto/chatto/api/v1/`](../proto/chatto/api/v1/)
...
- **GraphQL**: Compatibility API for existing tools and non-bundled consumers. Subscriptions over WebSocket remain available for compatibility. Fields require authentication by default unless marked public in the schema; resolvers call Core methods directly and enforce operation-specific authorization before each call. The bundled web client no longer imports the generated GraphQL client/runtime or issues GraphQL product operations.
- **Protobuf Wire API**: `/api/wire` WebSocket endpoint using binary protobuf `chatto.wire.v1.ClientFrame` / `ServerFrame` messages. It multiplexes request/response methods from `chatto.api.v1.ChattoApiService` and async live events adapted from `StreamMyEvents` over one authenticated socket. The bundled web client's authenticated product surfaces and legacy event-bus handler surface are backed by this stream, not by GraphQL `myEvents`.
- **Protobuf HTTP Assets**: `POST /api/rooms/{roomID}/attachments` accepts authenticated multipart room attachment uploads, checks room membership plus `message.post` / `message.post-in-thread`, stores binaries through `MediaService`, and returns binary protobuf `UploadRoomAttachmentsResponse` with uploaded asset IDs and video-processing hints. `POST /api/rooms/{roomID}/attachments/urls/refresh` accepts binary protobuf `RefreshMessageAttachmentUrlsRequest`, checks room membership, and returns `RefreshMessageAttachmentUrlsResponse` with freshly signed `AttachmentView` URL fields. `POST`/`DELETE /api/users/{userID}/avatar` perform self-or-`role.assign` gated avatar updates and return `UserAvatarAssetResponse`; `POST`/`DELETE /api/server/logo` and `/api/server/banner` perform `admin.instance.manage` gated server branding updates and return `ServerBrandingAssetResponse`.
...
- **Web Client**: SvelteKit-based SPA that gets compiled and embedded into the Go binary. Public server discovery/login metadata comes from `GET /api/server`; authenticated current-user bootstrap uses a short-lived wire `GetCurrentUser` call. `ServerConnection` and the shared wire event bus open one wire socket per active server for protobuf events, authenticated server runtime settings, editable server settings, server sidebar/bootstrap metadata, chat chrome access/bootstrap metadata, profile settings, account deletion, display preferences, notification level settings, notification center list/dismiss state, presence updates, push subscription registration, sidebar room lists, room directory reads, admin room layout editing, admin moderation ban list/unban, RBAC matrix reads/writes, admin security blocked-username management, admin system diagnostics, admin event-log diagnostics, admin member management, admin role CRUD, quick-switcher member search, room/thread bootstrap, timeline reads, followed-thread reads, composer writes, room attachment uploads followed by wire `PostMessage`, message attachment URL refresh through protobuf HTTP, profile asset upload/delete through protobuf HTTP, message/attachment/link-preview deletes, reactions, typing, read markers, thread follow state, active voice-call state, voice-call join/leave intents, LiveKit token lookup, room UI bans, DM start, channel room create/join, and room leave actions. TypeScript protobuf classes are generated under `frontend/src/lib/pb/`, with the typed `WireClient` under `frontend/src/lib/wire/`; UI-only DTOs that used to depend on GraphQL fragment names live in `frontend/src/lib/chatTypes.ts`.
...
The GraphQL API remains a compatibility interface for Chatto, especially for existing tools, external consumers, and server-side resolvers that have not yet been retired. It provides queries, mutations, and a single unified subscription over HTTP and WebSocket connections. The bundled web client no longer imports the generated GraphQL runtime/client and no longer issues frontend GraphQL product operations; first-party product reads/writes use protobuf wire or dedicated non-GraphQL HTTP endpoints. Fields require authentication by default unless explicitly marked public in the schema. Authentication accepts opaque bearer tokens first and falls back to cookie sessions when no bearer token is present. User registration, login, password reset, email verification, and external provider login flows are REST endpoints (under `/auth/...`) rather than GraphQL mutations; successful password login and registration issue both a cookie session and a bearer token. Public server metadata includes `Server.authProviders`, a list of configured external login providers with IDs, types, labels, and login URLs.
...
Key files: [`cli/internal/http_server/wire.go`](../cli/internal/http_server/wire.go), [`cli/internal/http_server/wire_admin_permissions.go`](../cli/internal/http_server/wire_admin_permissions.go), [`cli/internal/http_server/wire_admin_roles.go`](../cli/internal/http_server/wire_admin_roles.go), [`frontend/src/lib/state/server/serverConnection.svelte.ts`](../frontend/src/lib/state/server/serverConnection.svelte.ts), [`frontend/src/lib/state/server/wireEventBus.svelte.ts`](../frontend/src/lib/state/server/wireEventBus.svelte.ts), [`frontend/src/lib/wire/client.ts`](../frontend/src/lib/wire/client.ts), [`frontend/src/lib/chatTypes.ts`](../frontend/src/lib/chatTypes.ts), [`proto/chatto/wire/v1/protocol.proto`](../proto/chatto/wire/v1/protocol.proto), [`proto/chatto/api/v1/chat.proto`](../proto/chatto/api/v1/chat.proto), [`proto/buf.gen.yaml`](../proto/buf.gen.yaml), [`proto/buf.gen.frontend.yaml`](../proto/buf.gen.frontend.yaml)

`GET /api/wire` upgrades to a binary protobuf WebSocket. It is experimental but now feeds the bundled frontend's authenticated product surfaces and shared event bus: one authenticated socket carries protobuf request/response traffic and async live events. `buf generate` emits Go protobufs under `cli/internal/pb/`; `buf generate --template buf.gen.frontend.yaml` emits TypeScript protobufs under `frontend/src/lib/pb/`.

The frontend starts a shared `WireClient` for each authenticated server connection through `ServerConnection` and `wireEventBusManager`. Auth bootstrap uses a short-lived `WireClient` for `GetCurrentUser`; public login/server branding metadata uses `GET /api/server`. The shared client handles authenticated server runtime settings, editable server settings, server profile/permission/notification bootstrap, chat chrome access metadata, profile settings reads/updates, account deletion, display preference reads/updates, notification level reads/updates, notification center list/dismiss state, presence updates, push subscription registration, room directory reads, sidebar room lists, admin room layout reads/edits, admin moderation ban list/unban, RBAC matrix reads/writes, admin security blocked-username reads/updates, admin system diagnostics and projection health, admin event-log diagnostics, admin member list/detail and member edits, admin role create/detail/update/delete, quick-switcher member search, room bootstrap, member pagination, timeline pages, around-message windows, thread windows, followed-thread reads, event refetches, protobuf live-event ingestion through the legacy `EventBus` adapter, composer sends, edits, message/attachment/link-preview deletes, reactions, typing indicators, read markers, room UI bans, active voice-call state, observer call participants, voice-call join/leave intents, LiveKit token lookup, DM creation, channel room creation, room join/leave, group join-all, and thread follow/unfollow. Room composer attachments upload through `POST /api/rooms/{roomID}/attachments`, which returns protobuf `UploadRoomAttachmentsResponse`; the composer then sends returned asset IDs through wire `PostMessage`. Stale message attachment URLs refresh through `POST /api/rooms/{roomID}/attachments/urls/refresh`, which returns protobuf `RefreshMessageAttachmentUrlsResponse`. Profile asset updates use protobuf HTTP endpoints: `POST`/`DELETE /api/users/{userID}/avatar`, `POST`/`DELETE /api/server/logo`, and `POST`/`DELETE /api/server/banner`. No frontend product source imports the generated GraphQL runtime/client.
...
| `ServerFrame.error` | Server → client | Returns structured wire errors such as `UNAUTHENTICATED`, `PERMISSION_DENIED`, `UNIMPLEMENTED`, `FULL_REFRESH_REQUIRED`, and typed details such as mention-confirmation requirements. |

The API method schema lives in `chatto.api.v1.ChattoApiService`. Current prototype methods are:

| Method                                                       | Request                           | Response                          | Notes                                                                                                           |
| ------------------------------------------------------------ | --------------------------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `/chatto.api.v1.ChattoApiService/GetViewer`                  | `GetViewerRequest`                | `GetViewerResponse`               | Returns the authenticated user plus sidebar/chrome bootstrap data: server profile including banner URL, viewer permissions, server notification preference, and room notification preferences. |
| `/chatto.api.v1.ChattoApiService/GetCurrentUser`             | `GetCurrentUserRequest`           | `GetCurrentUserResponse`          | Returns the current-user auth bootstrap shape: user fields, avatar URL, presence, verified-email flag, and display preferences. |
| `/chatto.api.v1.ChattoApiService/GetAuthenticatedServerSettings` | `GetAuthenticatedServerSettingsRequest` | `GetAuthenticatedServerSettingsResponse` | Returns authenticated runtime settings such as push, LiveKit, video processing, upload limits, edit window, and MOTD. |
| `/chatto.api.v1.ChattoApiService/GetAccountDeletionStatus`   | `GetAccountDeletionStatusRequest` | `GetAccountDeletionStatusResponse` | Returns whether the current user can delete their own account.                                                    |
| `/chatto.api.v1.ChattoApiService/RequestAccountDeletion`     | `RequestAccountDeletionRequest`   | `RequestAccountDeletionResponse`  | Issues a short-lived account deletion confirmation token for the current user after checking self-delete permission. |
| `/chatto.api.v1.ChattoApiService/DeleteMyAccount`            | `DeleteMyAccountRequest`          | `DeleteMyAccountResponse`         | Consumes the account deletion confirmation token and deletes the current user's account.                         |
| `/chatto.api.v1.ChattoApiService/GetServerSettings`          | `GetServerSettingsRequest`        | `GetServerSettingsResponse`       | Returns editable server profile/settings fields plus the caller's server-management capability.                 |
| `/chatto.api.v1.ChattoApiService/UpdateServerSettings`       | `UpdateServerSettingsRequest`     | `UpdateServerSettingsResponse`    | Updates editable server profile/settings fields through the event-sourced config manager.                       |
| `/chatto.api.v1.ChattoApiService/GetAdminSecurityConfig`     | `GetAdminSecurityConfigRequest`   | `GetAdminSecurityConfigResponse`  | Returns the effective blocked-username security settings after checking server-management permission.           |
| `/chatto.api.v1.ChattoApiService/UpdateBlockedUsernames`     | `UpdateBlockedUsernamesRequest`   | `UpdateBlockedUsernamesResponse`  | Updates the newline-separated blocked-username list through the config manager after checking server-management permission. |
| `/chatto.api.v1.ChattoApiService/GetAdminSystemInfo`         | `GetAdminSystemInfoRequest`       | `GetAdminSystemInfoResponse`      | Returns owner-gated broker/account diagnostics plus projection health rows for the admin system page.           |
| `/chatto.api.v1.ChattoApiService/ListAdminEventLog`          | `ListAdminEventLogRequest`        | `ListAdminEventLogResponse`       | Lists newest-first diagnostic EVT entries after checking audit-view permission.                                 |
| `/chatto.api.v1.ChattoApiService/GetAdminEventLogEntry`      | `GetAdminEventLogEntryRequest`    | `GetAdminEventLogEntryResponse`   | Reads one diagnostic EVT entry by stream sequence after checking audit-view permission.                         |
| `/chatto.api.v1.ChattoApiService/ListAdminMembers`           | `ListAdminMembersRequest`         | `ListAdminMembersResponse`        | Lists paginated server members with avatar URLs, explicit roles, role catalog, and total/has-more metadata.     |
| `/chatto.api.v1.ChattoApiService/GetAdminMember`             | `GetAdminMemberRequest`           | `GetAdminMemberResponse`          | Reads one admin member detail view with role catalog, permission catalog, capability hints, and visible cooldown state. |
| `/chatto.api.v1.ChattoApiService/AdminUpdateUser`            | `AdminUpdateUserRequest`          | `AdminUpdateUserResponse`         | Updates a target user's login and/or display name after the existing self-or-`role.assign` admin target gate.   |
| `/chatto.api.v1.ChattoApiService/AdminClearUsernameCooldown` | `AdminClearUsernameCooldownRequest` | `AdminClearUsernameCooldownResponse` | Clears a target user's self-rename cooldown after the existing self-or-`role.assign` admin target gate.      |
| `/chatto.api.v1.ChattoApiService/AssignMemberRole`           | `AssignMemberRoleRequest`         | `AssignMemberRoleResponse`        | Assigns a server role to a user after checking `role.assign`, returning the refreshed member view.              |
| `/chatto.api.v1.ChattoApiService/RevokeMemberRole`           | `RevokeMemberRoleRequest`         | `RevokeMemberRoleResponse`        | Revokes a server role after checking `role.assign` and preserving existing self-owner/admin lockout checks.     |
| `/chatto.api.v1.ChattoApiService/GetAdminRoleCapabilities`   | `GetAdminRoleCapabilitiesRequest` | `GetAdminRoleCapabilitiesResponse` | Returns role-management capability flags for admin role pages.                                                 |
| `/chatto.api.v1.ChattoApiService/GetAdminRole`               | `GetAdminRoleRequest`             | `GetAdminRoleResponse`            | Reads one role detail with assigned users when the caller can assign roles.                                    |
| `/chatto.api.v1.ChattoApiService/CreateAdminRole`            | `CreateAdminRoleRequest`          | `CreateAdminRoleResponse`         | Creates a custom server role after checking `role.manage`.                                                     |
| `/chatto.api.v1.ChattoApiService/UpdateAdminRole`            | `UpdateAdminRoleRequest`          | `UpdateAdminRoleResponse`         | Updates role metadata and pingability after checking `role.manage`.                                            |
| `/chatto.api.v1.ChattoApiService/DeleteAdminRole`            | `DeleteAdminRoleRequest`          | `DeleteAdminRoleResponse`         | Deletes a custom role after checking `role.manage`.                                                            |
| `/chatto.api.v1.ChattoApiService/GetRolePermissionTierMatrix` | `GetRolePermissionTierMatrixRequest` | `GetRolePermissionTierMatrixResponse` | Reads the server/group/room role-permission tier matrix after checking the relevant role/room permission-management gate. |
| `/chatto.api.v1.ChattoApiService/GetRolePermissionMatrix`    | `GetRolePermissionMatrixRequest`  | `GetRolePermissionMatrixResponse` | Reads a per-role permission matrix after checking role-management permission.                                  |
| `/chatto.api.v1.ChattoApiService/GetUserPermissionMatrix`    | `GetUserPermissionMatrixRequest`  | `GetUserPermissionMatrixResponse` | Reads a user's effective permission matrix after checking user-permission management permission.                |
| `/chatto.api.v1.ChattoApiService/SetRolePermissionState`     | `SetRolePermissionStateRequest`   | `SetPermissionStateResponse`      | Grants, denies, or clears a role permission at server, group, or room scope after the matching permission-management gate. |
| `/chatto.api.v1.ChattoApiService/SetUserPermissionState`     | `SetUserPermissionStateRequest`   | `SetPermissionStateResponse`      | Grants, denies, or clears a direct user permission at server, group, or room scope after the user-permission management gate. |
| `/chatto.api.v1.ChattoApiService/GetProfileSettings`         | `GetProfileSettingsRequest`       | `GetProfileSettingsResponse`      | Returns the current user's editable profile fields, avatar URL, and last self-rename timestamp.                |
| `/chatto.api.v1.ChattoApiService/UpdateProfile`              | `UpdateProfileRequest`            | `UpdateProfileResponse`           | Updates the current user's display name and/or login through the event-sourced user service.                  |
| `/chatto.api.v1.ChattoApiService/GetUserSettings`            | `GetUserSettingsRequest`          | `GetUserSettingsResponse`         | Returns the current user's display preferences.                                                                |
| `/chatto.api.v1.ChattoApiService/UpdateUserSettings`         | `UpdateUserSettingsRequest`       | `UpdateUserSettingsResponse`      | Updates the current user's timezone and time-format display preferences.                                       |
| `/chatto.api.v1.ChattoApiService/SetServerNotificationLevel` | `SetServerNotificationLevelRequest` | `SetNotificationLevelResponse`  | Updates the current user's server-level notification preference.                                               |
| `/chatto.api.v1.ChattoApiService/SetRoomNotificationLevel`   | `SetRoomNotificationLevelRequest` | `SetNotificationLevelResponse`    | Updates the current user's room-level notification override after checking room membership.                    |
| `/chatto.api.v1.ChattoApiService/SubscribeToPush`            | `SubscribeToPushRequest`          | `SubscribeToPushResponse`         | Stores or updates the current user's Browser Push API subscription when push is configured.                    |
| `/chatto.api.v1.ChattoApiService/UnsubscribeFromPush`        | `UnsubscribeFromPushRequest`      | `UnsubscribeFromPushResponse`     | Deletes the current user's Browser Push API subscription by endpoint.                                          |
| `/chatto.api.v1.ChattoApiService/ListNotifications`          | `ListNotificationsRequest`        | `ListNotificationsResponse`       | Returns display-ready notification-center rows, including actor preview, room target, summary, server name, and pagination metadata. |
| `/chatto.api.v1.ChattoApiService/HasNotifications`           | `HasNotificationsRequest`         | `HasNotificationsResponse`        | Checks whether the current user has any pending notifications.                                                 |
| `/chatto.api.v1.ChattoApiService/DismissNotification`        | `DismissNotificationRequest`      | `DismissNotificationResponse`     | Dismisses one current-user notification and publishes the existing notification-dismissed live sync event.      |
| `/chatto.api.v1.ChattoApiService/DismissAllNotifications`    | `DismissAllNotificationsRequest`  | `DismissAllNotificationsResponse` | Dismisses every current-user notification and returns the deleted count.                                       |
| `/chatto.api.v1.ChattoApiService/ListMyRooms`                | `ListMyRoomsRequest`              | `ListMyRoomsResponse`             | Returns sidebar-ready channel or DM room rows where the caller is an explicit member, including unread state, notification preference, member previews, and channel room groups. |
| `/chatto.api.v1.ChattoApiService/GetRoomDirectory`           | `GetRoomDirectoryRequest`         | `GetRoomDirectoryResponse`        | Returns visible channel rooms for the room directory, including the caller's join capability for each row.       |
| `/chatto.api.v1.ChattoApiService/GetRoom`                    | `GetRoomRequest`                  | `GetRoomResponse`                 | Reads room metadata, permissions, mention roles, and the first member page for room bootstrap.                  |
| `/chatto.api.v1.ChattoApiService/GetRoomMembers`             | `GetRoomMembersRequest`           | `GetRoomMembersResponse`          | Reads paginated room members for the sidebar and mention autocomplete context.                                  |
| `/chatto.api.v1.ChattoApiService/SearchMembers`              | `SearchMembersRequest`            | `SearchMembersResponse`           | Searches authenticated server members for quick-switcher DM targets, returning viewer ID and DM-start capability hints. |
| `/chatto.api.v1.ChattoApiService/StartDM`                    | `StartDMRequest`                  | `StartDMResponse`                 | Finds or creates a DM room for the caller and participant IDs; empty participant IDs create/open a self-DM.     |
| `/chatto.api.v1.ChattoApiService/CreateRoom`                 | `CreateRoomRequest`               | `CreateRoomResponse`              | Creates a channel room after checking `room.create` at the target group/server scope.                          |
| `/chatto.api.v1.ChattoApiService/GetAdminRoomLayout`         | `GetAdminRoomLayoutRequest`       | `GetAdminRoomLayoutResponse`      | Returns the visible channel room-group layout for the admin room layout editor.                                |
| `/chatto.api.v1.ChattoApiService/CreateAdminRoomGroup`       | `CreateAdminRoomGroupRequest`     | `CreateAdminRoomGroupResponse`    | Creates a channel room group after checking `role.manage`.                                                     |
| `/chatto.api.v1.ChattoApiService/UpdateAdminRoomGroup`       | `UpdateAdminRoomGroupRequest`     | `UpdateAdminRoomGroupResponse`    | Updates channel room-group metadata after checking `role.manage`.                                              |
| `/chatto.api.v1.ChattoApiService/DeleteAdminRoomGroup`       | `DeleteAdminRoomGroupRequest`     | `DeleteAdminRoomGroupResponse`    | Deletes an empty channel room group after checking `role.manage`.                                              |
| `/chatto.api.v1.ChattoApiService/ReorderAdminRoomGroups`     | `ReorderAdminRoomGroupsRequest`   | `ReorderAdminRoomGroupsResponse`  | Replaces channel room-group ordering after checking `role.manage`.                                             |
| `/chatto.api.v1.ChattoApiService/MoveAdminRoomToGroup`       | `MoveAdminRoomToGroupRequest`     | `MoveAdminRoomToGroupResponse`    | Moves a channel room into another group after checking `role.manage`.                                          |
| `/chatto.api.v1.ChattoApiService/ReorderAdminRoomsInGroup`   | `ReorderAdminRoomsInGroupRequest` | `ReorderAdminRoomsInGroupResponse` | Replaces room ordering within a channel room group after checking `role.manage`.                               |
| `/chatto.api.v1.ChattoApiService/UpdateAdminRoom`            | `UpdateAdminRoomRequest`          | `UpdateAdminRoomResponse`         | Updates room metadata after checking server-level room management.                                             |
| `/chatto.api.v1.ChattoApiService/ArchiveAdminRoom`           | `ArchiveAdminRoomRequest`         | `ArchiveAdminRoomResponse`        | Archives a room after checking server-level room management.                                                   |
| `/chatto.api.v1.ChattoApiService/UnarchiveAdminRoom`         | `UnarchiveAdminRoomRequest`       | `UnarchiveAdminRoomResponse`      | Unarchives a room after checking server-level room management.                                                 |
| `/chatto.api.v1.ChattoApiService/JoinRoom`                   | `JoinRoomRequest`                 | `JoinRoomResponse`                | Joins one room after checking the room-specific join gate.                                                      |
| `/chatto.api.v1.ChattoApiService/LeaveRoom`                  | `LeaveRoomRequest`                | `LeaveRoomResponse`               | Leaves a channel room for the caller; DM rooms cannot be left.                                                  |
| `/chatto.api.v1.ChattoApiService/JoinGroup`                  | `JoinGroupRequest`                | `JoinGroupResponse`               | Joins every non-archived, joinable room in a room group and returns newly joined room IDs.                      |
| `/chatto.api.v1.ChattoApiService/BanRoomMember`              | `BanRoomMemberRequest`            | `BanRoomMemberResponse`           | Bans a channel room member after checking room moderation permission.                                           |
| `/chatto.api.v1.ChattoApiService/ListRoomBans`               | `ListRoomBansRequest`             | `ListRoomBansResponse`            | Lists active room bans for the admin moderation page after checking server-scope moderation permission.         |
| `/chatto.api.v1.ChattoApiService/UnbanRoomMember`            | `UnbanRoomMemberRequest`          | `UnbanRoomMemberResponse`         | Clears an active channel room ban after checking room moderation permission.                                    |
| `/chatto.api.v1.ChattoApiService/GetRoomEvent`               | `GetRoomEventRequest`             | `GetRoomEventResponse`            | Reads one display-ready room event by ID; requires room membership.                                             |
| `/chatto.api.v1.ChattoApiService/GetRoomTimeline`            | `GetRoomTimelineRequest`          | `GetRoomTimelineResponse`         | Reads visible room timeline events as display-ready `RoomEventView` rows.                                       |
| `/chatto.api.v1.ChattoApiService/GetRoomTimelineAfter`       | `GetRoomTimelineAfterRequest`     | `GetRoomTimelineAfterResponse`    | Reads events newer than a room timeline sequence cursor.                                                        |
| `/chatto.api.v1.ChattoApiService/GetRoomTimelineAround`      | `GetRoomTimelineAroundRequest`    | `GetRoomTimelineAroundResponse`   | Reads a room timeline window centered around an event ID.                                                       |
| `/chatto.api.v1.ChattoApiService/GetThreadEvents`            | `GetThreadEventsRequest`          | `GetThreadEventsResponse`         | Reads a thread root plus paginated reply window.                                                                |
| `/chatto.api.v1.ChattoApiService/GetThreadEventsAround`      | `GetThreadEventsAroundRequest`    | `GetThreadEventsAroundResponse`   | Reads a thread root plus reply window around an anchor reply.                                                   |
| `/chatto.api.v1.ChattoApiService/ListMyFollowedThreads`      | `ListMyFollowedThreadsRequest`    | `ListMyFollowedThreadsResponse`   | Reads the caller's followed threads with hydrated room, root-message, reply-count, participant, and unread data. |
| `/chatto.api.v1.ChattoApiService/GetLinkPreview`             | `GetLinkPreviewRequest`           | `GetLinkPreviewResponse`          | Fetches/caches a URL preview for composer link-preview sends.                                                   |
| `/chatto.api.v1.ChattoApiService/PostMessage`                | `PostMessageRequest`              | `PostMessageResponse`             | Posts a root/thread message, optional link preview, and already-uploaded attachment asset IDs; room composer uploads use `POST /api/rooms/{roomID}/attachments` first. |
| `/chatto.api.v1.ChattoApiService/UpdateMessage`              | `UpdateMessageRequest`            | `UpdateMessageResponse`           | Edits an existing message body; author or manage-others permission required.                                    |
| `/chatto.api.v1.ChattoApiService/DeleteMessage`              | `DeleteMessageRequest`            | `DeleteMessageResponse`           | Retracts an existing message; author or manage-others permission required.                                      |
| `/chatto.api.v1.ChattoApiService/DeleteAttachment`           | `DeleteAttachmentRequest`         | `DeleteAttachmentResponse`        | Removes one attachment from the caller's own message and tombstones the asset.                                  |
| `/chatto.api.v1.ChattoApiService/DeleteLinkPreview`          | `DeleteLinkPreviewRequest`        | `DeleteLinkPreviewResponse`       | Removes the link preview from the caller's own message.                                                        |
| `/chatto.api.v1.ChattoApiService/AddReaction`                | `AddReactionRequest`              | `AddReactionResponse`             | Adds a shortcode reaction to a message; requires reaction permission.                                           |
| `/chatto.api.v1.ChattoApiService/RemoveReaction`             | `RemoveReactionRequest`           | `RemoveReactionResponse`          | Removes the caller's shortcode reaction from a message; requires reaction permission.                           |
| `/chatto.api.v1.ChattoApiService/FollowThread`               | `FollowThreadRequest`             | `FollowThreadResponse`            | Follows a thread for notification/read-state behavior.                                                          |
| `/chatto.api.v1.ChattoApiService/UnfollowThread`             | `UnfollowThreadRequest`           | `UnfollowThreadResponse`          | Unfollows a thread.                                                                                             |
| `/chatto.api.v1.ChattoApiService/MarkRoomAsRead`             | `MarkRoomAsReadRequest`           | `MarkRoomAsReadResponse`          | Advances the caller's room read marker without moving it backwards.                                             |
| `/chatto.api.v1.ChattoApiService/MarkThreadAsRead`           | `MarkThreadAsReadRequest`         | `MarkThreadAsReadResponse`        | Marks a followed thread read up to an optional event.                                                           |
...
| `/chatto.api.v1.ChattoApiService/UpdateMyPresence`           | `UpdateMyPresenceRequest`         | `UpdateMyPresenceResponse`        | Updates the current user's transient presence state in `MEMORY_CACHE`.                                          |
| `/chatto.api.v1.ChattoApiService/ListActiveCalls`            | `ListActiveCallsRequest`          | `ListActiveCallsResponse`         | Reads active channel calls visible to the caller, with participant previews.                                    |
| `/chatto.api.v1.ChattoApiService/GetCallParticipants`        | `GetCallParticipantsRequest`      | `GetCallParticipantsResponse`     | Reads current participants for one active room call; requires room membership.                                  |
| `/chatto.api.v1.ChattoApiService/JoinVoiceCall`              | `JoinVoiceCallRequest`            | `JoinVoiceCallResponse`           | Records the caller's voice-call join intent as durable room facts when LiveKit is configured.                   |
| `/chatto.api.v1.ChattoApiService/LeaveVoiceCall`             | `LeaveVoiceCallRequest`           | `LeaveVoiceCallResponse`          | Records the caller's voice-call leave intent as durable room facts when LiveKit is configured.                  |
| `/chatto.api.v1.ChattoApiService/GetVoiceCallToken`          | `GetVoiceCallTokenRequest`        | `GetVoiceCallTokenResponse`       | Generates a LiveKit join token and returns the active call ID plus E2EE key; requires room membership.          |

Room, thread, current-user, server-settings, account, profile, preference, notification-level, notification-center, presence, push, RBAC, and profile-asset responses use transient protobuf DTOs such as `ServerProfileView`, `CurrentUserView`, `GetCurrentUserResponse`, `AuthenticatedServerSettingsView`, `GetAuthenticatedServerSettingsResponse`, `ServerSettingsView`, `GetServerSettingsResponse`, `UpdateServerSettingsResponse`, `AdminSecurityConfigView`, `GetAdminSecurityConfigResponse`, `UpdateBlockedUsernamesResponse`, `AdminSystemInfoView`, `AdminProjectionStateView`, `GetAdminSystemInfoResponse`, `AdminEventLogEntryView`, `ListAdminEventLogResponse`, `GetAdminEventLogEntryResponse`, `AdminRoleView`, `AdminMemberView`, `ListAdminMembersResponse`, `GetAdminMemberResponse`, `AdminUpdateUserResponse`, `AdminClearUsernameCooldownResponse`, `AssignMemberRoleResponse`, `RevokeMemberRoleResponse`, `GetAdminRoleCapabilitiesResponse`, `GetAdminRoleResponse`, `CreateAdminRoleResponse`, `UpdateAdminRoleResponse`, `DeleteAdminRoleResponse`, `RolePermissionTierMatrixView`, `RolePermissionMatrixView`, `UserPermissionMatrixView`, `PermissionMatrixScopeView`, `PermissionMatrixCellView`, `SetPermissionStateResponse`, `GetAccountDeletionStatusResponse`, `RequestAccountDeletionResponse`, `DeleteMyAccountResponse`, `ProfileSettingsView`, `GetProfileSettingsResponse`, `UpdateProfileResponse`, `GetUserSettingsResponse`, `UpdateUserSettingsResponse`, `SetNotificationLevelResponse`, `NotificationItemView`, `ListNotificationsResponse`, `HasNotificationsResponse`, `DismissNotificationResponse`, `DismissAllNotificationsResponse`, `UpdateMyPresenceResponse`, `SubscribeToPushResponse`, `UnsubscribeFromPushResponse`, `ViewerPermissionsView`, `ViewerNotificationPreferenceView`, `RoomNotificationPreferenceView`, `RoomListItemView`, `RoomDirectoryItemView`, `GetRoomDirectoryResponse`, `SearchMembersResponse`, `StartDMResponse`, `CreateRoomResponse`, `AdminRoomGroupView`, `AdminRoomInfoView`, `GetAdminRoomLayoutResponse`, `CreateAdminRoomGroupResponse`, `UpdateAdminRoomGroupResponse`, `ReorderAdminRoomGroupsResponse`, `MoveAdminRoomToGroupResponse`, `ReorderAdminRoomsInGroupResponse`, `UpdateAdminRoomResponse`, `ArchiveAdminRoomResponse`, `UnarchiveAdminRoomResponse`, `UserAvatarView`, `RoomBanView`, `ListRoomBansResponse`, `UnbanRoomMemberResponse`, `RoomEventView`, `RoomEventsPage`, `RoomMembersPage`, `FollowedThreadView`, `MentionRole`, `MessagePostedView`, `AttachmentView`, `AssetFitMode`, `UploadedAttachment`, `UploadRoomAttachmentsResponse`, `RefreshMessageAttachmentUrlsRequest`, `RefreshMessageAttachmentUrlsResponse`, `UserAvatarAssetResponse`, `ServerBrandingAssetResponse`, `LinkPreviewView`, `ReactionSummaryView`, `VideoProcessingView`, `ActiveCallView`, `CallParticipantView`, and `VoiceCallTokenView`. Notification-level reads reuse `GetViewer.server_notification_preference` and `ListMyRooms.room_views.viewer_notification_preference`; notification-center reads use `ListNotifications` to map persisted `corev1.Notification` records into display-ready `NotificationItemView` rows. Admin moderation reads use `ListRoomBans` to map active `RoomBan` projection rows into `RoomBanView` rows with room and avatar previews. Admin security reads use `GetAdminSecurityConfig` to expose effective blocked usernames and `UpdateBlockedUsernames` to persist configured overrides. Admin system diagnostics use `GetAdminSystemInfo` to map point-in-time NATS/account stats and projection health into protobuf DTOs. Admin event-log diagnostics use `ListAdminEventLog` and `GetAdminEventLogEntry` to decode EVT stream messages into human-inspection rows. Admin member management uses `AdminMemberView` and `AdminRoleView` to map user identity, avatar URL, explicit roles, visible cooldown state, role catalog, permission catalog, and viewer capability hints into protobuf DTOs. Admin role CRUD uses `AdminRoleView` for role catalog/details and assigned-user rows when the caller can assign roles. RBAC matrix views map role/user allow-deny state and effective permissions into protobuf matrix DTOs without importing GraphQL fragments. These mirror the display data previously assembled by GraphQL resolvers without making GraphQL the compatibility surface for the bundled frontend.
...
GraphQL attachment fields, wire room-event views, and the protobuf HTTP attachment URL refresh endpoint primarily return stable asset paths with a
...
the shorter `AttachmentURLTTL`. GraphQL attachment fields and protobuf
`AttachmentView`/`RefreshMessageAttachmentUrlsResponse` values use the stable `/assets/files/...` URL plus
...
- Reconnect catch-up in the web client refreshes the currently viewed room window from `RoomTimelineProjection` after browser wake or event-bus heartbeat-stall catch-up notifications. When the user is at the bottom it fetches the latest page; when scrolled up it uses `eventsAround` for the visible anchor event and preserves scroll by event ID. Server-scoped stores also refetch projected state after event-bus stall notifications so notifications, unread/sidebar state, room layout, server profile/settings, and active-call indicators do not depend on replayed subscription events. GraphQL `Subscription.myEvents` remains live-only and does not expose a replay cursor; `/api/wire` carries signed delivery cursors on `StreamEvent` frames for transport-level resume.
