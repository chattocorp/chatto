...
describe('catalog validation', () => {
  it.each([
    ...
    ['a non-string leaf', { common: { title: null } }, 'must be a string or object']
  ])('rejects %s', (_name, document, message) => {
    expect(() => extractSection(document, 'common')).toThrow(CatalogValidationError);
    expect(() => extractSection(document, 'common')).toThrow(message);
  });

  it('rejects every translated/base value-kind mismatch defensively', () => {
    expect(() =>
      validateTranslationOverlay({ nested: { title: 'Base' } }, { nested: 'Translated' }, 'common')
    ).toThrow('same value kind as the base locale');

    expect(() =>
      validateTranslationOverlay({ title: 'Base' }, { title: { nested: 'Translated' } }, 'common')
    ).toThrow('same value kind as the base locale');
  ...
  it('unwraps raw and default-export catalog modules', () => {
    ...
    expect(extractSection({ default: { common: section } }, 'common')).toEqual(section);
  });
