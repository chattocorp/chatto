...
test('sends thread-scoped typing through ConnectRPC', async () => {
  ...
  await createChattoTyping('https://chat.example', 'key')(
    destination,
...
test('typing failures do not block work, and an in-flight update is aborted on completion', async () => {
  ...
  const typing = vi.fn(async (_destination, value: AbortSignal) => {
    signal = value;
    return pending.promise;
  });
  expect(
    await withChattoTyping(createWorkflowContext(), destination, typing, async () => 'done')
  ).toBe('done');
...
test('stops after a failed turn', async () => {
  vi.useFakeTimers();
  const typing = vi.fn(async () => {
    throw new Error('no typing permission');
  });
  await expect(
    withChattoTyping(createWorkflowContext(), destination, typing, async () => {
