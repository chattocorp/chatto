...
import { messageSignal, type ChattoInbox, type ChattoPost, type Destination, type Delivery } from "./routing.ts";
...
export async function runConversationTask<Result>(
  ...
  { destination, post, typing, acknowledge, delivery, onMessage }: {
  ...
  return withTyping(ctx.signal, refreshTyping ? signal => refreshTyping(destination, signal) : undefined, async () => {
