...
| `renewable_session.{hmac}` | Mutable JSON authority for one human first-party or delegated bearer session: user ID, optional OAuth client ID, kind/source and safe request metadata, creation and current window expiry, auth generation, current refresh generation, last refresh request ID/time, and authoritative fresh-auth metadata. Ordinary KV revision OCC serializes refresh rotation across replicas. A refresh in the final quarter advances the window. Exact retry of the immediately previous generation and request ID recreates the deterministic result during the access lifetime; other stale reuse revokes this key and thereby every access generation. Raw refresh credentials are HMAC-authenticated but never stored. |
| `expiry.session.{hmac}` | Fixed expiry marker for a mutable cookie-presentation `session.{hmac}` record. Its value is the RFC 3339 absolute expiry and its per-key TTL ends one second after the fixed cookie lifetime. Every replica watches marker deletion and deletes the related record after it confirms the explicit expiry. Startup reconciliation creates a missing marker for a live record or deletes an explicitly expired record. |
| `expiry.renewable_session.{hmac}` | Expiry marker for a mutable `renewable_session.{hmac}` authority. Its value is the RFC 3339 current window expiry. Refresh deletes and recreates the marker through the public KV API when it advances the window. A marker deletion repairs a live record's marker or deletes an explicitly expired record. |
