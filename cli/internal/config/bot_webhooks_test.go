...
func TestBotWebhooksOperatorPolicy(t *testing.T) {
	...
	require.NoError(t, cfg.Core.BotWebhooks.Validate())
	require.Equal(t, 7, cfg.Core.BotWebhooks.MaxAttemptsOrDefault())
	require.Equal(t, 2*time.Minute, cfg.Core.BotWebhooks.RetryDelayOrDefault())
	require.Equal(t, 72*time.Hour, cfg.Core.BotWebhooks.ExpiryOrDefault())
...
func TestBotWebhooksEnvironment(t *testing.T) {
	...
	t.Setenv("CHATTO_CORE_BOT_WEBHOOKS_MAX_ATTEMPTS", "9")
	t.Setenv("CHATTO_CORE_BOT_WEBHOOKS_RETRY_DELAY", "15s")
	t.Setenv("CHATTO_CORE_BOT_WEBHOOKS_EXPIRY", "2h")
	cfg, err := ReadConfig("")
	require.NoError(t, err)
	require.Equal(t, 9, cfg.Core.BotWebhooks.MaxAttemptsOrDefault())
	require.Equal(t, 15*time.Second, cfg.Core.BotWebhooks.RetryDelayOrDefault())
	require.Equal(t, 2*time.Hour, cfg.Core.BotWebhooks.ExpiryOrDefault())
}
