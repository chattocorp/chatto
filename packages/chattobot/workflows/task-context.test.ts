...
test("new host phase supersedes an old setup announcement without changing retained history", () => {
  const task = { id: "worker", name: "Implementation", status: "running" as const,
    state: { phase: "editing" }, stateAt: 200, progress: "Installing dependencies", progressAt: 100,
    output: [], droppedOutput: 0 };
...
test("completed blocked results supersede stale active state and notification snapshots", () => {
  const result = JSON.stringify({ outcome: "blocked", checks: [{ passed: false, diagnostic: "Expected 1, received 0" }] });
  const [current] = taskContext([{ id: "worker", name: "Implementation", status: "completed",
    state: { phase: "validating" }, result, output: [], droppedOutput: 0 }]);
  expect(current?.state).toBeUndefined();
  expect(current?.result).toEqual(JSON.parse(result));
  expect(JSON.parse(taskNotification(JSON.stringify({ type: "task.progress", task: { id: "worker", state: { phase: "preparing" } } })))).toEqual({ type: "task.progress", taskId: "worker" });
});

test.each(["Plain task failure", '{"outcome":"blocked"\n[Task result truncated]', ""])("preserves non-JSON results: %s", result => {
  const task = { id: "worker", name: "Worker", status: "completed" as const, result, output: [], droppedOutput: 0 };
...
test("decoded results are detached from the original retained snapshot", () => {
  const task = { id: "worker", name: "Worker", status: "completed" as const,
    result: '{"outcome":"completed","plan":{"goal":"Original"}}', output: [], droppedOutput: 0 };
