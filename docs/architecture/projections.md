...
| Server/user/policy config | Server Config   | `evt.config.>`, selected user preference/deletion facts, room and room-group deletion facts | `ConfigModel`; server config, branding refs, user preferences, notification levels, blocked usernames, and sparse baseline runtime-policy overrides |
