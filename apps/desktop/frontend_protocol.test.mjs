...
test('rejects other hosts on the custom scheme', async () => {
  const handler = createFrontendProtocolHandler('/unused', async () => new Response('unexpected'));
...
test('does not serve files outside the frontend build', async (t) => {
  const root = await fixture(t);
  const handler = createFrontendProtocolHandler(root, async (input) => new Response(String(input)));

  const response = await handler(new Request(`${APP_ORIGIN}/..%2Fpackage.json`));
  assert.equal(response.status, 404);
});
