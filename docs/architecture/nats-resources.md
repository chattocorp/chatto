...
| KV bucket    | `RUNTIME_STATE`     | File    | Yes    | Persisted latest-value runtime state, fixed-expiry bearer access verifiers, mutable cookie and renewable-session authorities with explicit expiry and per-message TTL, workflow credentials, notification read/visibility boundaries, linked-image import reservations, wrapped app DEKs, and encrypted snapshot pointers |
