...
<EnvVar name="CHATTO_LIMITS_MAX_SPACES" toml="limits.max_spaces" default="-1">
  Maximum number of spaces in this instance. Each Chatto instance hosts a single server (also called a space in storage), so this limit is effectively a switch: `0` disables creation and any positive value or `-1` allows the single server to exist. The TOML key and env var still use the legacy `space` name.
</EnvVar>
