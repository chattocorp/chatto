...
- Every server route uses the saved view on a cold launch. A message permalink finds its target after the server confirms the viewer, because the target can be outside the saved view. Account settings and management pages open after the server confirms the viewer or current permissions.
