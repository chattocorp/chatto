...
Account deletion attempts to shred the user's encryption keys in the live server and remove message-owned assets from active storage. These cleanup steps can log warnings and continue on individual storage failures, so operators should monitor deletion-time errors. Existing backups are not modified.
