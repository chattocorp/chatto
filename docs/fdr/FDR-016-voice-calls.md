...
- Screen sharing is one call control on every host. In a web browser it opens the browser's own window/tab/screen picker. On macOS 15 and newer and supported Windows x64 hosts, Chatto Desktop instead opens a Chatto picker with static previews of ordinary visible application windows and complete displays. Selecting a window publishes its video and isolated owning-application audio; selecting a display is video-only so Chatto's remote call playback cannot be captured and echoed upstream. macOS native sharing offers multiple quality layers so LiveKit can match receiver size and network conditions while pausing unused layers; Windows currently publishes one full-cadence layer. Windows follows a selected application across temporary window replacement or capture stalls such as a fullscreen transition, and ends the share when the application has actually closed. Browser and native sharing occupy one mutually exclusive share slot, while camera and microphone remain independent.
- A Windows native-share sender sees a local mirror of the encoded video in their own call tile. That preview does not travel through LiveKit, consume downlink bandwidth, or depend on the sender subscribing to the native companion participant. Other participants continue to receive the share through LiveKit.
...
**Decision:** Voice controls expose one screen-share action. When the host advertises the narrow native screen-share capability, Chatto presents short-lived, single-use opaque window and display offers with in-memory previews in its own picker. Enumeration requires a user action, supersedes any earlier enumeration, and excludes Chatto Desktop's own windows. Without that capability, the action delegates to LiveKit and the browser's picker. Selecting a native source starts a publish-only LiveKit companion connection with its own opaque identity and the same per-call E2EE key. The frontend merges that connection's media into the owning member's logical participant and hides the companion as a separate tile. Remote viewers subscribe normally; on Windows, the local owner unsubscribes from both companion tracks and uses the helper's local encoded preview in their tile. Window capture publishes owning-application audio; full-display capture is video-only because system audio would contain remote call playback. Starting either implementation replaces the other; camera and microphone publications are unaffected.
...
**Tradeoff:** Desktop source enumeration requires macOS Screen & System Audio Recording permission, and static previews can become stale before selection; each helper re-resolves the offered source, verifies a window still belongs to the enumerated application, and fails safely. The Windows provider hashes the executable path for that binding rather than exposing it to the renderer. Windows gives temporary window replacement a short recovery period, so closing an application is not reflected instantaneously. Full-display viewers do not receive source audio until the native path can exclude Chatto's own playback before capture. The server must mint a separate short-lived publisher credential because reusing the member's LiveKit identity would disconnect their primary call connection. Webhooks and reconciliation exclude companion identities from durable call membership, while stale-room cleanup still removes them. The first macOS target publishes aspect-ratio-preserving H.264 quality classes with maximum edges of 1920, 1280, and 640 pixels at 60, 60, and 30 fps respectively. Windows bounds publication to 1920×1080 at up to 60 fps and publishes one full-cadence layer: LiveKit C++ 1.7 does not expose custom screen-share simulcast layers, while its default lowest layer is limited to 3 fps and makes compact game previews unusable. This costs more receiving bandwidth than a tuned game-oriented ladder; the pinned public C++/Rust fork currently adds pre-encoded transport, not custom layers. Windows prefers direct NVIDIA NVENC with low-latency multipass and adaptive quantisation, and falls back to a Media Foundation hardware MFT when direct NVENC is unavailable. It forwards complete H.264 access units without software re-encoding, while WebRTC keyframe and bitrate feedback returns to the selected hardware encoder. macOS encoding of multiple layers can consume more publisher CPU and upload when receivers simultaneously request different qualities, while dynacast avoids that cost for unused layers. Windows prioritizes a current screen-share image over delivering every captured frame: when publication falls behind, superseded pending frames are dropped rather than accumulating viewer latency. Actual cadence and quality remain dependent on the source, hardware encoder, network, SFU, and receiving device.
