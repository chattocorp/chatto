...
| `create_thread` | `bool` | True to establish the new root message as a thread immediately. Requires both message.post and message.post.replies. Channel rooms only; cannot be combined with thread_root_event_id. |
