...
Room-relevant permissions such as `message.read`, `message.read-interactions`, `message.post`, `message.attach`, `message.react`, `message.manage`, `room.join`, and `room.ban-member` can be configured at room/group/server scope. Server-only permissions such as `server.manage`, `server.manage-neighbors`, `role.manage`, and `admin.view-audit` are configured only at server scope.
