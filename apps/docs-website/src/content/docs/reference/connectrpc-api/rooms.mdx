---
title: RoomService
description: Room lifecycle, timeline, read-state, membership, direct-message, typing indicator, and moderation RPCs.
editUrl: false
---

{/* Generated from proto/chatto/{auth,discovery,api,admin,realtime}/v1/*.proto. Do not edit directly. */}

Chatto exposes this service below `/api/connect`.



Shared message and enum definitions are documented in [Shared Types And Enums](/reference/connectrpc-api/types/).



<a id="chatto-api-v1-RoomService"></a>

## RoomService

Manages room-scoped operations for the current user.

RoomService owns operations whose authorization and state are primarily scoped
to one room, including lifecycle, membership, moderation, room timeline reads,
room read state, attachments, and live typing. Resource-specific services
should still be preferred when an operation is not naturally room-scoped or
when the resource needs an independent CRUD/batch surface.

<a id="chatto-api-v1-RoomService-GetRoomReadState"></a>

### GetRoomReadState

Reads the current viewer's stored marker without changing it. Requires the
same membership and read access as MarkRoomAsRead. Missing resources
return NOT_FOUND; inaccessible resources return PERMISSION_DENIED.

```http
POST /api/connect/chatto.api.v1.RoomService/GetRoomReadState
```

<a id="chatto-api-v1-GetRoomReadStateRequest"></a>

#### Input: GetRoomReadStateRequest

Request current room read state without changing it.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Room ID. |


<a id="chatto-api-v1-GetRoomReadStateResponse"></a>

#### Result: GetRoomReadStateResponse

Current room read state, including an absent marker when none is stored.

| Field | Type | Description |
| --- | --- | --- |
| `state` | [`RoomReadState`](/reference/connectrpc-api/types/#chatto-api-v1-RoomReadState) | State for the requested room and viewer. |


<a id="chatto-api-v1-RoomService-BatchGetRoomReadStates"></a>

### BatchGetRoomReadStates

Reads up to 100 states under the same authorization as GetRoomReadState.
Missing and inaccessible resources are omitted; duplicates use first-seen order.

```http
POST /api/connect/chatto.api.v1.RoomService/BatchGetRoomReadStates
```

<a id="chatto-api-v1-BatchGetRoomReadStatesRequest"></a>

#### Input: BatchGetRoomReadStatesRequest

Request room read states. Duplicates are removed in first-seen order.

| Field | Type | Description |
| --- | --- | --- |
| `room_ids` | `repeated string` | Up to 100 room IDs. Missing or inaccessible rooms are omitted. |


<a id="chatto-api-v1-BatchGetRoomReadStatesResponse"></a>

#### Result: BatchGetRoomReadStatesResponse

Accessible room read states in first-seen request order.

| Field | Type | Description |
| --- | --- | --- |
| `states` | repeated [`RoomReadState`](/reference/connectrpc-api/types/#chatto-api-v1-RoomReadState) | States for the current viewer. An absent marker does not omit the state. |


<a id="chatto-api-v1-RoomService-CreateRoom"></a>

### CreateRoom

Creates a new channel room in a room group. The caller must be allowed to
create rooms in the target group.

```http
POST /api/connect/chatto.api.v1.RoomService/CreateRoom
```

<a id="chatto-api-v1-CreateRoomRequest"></a>

#### Input: CreateRoomRequest

Request to create a channel room.

| Field | Type | Description |
| --- | --- | --- |
| `name` | `string` | Required. Visible 1–30-code-point Unicode name of the new channel room. The server trims and stores it in NFC form. Control characters and Unicode line or paragraph separators are rejected. |
| `description` | `string` | Optional room description. |
| `group_id` | `string` | Required. Room group that should contain the new channel. |
| `universal` | `bool` | Whether the new channel should grant effective membership to eligible server members. |
| `threading_mode` | [`RoomThreadingMode`](/reference/connectrpc-api/types/#chatto-api-v1-RoomThreadingMode) | Threading policy for the new channel. UNSPECIFIED selects ENABLED. |


<a id="chatto-api-v1-CreateRoomResponse"></a>

#### Result: CreateRoomResponse

Result of creating a room.

| Field | Type | Description |
| --- | --- | --- |
| `room` | [`Room`](/reference/connectrpc-api/types/#chatto-api-v1-Room) | Created room. |


<a id="chatto-api-v1-RoomService-UpdateRoom"></a>

### UpdateRoom

Updates a room's editable metadata. The caller must be allowed to manage
rooms. Direct-message rooms cannot be universal.

```http
POST /api/connect/chatto.api.v1.RoomService/UpdateRoom
```

<a id="chatto-api-v1-UpdateRoomRequest"></a>

#### Input: UpdateRoomRequest

Request to update a room's editable metadata.
update_mask selects editable fields to apply or reset. Fields outside the
mask stay unchanged.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Room to update. |
| `name` | `optional string` | New visible 1–30-code-point Unicode room name, when changing it. The server trims and stores it in NFC form. Control characters and Unicode line or paragraph separators are rejected. |
| `description` | `optional string` | New room description, when changing it. Empty clears the description. |
| `universal` | `optional bool` | New universal membership state, when changing it. Direct-message rooms cannot be universal. |
| `slow_mode_seconds` | `optional uint32` | New per-user posting interval in seconds. Zero disables slow mode. The maximum supported interval is six hours. Direct-message rooms cannot use slow mode. |
| `threading_mode` | optional [`RoomThreadingMode`](/reference/connectrpc-api/types/#chatto-api-v1-RoomThreadingMode) | New threading policy. Direct-message rooms cannot set this field, and an explicitly supplied UNSPECIFIED or unknown value is rejected. |
| `update_mask` | `google.protobuf.FieldMask` | Editable fields to apply or reset: name, description, universal, slow_mode_seconds, threading_mode. Omit to infer populated fields; * selects all editable fields. An explicit empty mask is invalid. Unselected values are ignored. Selected absent values reset the field to its default, subject to field validation. |


<a id="chatto-api-v1-UpdateRoomResponse"></a>

#### Result: UpdateRoomResponse

Result of updating a room.

| Field | Type | Description |
| --- | --- | --- |
| `room` | [`Room`](/reference/connectrpc-api/types/#chatto-api-v1-Room) | Updated room. |


<a id="chatto-api-v1-RoomService-ArchiveRoom"></a>

### ArchiveRoom

Archives a room so it is hidden from active room lists. The caller must be
allowed to manage rooms.

```http
POST /api/connect/chatto.api.v1.RoomService/ArchiveRoom
```

<a id="chatto-api-v1-ArchiveRoomRequest"></a>

#### Input: ArchiveRoomRequest

Request to archive a room.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Room to archive. |


<a id="chatto-api-v1-ArchiveRoomResponse"></a>

#### Result: ArchiveRoomResponse

Result of archiving a room.

| Field | Type | Description |
| --- | --- | --- |
| `room` | [`Room`](/reference/connectrpc-api/types/#chatto-api-v1-Room) | Archived room. |


<a id="chatto-api-v1-RoomService-UnarchiveRoom"></a>

### UnarchiveRoom

Restores an archived room to active room lists. The caller must be allowed
to manage rooms.

```http
POST /api/connect/chatto.api.v1.RoomService/UnarchiveRoom
```

<a id="chatto-api-v1-UnarchiveRoomRequest"></a>

#### Input: UnarchiveRoomRequest

Request to unarchive a room.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Room to unarchive. |


<a id="chatto-api-v1-UnarchiveRoomResponse"></a>

#### Result: UnarchiveRoomResponse

Result of unarchiving a room.

| Field | Type | Description |
| --- | --- | --- |
| `room` | [`Room`](/reference/connectrpc-api/types/#chatto-api-v1-Room) | Unarchived room. |


<a id="chatto-api-v1-RoomService-JoinRoom"></a>

### JoinRoom

Joins the room as the current user when room permissions allow it.

```http
POST /api/connect/chatto.api.v1.RoomService/JoinRoom
```

<a id="chatto-api-v1-JoinRoomRequest"></a>

#### Input: JoinRoomRequest

Request to join a room as the current user.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Room to join. |


<a id="chatto-api-v1-JoinRoomResponse"></a>

#### Result: JoinRoomResponse

Result of joining a room.

| Field | Type | Description |
| --- | --- | --- |
| `room` | [`Room`](/reference/connectrpc-api/types/#chatto-api-v1-Room) | Joined room. |


<a id="chatto-api-v1-RoomService-JoinRoomGroup"></a>

### JoinRoomGroup

Joins every unarchived room in a group that the current user can join.
Already-joined and non-joinable rooms are skipped.

```http
POST /api/connect/chatto.api.v1.RoomService/JoinRoomGroup
```

<a id="chatto-api-v1-JoinRoomGroupRequest"></a>

#### Input: JoinRoomGroupRequest

Request to join all joinable rooms in one room group.

| Field | Type | Description |
| --- | --- | --- |
| `group_id` | `string` | Required. Room group whose rooms should be joined. |


<a id="chatto-api-v1-JoinRoomGroupResponse"></a>

#### Result: JoinRoomGroupResponse

Result of joining all joinable rooms in one room group.

| Field | Type | Description |
| --- | --- | --- |
| `joined_room_ids` | `repeated string` | Room IDs that transitioned from not joined to joined. |


<a id="chatto-api-v1-RoomService-StartDM"></a>

### StartDM

Starts a direct-message room for the current human user and the requested
participant set, or fetches its existing DM. message.post is required
only when the DM must be created. A valid request from a bot receives
PERMISSION_DENIED and cannot use this RPC to fetch an existing DM.

```http
POST /api/connect/chatto.api.v1.RoomService/StartDM
```

<a id="chatto-api-v1-StartDMRequest"></a>

#### Input: StartDMRequest

Request to start a direct-message room for a human caller, or fetch an
existing one. A human caller can fetch an existing DM without message.post.
A valid request from a bot receives PERMISSION_DENIED, including when the
room already exists.

| Field | Type | Description |
| --- | --- | --- |
| `participant_ids` | `repeated string` | Other participants to include in the direct-message room. The current user is always included by the server. An empty list creates or fetches the caller's self-DM. |


<a id="chatto-api-v1-StartDMResponse"></a>

#### Result: StartDMResponse

Result of starting or fetching a direct-message room.

| Field | Type | Description |
| --- | --- | --- |
| `room` | [`Room`](/reference/connectrpc-api/types/#chatto-api-v1-Room) | Direct-message room for the participant set. |


<a id="chatto-api-v1-RoomService-LeaveRoom"></a>

### LeaveRoom

Leaves the room as the current user. Direct-message and universal rooms
cannot be left.

```http
POST /api/connect/chatto.api.v1.RoomService/LeaveRoom
```

<a id="chatto-api-v1-LeaveRoomRequest"></a>

#### Input: LeaveRoomRequest

Request to leave a room as the current user.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Room to leave. |


<a id="chatto-api-v1-LeaveRoomResponse"></a>

#### Result: LeaveRoomResponse

Result of leaving a room.


<a id="chatto-api-v1-RoomService-ListMembers"></a>

### ListMembers

Lists effective room member IDs in ascending ID order. Resolve user details
with UserService.BatchGetUsers. Existing members and room.manage holders may
list a channel room; other nonmembers need both room.list and room.join.

```http
POST /api/connect/chatto.api.v1.RoomService/ListMembers
```

<a id="chatto-api-v1-ListMembersRequest"></a>

#### Input: ListMembersRequest

Request for members of one room.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Room whose effective members should be listed. Existing members and room.manage holders may list a channel room; other nonmembers need both room.list and room.join. |
| `search` | `string` | Optional case-insensitive search against login and display name. |
| `page` | [`PageRequest`](/reference/connectrpc-api/types/#chatto-api-v1-PageRequest) | Defaults to 250 results when absent or limit is zero. Maximum: 500. |
| `presence_statuses` | repeated [`PresenceStatus`](/reference/connectrpc-api/types/#chatto-api-v1-PresenceStatus) | Optional status filter applied before pagination. Empty means all statuses. Select ONLINE, AWAY, and DO_NOT_DISTURB for all connected members. Presence can change between pages; use realtime updates to keep it current. |


<a id="chatto-api-v1-ListMembersResponse"></a>

#### Result: ListMembersResponse

Room membership IDs. Resolve profiles with UserService.BatchGetUsers.
Empty searches do not read user profiles. Search results require profile
reads to match names. Both paths use ascending user ID order.

| Field | Type | Description |
| --- | --- | --- |
| `user_ids` | `repeated string` | Current member IDs in ascending order. Missing or deleted users are omitted. |
| `page` | [`PageInfo`](/reference/connectrpc-api/types/#chatto-api-v1-PageInfo) | Page metadata for the members that match both filters. |


<a id="chatto-api-v1-RoomService-GetMember"></a>

### GetMember

Gets one effective member, including automatic universal membership.
Channel-room members, room.manage holders, and user.manage-accounts holders
may read members. Bot owners and bot.manage holders may read their target
bots only. DMs require caller membership. Returns NOT_FOUND for nonmembers.

```http
POST /api/connect/chatto.api.v1.RoomService/GetMember
```

<a id="chatto-api-v1-GetMemberRequest"></a>

#### Input: GetMemberRequest

Request one member of one room by stable user ID.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required room ID. |
| `user_id` | `string` | Required target user ID. |


<a id="chatto-api-v1-GetMemberResponse"></a>

#### Result: GetMemberResponse

Room member response.

| Field | Type | Description |
| --- | --- | --- |
| `member` | [`DirectoryMember`](/reference/connectrpc-api/types/#chatto-api-v1-DirectoryMember) | Requested room member. |


<a id="chatto-api-v1-RoomService-BatchGetMembers"></a>

### BatchGetMembers

Gets effective member rows for the requested accounts. Uses the same
authorization as GetMember; omits unknown accounts and nonmembers.

```http
POST /api/connect/chatto.api.v1.RoomService/BatchGetMembers
```

<a id="chatto-api-v1-BatchGetMembersRequest"></a>

#### Input: BatchGetMembersRequest

Request room member rows for a set of stable user IDs.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required room ID. |
| `user_ids` | `repeated string` | Required target user IDs. Unknown IDs and users that are not members of the room are omitted from the response. |


<a id="chatto-api-v1-BatchGetMembersResponse"></a>

#### Result: BatchGetMembersResponse

Batch room member response.

| Field | Type | Description |
| --- | --- | --- |
| `members` | repeated [`DirectoryMember`](/reference/connectrpc-api/types/#chatto-api-v1-DirectoryMember) | Found members. The server preserves first-seen request order and de-duplicates repeated IDs. |


<a id="chatto-api-v1-RoomService-AddMember"></a>

### AddMember

Adds an account as an explicit channel-room member. room.manage for this
room or user.manage-accounts overrides the target's missing room.join.
Bot owners and bot.manage holders may also add bots, but without either
override the bot needs effective room.join, bounded by its owner's authority.
Preserves permission grants. Suspensions, archived, universal, and DM rooms prevent adding.

```http
POST /api/connect/chatto.api.v1.RoomService/AddMember
```

<a id="chatto-api-v1-AddMemberRequest"></a>

#### Input: AddMemberRequest

Request to add a user to a channel room.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Channel room to add the user to. |
| `user_id` | `string` | Required. User to add as an explicit room member. |


<a id="chatto-api-v1-AddMemberResponse"></a>

#### Result: AddMemberResponse

Result of adding a user to a room.

| Field | Type | Description |
| --- | --- | --- |
| `member` | [`DirectoryMember`](/reference/connectrpc-api/types/#chatto-api-v1-DirectoryMember) | Added room member. |


<a id="chatto-api-v1-RoomService-RemoveMember"></a>

### RemoveMember

Removes an explicit channel-room member. Requires room.manage for this room,
user.manage-accounts, ownership of the target bot, or bot.manage.
Removal is allowed after room.join is lost and from archived rooms.
Preserves permission grants. Universal and DM membership cannot be edited.

```http
POST /api/connect/chatto.api.v1.RoomService/RemoveMember
```

<a id="chatto-api-v1-RemoveMemberRequest"></a>

#### Input: RemoveMemberRequest

Request to remove a user from a channel room.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Channel room to remove the user from. |
| `user_id` | `string` | Required. User to remove from the room's explicit members. |


<a id="chatto-api-v1-RemoveMemberResponse"></a>

#### Result: RemoveMemberResponse

Result of removing a user from a room.

| Field | Type | Description |
| --- | --- | --- |
| `removed` | `bool` | True when an explicit room membership was removed by this call. |


<a id="chatto-api-v1-RoomService-ListSuspensions"></a>

### ListSuspensions

Lists active channel room suspensions. Requires server-scope
room.remove-member.

```http
POST /api/connect/chatto.api.v1.RoomService/ListSuspensions
```

<a id="chatto-api-v1-ListSuspensionsRequest"></a>

#### Input: ListSuspensionsRequest

Request to list active room suspensions.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Optional channel room filter. Empty lists active suspensions across all rooms. |
| `page` | [`PageRequest`](/reference/connectrpc-api/types/#chatto-api-v1-PageRequest) | Defaults to 50 results when absent or limit is zero. Maximum: 100. |


<a id="chatto-api-v1-ListSuspensionsResponse"></a>

#### Result: ListSuspensionsResponse

Active room suspensions visible to the current moderator.

| Field | Type | Description |
| --- | --- | --- |
| `suspensions` | repeated [`RoomSuspension`](/reference/connectrpc-api/types/#chatto-api-v1-RoomSuspension) | Active suspensions by creation time, newest first; equal times use event ID in ascending order. |
| `page` | [`PageInfo`](/reference/connectrpc-api/types/#chatto-api-v1-PageInfo) | Page metadata. |


<a id="chatto-api-v1-RoomService-ListRoomAttachments"></a>

### ListRoomAttachments

Lists current message-owned room attachments. Authentication and room
membership are required. Attachments also require message.read or a
matching thread relationship with message.read-interactions. The server omits attachments from
inaccessible threads. Returns PERMISSION_DENIED when the room is
inaccessible to the caller.

```http
POST /api/connect/chatto.api.v1.RoomService/ListRoomAttachments
```

<a id="chatto-api-v1-ListRoomAttachmentsRequest"></a>

#### Input: ListRoomAttachmentsRequest

Request for room-scoped attachment list pages.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required room ID. |
| `thumbnail` | [`ImageTransformOptions`](/reference/connectrpc-api/types/#chatto-api-v1-ImageTransformOptions) | Thumbnail URL options. Defaults are applied when absent. |
| `page` | [`PageRequest`](/reference/connectrpc-api/types/#chatto-api-v1-PageRequest) | Defaults to 50 results when absent or limit is zero. Maximum: 100. |


<a id="chatto-api-v1-ListRoomAttachmentsResponse"></a>

#### Result: ListRoomAttachmentsResponse

Room-scoped attachment list response.

| Field | Type | Description |
| --- | --- | --- |
| `attachments` | repeated [`RoomAttachmentListItem`](/reference/connectrpc-api/types/#chatto-api-v1-RoomAttachmentListItem) | Current attachments in newest message order. |
| `page` | [`PageInfo`](/reference/connectrpc-api/types/#chatto-api-v1-PageInfo) | Page metadata. |


<a id="chatto-api-v1-RoomService-ListPinnedMessages"></a>

### ListPinnedMessages

Lists current pinned messages in a channel room. Room membership plus
message.read or message.read-interactions are required. The server omits
pins from threads that the caller cannot read. Direct-message rooms do not
support pins.

```http
POST /api/connect/chatto.api.v1.RoomService/ListPinnedMessages
```

<a id="chatto-api-v1-ListPinnedMessagesRequest"></a>

#### Input: ListPinnedMessagesRequest

Request to list a channel room's current pinned messages.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required channel room ID. |
| `page` | [`PageRequest`](/reference/connectrpc-api/types/#chatto-api-v1-PageRequest) | Defaults to 50 results when absent or limit is zero. Maximum: 100. |


<a id="chatto-api-v1-ListPinnedMessagesResponse"></a>

#### Result: ListPinnedMessagesResponse

Current pinned messages in one channel room.

| Field | Type | Description |
| --- | --- | --- |
| `pinned_messages` | repeated [`PinnedMessage`](/reference/connectrpc-api/types/#chatto-api-v1-PinnedMessage) | Pinned messages in newest-pin-first order. |
| `page` | [`PageInfo`](/reference/connectrpc-api/types/#chatto-api-v1-PageInfo) | Page metadata. |
| `latest_pin_marker` | `string` | Opaque marker for the latest pin fact observed for this room. The value remains stable when pins are removed so clients can detect new pins without comparing wall-clock timestamps. Clients must not interpret it. |


<a id="chatto-api-v1-RoomService-CreatePinnedMessage"></a>

### CreatePinnedMessage

Pins a current message. The caller must have room.manage and must be able
to read the message. Repeating an existing pin is idempotent. Direct-message
rooms are rejected.

```http
POST /api/connect/chatto.api.v1.RoomService/CreatePinnedMessage
```

<a id="chatto-api-v1-CreatePinnedMessageRequest"></a>

#### Input: CreatePinnedMessageRequest

Request to pin a message in a channel room.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required channel room ID. |
| `message_event_id` | `string` | Required canonical message event ID. Thread replies are supported. |


<a id="chatto-api-v1-CreatePinnedMessageResponse"></a>

#### Result: CreatePinnedMessageResponse

Result of pinning a message.

| Field | Type | Description |
| --- | --- | --- |
| `pinned_message` | [`PinnedMessage`](/reference/connectrpc-api/types/#chatto-api-v1-PinnedMessage) | Current pinned-message resource. Repeating an existing pin returns it. |


<a id="chatto-api-v1-RoomService-DeletePinnedMessage"></a>

### DeletePinnedMessage

Removes a current pin. The caller must have room.manage. Removing a
missing pin is idempotent. Direct-message rooms are rejected.

```http
POST /api/connect/chatto.api.v1.RoomService/DeletePinnedMessage
```

<a id="chatto-api-v1-DeletePinnedMessageRequest"></a>

#### Input: DeletePinnedMessageRequest

Request to unpin a message from a channel room.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required channel room ID. |
| `message_event_id` | `string` | Required canonical message event ID. |


<a id="chatto-api-v1-DeletePinnedMessageResponse"></a>

#### Result: DeletePinnedMessageResponse

Result of unpinning a message.

| Field | Type | Description |
| --- | --- | --- |
| `deleted` | `bool` | True when the message is not pinned after this call. |


<a id="chatto-api-v1-RoomService-RefreshTypingIndicator"></a>

### RefreshTypingIndicator

Refreshes the current user's live-only typing indicator for a room or
thread. Room membership is required; message posting permission is not.
Send at most once every 2 seconds while typing. After posting a message,
the next typing activity can send immediately. Stop sending when typing
stops; receivers clear the indicator after 6 seconds without an event.

```http
POST /api/connect/chatto.api.v1.RoomService/RefreshTypingIndicator
```

<a id="chatto-api-v1-RefreshTypingIndicatorRequest"></a>

#### Input: RefreshTypingIndicatorRequest

Request to refresh the current user's live-only typing indicator.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Room where the current user is typing. |
| `thread_root_event_id` | `string` | Event ID of the thread root when typing inside a thread. |


<a id="chatto-api-v1-RefreshTypingIndicatorResponse"></a>

#### Result: RefreshTypingIndicatorResponse

Result of refreshing a typing indicator.


<a id="chatto-api-v1-RoomService-GetRoomEvents"></a>

### GetRoomEvents

Returns one page of room timeline events, including related user data
needed to render the page. Room membership is required. Reads also require
message.read or message.read-interactions. The server returns only related
thread roots for an interaction-scoped caller.

```http
POST /api/connect/chatto.api.v1.RoomService/GetRoomEvents
```

<a id="chatto-api-v1-GetRoomEventsRequest"></a>

#### Input: GetRoomEventsRequest

Request for a page of room timeline events.

Omit the cursor to load the initial page. Set before to a previously returned
start_cursor to page toward older events, or after to a previously returned
end_cursor to page toward newer events.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Room whose timeline should be loaded. |
| `limit` | `int32` | Maximum events. Values at or below zero use 50; values above 500 use 500. |
| `cursor.before` | `string` | Return events older than this opaque cursor. |
| `cursor.after` | `string` | Return events newer than this opaque cursor. |


<a id="chatto-api-v1-GetRoomEventsResponse"></a>

#### Result: GetRoomEventsResponse

Response containing one room timeline page.

| Field | Type | Description |
| --- | --- | --- |
| `page` | [`RoomTimelinePage`](/reference/connectrpc-api/types/#chatto-api-v1-RoomTimelinePage) | Loaded timeline page. |


<a id="chatto-api-v1-RoomService-GetRoomEventsAround"></a>

### GetRoomEventsAround

Returns a room timeline window centered around a specific event. Use this to
open a permalink, search result, or notification target in context. Returns
NOT_FOUND when the anchor event is missing or not visible in the room
timeline. Returns PERMISSION_DENIED when room membership or both read modes
are missing, or when the anchor is in an unrelated thread.

```http
POST /api/connect/chatto.api.v1.RoomService/GetRoomEventsAround
```

<a id="chatto-api-v1-GetRoomEventsAroundRequest"></a>

#### Input: GetRoomEventsAroundRequest

Request for room timeline events around a specific event.

Use this when a client needs to jump to a known event and render enough
context around it.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Room whose timeline should be loaded. |
| `event_id` | `string` | Required. Anchor event ID that should appear in the returned page. |
| `limit` | `int32` | Maximum events around the anchor. Values at or below zero use 50; values above 500 use 500. |


<a id="chatto-api-v1-GetRoomEventsAroundResponse"></a>

#### Result: GetRoomEventsAroundResponse

Response containing a room timeline window around a specific event.

| Field | Type | Description |
| --- | --- | --- |
| `page` | [`RoomTimelinePage`](/reference/connectrpc-api/types/#chatto-api-v1-RoomTimelinePage) | Loaded timeline page. |
| `target_index` | `int32` | Zero-based index of the anchor event within page.events. |


<a id="chatto-api-v1-RoomService-MarkRoomAsRead"></a>

### MarkRoomAsRead

Marks a room timeline as read through the supplied event. Room membership
is required. Reads also require message.read or
message.read-interactions. If no event
is supplied, the server marks through the latest root event that the caller
can read. Clients
usually call this after the user has viewed the latest visible event in the
room.

```http
POST /api/connect/chatto.api.v1.RoomService/MarkRoomAsRead
```

<a id="chatto-api-v1-MarkRoomAsReadRequest"></a>

#### Input: MarkRoomAsReadRequest

Request to mark a room timeline as read.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Room whose timeline should be marked read. |
| `up_to_event_id` | `string` | Highest room event ID the current user has read. When set, the event must exist as a root event in the room timeline. Leave empty to mark through the room's current latest root event. |


<a id="chatto-api-v1-MarkRoomAsReadResponse"></a>

#### Result: MarkRoomAsReadResponse

Result of marking a room timeline as read.

Clients can use the previous timestamp to place a local New messages
separator. Notification policy controls room Badge attention separately.

| Field | Type | Description |
| --- | --- | --- |
| `last_read_at` | `google.protobuf.Timestamp` | New room read timestamp stored for the current user. |
| `previous_last_read_at` | `google.protobuf.Timestamp` | Previous room read timestamp, when one existed. |


<a id="chatto-api-v1-RoomService-RemoveUser"></a>

### RemoveUser

Removes a current channel room member with a required reason. Requires
room.remove-member. Without suspension, ordinary join rules apply afterward.
Universal rooms require a suspension. Direct-message rooms are excluded.

```http
POST /api/connect/chatto.api.v1.RoomService/RemoveUser
```

<a id="chatto-api-v1-RemoveUserRequest"></a>

#### Input: RemoveUserRequest

Request to remove a user from a channel room for moderation.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Channel room to remove the user from. |
| `user_id` | `string` | Required. Current room member to remove. |
| `reason` | `string` | Required moderator-entered reason stored for audit. |
| `suspension.suspension_expires_at` | `google.protobuf.Timestamp` | Future time after which the user may rejoin. |
| `suspension.suspend_indefinitely` | `bool` | Must be true. The user cannot rejoin until the suspension is lifted. |


<a id="chatto-api-v1-RemoveUserResponse"></a>

#### Result: RemoveUserResponse

Result of a moderated room removal.


<a id="chatto-api-v1-RoomService-LiftSuspension"></a>

### LiftSuspension

Lifts an active channel room suspension. Requires room.remove-member.
Calling this when no active suspension exists succeeds.

```http
POST /api/connect/chatto.api.v1.RoomService/LiftSuspension
```

<a id="chatto-api-v1-LiftSuspensionRequest"></a>

#### Input: LiftSuspensionRequest

Request to lift a channel room suspension.

| Field | Type | Description |
| --- | --- | --- |
| `room_id` | `string` | Required. Channel room containing the suspension. |
| `user_id` | `string` | Required. Suspended user. |
| `reason` | `string` | Required moderator-entered reason stored for audit. |


<a id="chatto-api-v1-LiftSuspensionResponse"></a>

#### Result: LiftSuspensionResponse

Result of lifting a room suspension.
