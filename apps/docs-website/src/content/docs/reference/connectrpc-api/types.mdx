...
| `reaction_emoji` | `string` | Exact emoji for a reaction occurrence. Empty for every other cause. |
| `expires_at` | `google.protobuf.Timestamp` | Absolute expiry, 90 days after the source activity. |
| `thread_root_message_excerpt` | `optional string` | Current whitespace-collapsed text excerpt from the target thread's root message. Absent outside threads and when no root text is available. The server truncates the excerpt to at most 180 Unicode code points. |
...
| `important_unread_count` | `optional int32` | Unread occurrences in this room with Important attention. Absence has the same compatibility meaning as the response-level field. |
