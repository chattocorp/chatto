...
const perspectives = [
  {
    name: 'correctness',
    prompt: 'Review the change for correctness, regressions, edge cases, and unsafe behavior.'
  },
  {
    name: 'testing',
    prompt: "Review the change's test coverage, including missing cases and fragile assertions."
  },
  {
    name: 'simplicity',
    prompt:
...
export const review = task(
  {
    ...
    input: Type.Object({
      ...
      prompt: Type.String({ description: 'Optional review focus' })
    }),
    output: Type.Object({
      summary: Type.String(),
  ...
  async (ctx, { directory, prompt: input }) => {
    ...
    const diff = await exec`git diff HEAD --stat --patch --no-ext-diff`.cwd(directory).text();
      ...
      orchestrator.run(
        ...
        concat(
          ...
          reviews.map((review, index) =>
            concat(`## ${perspectives[index]!.name}`, review.summary, review.details ?? '')
          )
        )
      )
    );
