...
| `grant.{hmac}` | OAuth authorization code JSON bound to user ID, optional client ID, exact redirect URI, PKCE challenge, and the user auth generation it was issued against. A code can contain the authorizing session's original fresh-authentication time. Code creation and exchange retain that time only while it remains in the standard window; exchange does not restart the window. Uses per-key 5-minute TTL and is claimed with a revision-matched delete before exchange validation and token issuance. |
