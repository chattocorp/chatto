...
| `client_sync.{userId}.deletion_preparing` | No-TTL exclusive pre-commit fence containing its creation time. It blocks mutations before the account event. After projection catch-up, every replica revision-purges preparations for deleted accounts and active-account preparations older than one hour. |
