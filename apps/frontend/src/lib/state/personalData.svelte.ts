...
import { createPersonalDataAPI, PersonalTimeFormat } from '$lib/api-client/personalData';
...
export type PersonalDataSyncStatus = 'local' | 'loading' | 'synced' | 'unavailable' | 'error';
...
class PersonalDataState {
  ...
  status = $state<PersonalDataSyncStatus>('local');
    ...
    await this.#savePreferences(
      {
        ...
        timeFormat: renderTimeFormatToPersonal(timeFormat)
      },
  ...
  async selectHomeServer(homeServerId: string): Promise<boolean> {
    if (!serverRegistry.getServer(homeServerId)) return false;
    ...
    try {
      ...
      if (hasRemotePreferences) {
        ...
        this.#persist({
          ...
          timeFormat: personalTimeFormatToRender(remotePreferences.timeFormat)
        });
        ...
        await api.updatePreferences(
          {
            ...
            timeFormat: renderTimeFormatToPersonal(this.#cache.timeFormat)
          },
      ...
      if (
        this.#explicitHomeServerId === null &&
        remoteHome &&
        serverRegistry.getServer(remoteHome)
      ) {
      ...
      console.error('[personal-data] failed to load home-server data', error);
      this.status = 'error';
    }
  }
  ...
  async #moveToHomeServer(homeServerId: string, previousHomeServerId: string): Promise<void> {
    ...
    try {
      ...
      console.error('[personal-data] failed to move home-server data', error);
      this.status = 'error';
    }
  }
      ...
      .then(async () => {
        for (const [previousOrigin, move] of Object.entries(this.#pendingHomeMoves)) {
          ...
          try {
            ...
            console.error('[personal-data] failed to propagate home move; will retry', error);
  ...
  async #savePreferences(
    preferences: Parameters<ReturnType<typeof createPersonalDataAPI>['updatePreferences']>[0],
    ...
    try {
      ...
      console.error('[personal-data] failed to save preferences', error);
      this.status = 'error';
      throw error;
    }
  }
...
function renderTimeFormatToPersonal(value: TimeFormat): PersonalTimeFormat {
  if (value === TimeFormat.TwelveHour) return PersonalTimeFormat.TIME_FORMAT_12_HOUR;
  if (value === TimeFormat.TwentyFourHour) return PersonalTimeFormat.TIME_FORMAT_24_HOUR;
  return PersonalTimeFormat.TIME_FORMAT_UNSPECIFIED;
}

function personalTimeFormatToRender(value: PersonalTimeFormat | undefined): TimeFormat {
  if (value === PersonalTimeFormat.TIME_FORMAT_12_HOUR) return TimeFormat.TwelveHour;
  if (value === PersonalTimeFormat.TIME_FORMAT_24_HOUR) return TimeFormat.TwentyFourHour;
  return TimeFormat.Auto;
}

export const personalData = new PersonalDataState();

/** Wire home-server loading and server-directory reconciliation to the root shell. */
export function usePersonalDataSync(): void {
  $effect(() => {
    ...
    if (!homeServerId || !authenticated) {
      personalData.reset();
      return;
    }
    void personalData.load(homeServerId);
  });
  ...
  const authenticationFingerprint = $derived(
    JSON.stringify(
      serverRegistry.servers.map((server) => [server.id, serverRegistry.isAuthenticated(server.id)])
    )
  );

  $effect(() => {
    void directoryFingerprint;
    void authenticationFingerprint;
    void serverRegistry.homeServerId;
    void personalData.reconcileDirectory();
    void personalData.retryPendingHomeMoves();
  });
}
