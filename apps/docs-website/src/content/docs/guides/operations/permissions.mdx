...
<Aside type="tip">
  Prefer room permissions for local policy. Use `everyone` for ordinary-member
  defaults, named roles for reusable access, and a direct-user or
  restriction-role deny for a restriction. Avoid adding a nearer allow to the
  same user or restriction role.
</Aside>
