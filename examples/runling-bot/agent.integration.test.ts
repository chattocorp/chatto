  ...
  test(`real Runling validates and delivers an outcome, recovery=${recover}`, async (t) => {
    ...
    t.mock.method(globalThis, 'fetch', async (url: string | URL | Request, init?: RequestInit) => {
      try {
        ...
        assert.ok(system.includes('{"outcome":"completed","summary":"Hello!"}'));
        assert.deepEqual(
        ...
        const report = body.tools.find(
          (tool: { function: { name: string } }) => tool.function.name === 'report_outcome'
        );
        assert.deepEqual(
        ...
        assert.deepEqual(posts, [], 'No message is posted before the final report');
        requests++;
        ...
        return new Response('{"error":{"message":"Synthetic request assertion failed"}}', {
          status: 400
        });
      }
    });
