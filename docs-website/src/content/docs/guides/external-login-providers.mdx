...
For container platforms, set `CHATTO_AUTH_PROVIDERS` to a JSON array. The field names match the TOML keys:

```bash
CHATTO_AUTH_PROVIDERS='[
  {
    "id": "chatto-hub",
    "type": "openid-connect",
    "label": "Chatto Hub",
...
`CHATTO_AUTH_PROVIDERS` replaces any providers from `chatto.toml`.
...
Those legacy variables create one `openid-connect` provider with `id = "oidc"`. Do not combine `CHATTO_AUTH_PROVIDERS` with `CHATTO_AUTH_OIDC_ENABLED`.
