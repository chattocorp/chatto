...
<span
  class={[
    ...
    compact ? 'flex min-w-0 gap-2 whitespace-nowrap' : 'inline-flex flex-wrap gap-x-3 gap-y-1'
  ]}
...
>
  <span class="min-w-0 truncate">{tokenCount(usage).toLocaleString()} tokens</span>
...
{#if detail}
  ...
  {#if usage.costIncomplete}<p class="text-base-content/60 text-xs">
      Some tokens have no reported price. The estimate includes only known costs.
    </p>{/if}
{/if}
