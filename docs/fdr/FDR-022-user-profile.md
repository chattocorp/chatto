...
**Decision:** Login changes wait for the user projection to catch up, check its normalised login-digest index, and append the encrypted login-change event plus digest with optimistic concurrency over the user subject family. If another writer wins first, the operation retries against the updated projection. The plaintext login is decrypted only when a user read is hydrated.
