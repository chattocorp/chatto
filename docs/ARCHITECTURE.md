...
| `MyEventsService`       | [`my_events_service.go`](../cli/internal/core/my_events_service.go)                                                           | `myEvents` live delivery, projection readiness, heartbeats, and per-user authorization                                       |
...
- Live-only subscription delivery. Missed visible room state after reconnect is recovered from projected reads, while transient sync and presence signals remain live-only.
...
- Reconnect catch-up in the web client refreshes the currently viewed room window from `RoomTimelineProjection` after wake/reconnect. When the user is at the bottom it fetches the latest page; when scrolled up it uses `eventsAround` for the visible anchor event and preserves scroll by event ID. `Subscription.myEvents` is live-only and does not expose a replay cursor.
