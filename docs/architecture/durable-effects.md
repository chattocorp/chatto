...
| Client-sync privacy cleanup after account deletion | Retained `client_sync.{userId}.deleted` fence blocks later writes; `client_sync.{userId}.deletion_pending` identifies unfinished cleanup | The deletion command exclusively creates both markers, commits the account event, then purges preferences and the known-server directory | Every replica periodically scans only pending markers and verifies the account is deleted before purging; a post-write fence check handles mutations that raced deletion | Recoverable and idempotent; revision-owned rollback cannot remove another deletion attempt's markers, the pending marker is removed after success, and the fence is retained |
