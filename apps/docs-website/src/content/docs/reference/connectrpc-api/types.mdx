...
<a id="chatto-api-v1-BotPermission"></a>

### BotPermission

One permission in a bot's effective allowlist. Membership and other action
requirements still apply. Broader scopes replace equivalent narrower entries.

| Field | Type | Description |
| --- | --- | --- |
| `permission` | `string` | Stable permission key, such as message.read. |
| `scope` | [`BotPermissionScope`](#chatto-api-v1-BotPermissionScope) | Where the permission applies. |
| `scope_id` | `string` | Room or group ID. Empty for server and direct-message scopes. |
| `scope_name` | `string` | Current room or group name. Empty for server and direct-message scopes. |
| `active` | `bool` | False means the stored grant is unavailable under the owner's current permissions. Only the owner and authorised human bot managers see it. |
...
<a id="chatto-api-v1-BotPermissionScope"></a>

### BotPermissionScope

The scope of a bot permission. Channel scopes never include direct messages.

| Name | Number | Description |
| --- | --- | --- |
| `BOT_PERMISSION_SCOPE_UNSPECIFIED` | `0` | No scope was supplied. |
| `BOT_PERMISSION_SCOPE_SERVER` | `1` | Server authority, or a default for all channel rooms and groups. |
| `BOT_PERMISSION_SCOPE_GROUP` | `2` | A room group and its channel rooms. |
| `BOT_PERMISSION_SCOPE_ROOM` | `3` | One channel room. |
| `BOT_PERMISSION_SCOPE_DM` | `4` | Direct messages the bot belongs to; participant identities are not exposed. |
