...
| Type             | Use for                                      |
| ---------------- | -------------------------------------------- |
| `openid-connect` | Any standards-compatible OIDC provider       |
| `github`         | GitHub OAuth apps                            |
| `gitlab`         | GitLab OAuth applications                    |
| `google`         | Google OAuth clients                         |
| `discord`        | Discord OAuth2 applications                  |

Other provider types are not exposed as curated providers yet. Use `openid-connect` for standards-compatible OIDC providers.
...
CHATTO_AUTH_PROVIDERS_0_TYPE=openid-connect
...
Those legacy variables create one `openid-connect` provider with `id = "oidc"`. Do not combine counted `CHATTO_AUTH_PROVIDERS_<index>_*` variables with `CHATTO_AUTH_OIDC_ENABLED`.
...
Older clients may still read GraphQL `enabledAuthProviders` or `/api/server.authMethods`. Those legacy fields stay method-oriented, so OIDC appears as `oidc` even when the configured provider ID is something like `chatto-hub`.
