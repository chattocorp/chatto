...
func TestRequireSpaceMember(t *testing.T) {
	...
	t.Run("space member succeeds", func(t *testing.T) {
		user, err := requireSpaceMember(env.authContext(), env.core, "channel")
		if err != nil {
			t.Fatalf("Unexpected error: %v", err)
		}
		if user.Id != env.testUser.Id {
	...
	t.Run("unauthenticated returns auth error", func(t *testing.T) {
		_, err := requireSpaceMember(env.unauthContext(), env.core, "channel")
		if !errors.Is(err, ErrNotAuthenticated) {
			t.Errorf("Expected ErrNotAuthenticated, got %v", err)
		}
	})
...
func TestRequireRoomMember(t *testing.T) {
	...
	t.Run("room member succeeds", func(t *testing.T) {
		user, err := requireRoomMember(env.authContext(), env.core, "channel", env.testRoom.Id)
		if err != nil {
			t.Fatalf("Unexpected error: %v", err)
		}
		if user.Id != env.testUser.Id {
	...
	t.Run("space member but not room member fails", func(t *testing.T) {
		...
		_, err = requireRoomMember(env.authContextForUser(spaceMember), env.core, "channel", env.testRoom.Id)
	...
	t.Run("not even a space member fails", func(t *testing.T) {
		...
		_, err = requireRoomMember(env.authContextForUser(outsider), env.core, "channel", env.testRoom.Id)
		if !errors.Is(err, ErrNotRoomMember) {
			t.Errorf("Expected ErrNotRoomMember, got %v", err)
		}
	})
	...
	t.Run("unauthenticated returns auth error", func(t *testing.T) {
		_, err := requireRoomMember(env.unauthContext(), env.core, "channel", env.testRoom.Id)
		if !errors.Is(err, ErrNotAuthenticated) {
			t.Errorf("Expected ErrNotAuthenticated, got %v", err)
		}
	})
