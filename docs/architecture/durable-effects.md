...
| Video derivative processing | `AssetProcessingStartedEvent`; `AssetProcessingSucceededEvent` or `AssetProcessingFailedEvent` is terminal | A process-local callback starts bounded ffmpeg work, creates temporary MP4 renditions, uploads a thumbnail and HLS segments, then publishes the terminal manifest; animated GIF loops upload one MP4 derivative; partial uploads are tombstoned and deleted best-effort | Each enabled video service scans the asset projection at boot for unmanifested video attachments and resubmits them; terminal event OCC prevents later manifest replacement; HLS packaging/upload failure records a failed outcome | Partially recoverable; scheduling is durable, but ownership is process-local and duplicate derivative work is possible across replicas |
