...
const profile = {
  name: 'Recovered server', version: '0.5.0', welcomeMessage: null,
  description: null, iconUrl: null, bannerUrl: null,
  directRegistrationEnabled: true, directLoginEnabled: true
};
const user = { id: 'test-user', login: 'test', displayName: 'Test', avatarUrl: null } as CurrentUser;

async function register() {
  registry.addServer({
    id: 'retry-test', url: 'https://retry.test', name: 'Saved server', iconUrl: null, addedAt: 1
...
describe('registered server recovery', () => {
  ...
  it('deduplicates concurrent recovery and cannot restore a removed server', async () => {
    ...
    mocks.viewer.mockImplementation(() => new Promise<CurrentUser>((finish) => { resolve = finish; }));
