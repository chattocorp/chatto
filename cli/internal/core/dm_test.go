...
func TestFindOrCreateDM(t *testing.T) {
	...
	t.Run("creates new DM between two users", func(t *testing.T) {
		...
		isMember1, _ := core.RoomMembershipExists(ctx, "dm", user1, room.Id)
		isMember2, _ := core.RoomMembershipExists(ctx, "dm", user2, room.Id)
		if !isMember1 {
	...
	t.Run("creates group DM with three users", func(t *testing.T) {
		...
		for _, userID := range []string{user1, user2, user3} {
			isMember, _ := core.RoomMembershipExists(ctx, "dm", userID, room.Id)
			if !isMember {
	...
	t.Run("creates self-DM with no other participants", func(t *testing.T) {
		...
		isMember, _ := core.RoomMembershipExists(ctx, "dm", user1, room.Id)
		if !isMember {
...
func TestListDMConversations(t *testing.T) {
	...
	t.Run("includes DMs with messages", func(t *testing.T) {
		...
		_, err = core.PostMessage(ctx, "dm", dm1.Id, user1.Id, "Hello!", nil, "", "", nil, false)
		if err != nil {
		...
		_, err = core.PostMessage(ctx, "dm", dm2.Id, user1.Id, "Hey!", nil, "", "", nil, false)
		if err != nil {
			t.Fatalf("PostMessage error: %v", err)
		}
...
func TestListDMConversationsSortedByLastMessage(t *testing.T) {
	...
	_, err = core.PostMessage(ctx, "dm", dmAB.Id, userA.Id, "First message in A-B", nil, "", "", nil, false)
	if err != nil {
		t.Fatalf("Failed to post message to A-B: %v", err)
	}

	// Post message to A-C second (more recent)
	_, err = core.PostMessage(ctx, "dm", dmAC.Id, userA.Id, "Message in A-C", nil, "", "", nil, false)
	if err != nil {
	...
	_, err = core.PostMessage(ctx, "dm", dmAB.Id, userB.Id, "New message in A-B", nil, "", "", nil, false)
	if err != nil {
...
func TestDMUnreadStatus(t *testing.T) {
	...
	t.Run("no unread when no messages", func(t *testing.T) {
		hasUnread, err := core.HasUnread(ctx, "dm", user2.Id, room.Id)
		if err != nil {
	...
	t.Run("user2 has unread after user1 posts", func(t *testing.T) {
		...
		_, err = core.PostMessage(ctx, "dm", room.Id, user1.Id, "Hello from user1!", nil, "", "", nil, false)
		if err != nil {
		...
		hasUnread, err = core.HasUnread(ctx, "dm", user1.Id, room.Id)
		if err != nil {
	...
	t.Run("unread clears after marking as read", func(t *testing.T) {
		...
		lastID, _, exists, err := core.GetRoomLastEvent(ctx, "dm", room.Id)
		if err != nil {
		...
		if err := core.SetLastReadEventID(ctx, "dm", user2.Id, room.Id, lastID); err != nil {
...
func TestDMReactions(t *testing.T) {
	...
	// Post a message
	event, err := core.PostMessage(ctx, "dm", room.Id, user1.Id, "Test DM message for reactions", nil, "", "", nil, false)
	if err != nil {
		t.Fatalf("Failed to post message: %v", err)
	}
	...
	t.Run("can add reaction to DM message", func(t *testing.T) {
		added, err := core.AddReaction(ctx, "dm", room.Id, messageEventID, "thumbsup", user2.Id)
		if err != nil {
	...
	t.Run("can remove reaction from DM message", func(t *testing.T) {
		removed, err := core.RemoveReaction(ctx, "dm", room.Id, messageEventID, "thumbsup", user2.Id)
		if err != nil {
...
func TestDMNotifications(t *testing.T) {
	...
	t.Run("DM message triggers notification to other participants", func(t *testing.T) {
		...
		_, err = core.PostMessage(ctx, "dm", room.Id, user1.Id, "Test DM notification message", nil, "", "", nil, false)
		if err != nil {
			t.Fatalf("Failed to post message: %v", err)
		}
	...
	t.Run("DM message does not notify sender", func(t *testing.T) {
		...
		// Post a message from user1
		_, err = core.PostMessage(ctx, "dm", room.Id, user1.Id, "Another test DM message", nil, "", "", nil, false)
		if err != nil {
			t.Fatalf("Failed to post message: %v", err)
		}
...
func TestDMThreadReplyEcho(t *testing.T) {
	...
	t.Run("echo works in DM rooms", func(t *testing.T) {
		// Post root message
		rootEvent, err := core.PostMessage(ctx, "dm", room.Id, user1.Id, "DM thread root", nil, "", "", nil, false)
		if err != nil {
			t.Fatalf("Failed to post root: %v", err)
		}

		// Post thread reply with echo
		replyEvent, err := core.PostMessage(ctx, "dm", room.Id, user1.Id, "DM thread reply echoed", nil, rootEvent.Id, "", nil, true)
		if err != nil {
		...
		roomEventsResult, err := core.GetRoomEvents(ctx, "dm", room.Id, 50, nil)
		if err != nil {
	...
	t.Run("echo does not appear in thread events", func(t *testing.T) {
		...
		rootEvent, _ := core.PostMessage(ctx, "dm", room.Id, user2.Id, "DM root for thread test", nil, "", "", nil, false)
		_, err = core.PostMessage(ctx, "dm", room.Id, user2.Id, "DM reply for thread test", nil, rootEvent.Id, "", nil, true)
		if err != nil {
			t.Fatalf("Failed to post echo reply: %v", err)
		}

		// Thread events should not contain the echo
		threadEvents, err := core.GetThreadEvents(ctx, "dm", room.Id, rootEvent.Id)
		if err != nil {
	...
	t.Run("reply_count only increments once with echo in DM", func(t *testing.T) {
		rootEvent, _ := core.PostMessage(ctx, "dm", room.Id, user1.Id, "DM root for count", nil, "", "", nil, false)
		_, err = core.PostMessage(ctx, "dm", room.Id, user1.Id, "DM reply with echo", nil, rootEvent.Id, "", nil, true)
		if err != nil {
