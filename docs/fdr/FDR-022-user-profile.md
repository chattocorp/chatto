...
- **Bio** — human and bot accounts can set a self-authored Markdown bio of up to 1,000 characters through `MyAccountService.UpdateProfile`. The bio is shown on the Profile Card and in the Profile View. The client disables source HTML and sanitizes rendered output. In the Profile View, a non-empty bio appears in a collapsible **Bio** section. It starts expanded. The browser remembers the choice per server. The content stays mounted while collapsed so Markdown loading does not interrupt the expansion animation.
...
- **Profile View** — “View profile” opens or returns to a one-to-one direct message with that user, then opens their complete public profile in that conversation's Room Sidebar. It stays available when the viewer can open an existing DM but cannot create a new one. If no such DM exists, the action is disabled. The Profile View shows the avatar, display name, login, custom status, bio, and local time. The direct-message header has an information button that opens the same view again, including in a self-DM. Closing it returns to the prior room-extras panel, or hides the Room Sidebar when no panel was open.
...
**Decision:** The Profile View is a view in the Room Sidebar of that user's one-to-one direct message. “View profile” opens that direct message and its Profile View. It can reuse an existing DM without permission to create new DMs, but it is disabled when no reusable DM exists. The direct-message header has an information button for the same view. Desktop DMs show this view by default, unless the user has saved another sidebar choice. Channel-room information will use that information-button pattern later.
...
**Tradeoff:** Viewing a profile can navigate away from the current room and can create a direct message when permission allows it. A direct profile URL is not available. Closing the profile restores the previous sidebar panel in the direct message, or keeps the sidebar closed if there was no previous panel. The browser stores this choice locally; it does not sync to another device.
