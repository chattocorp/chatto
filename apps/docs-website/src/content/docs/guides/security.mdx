...
Chatto never mounts AdminService on the public web listener. This is intentional defense in depth: normal public reverse-proxy routing for Chatto's web UI and public APIs cannot accidentally expose `/api/connect/chatto.api.v1.AdminService/*`. The `chatto admin ...` CLI also prefers the dedicated listener URL from `chatto.toml` or `CHATTO_ADMIN_API_LISTENER_*`, so local operator commands keep using the private endpoint automatically.
