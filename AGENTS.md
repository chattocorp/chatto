...
- Modal action buttons should follow the established modal footer style: visible `Button` variants, laid out horizontally in the footer with `justify-end gap-2`. Do not put primary modal actions in a vertical stack in the body, and do not use visually invisible/ghost buttons for core modal actions like Cancel.
