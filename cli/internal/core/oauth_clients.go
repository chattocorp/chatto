...
func (c *ChattoCore) CreateOAuthClientAuthorizationCode(ctx context.Context, authorization OAuthClientAuthorization, redirectURI, codeChallenge, codeChallengeMethod string, authGeneration uint64, freshAuthAt time.Time) (string, error) {
	return c.createOAuthClientAuthorizationCode(ctx, authorization, redirectURI, codeChallenge, codeChallengeMethod, authGeneration, freshAuthAt, c.oauthClientModel.projection.Projector().WaitFor)
}

func (c *ChattoCore) createOAuthClientAuthorizationCode(ctx context.Context, authorization OAuthClientAuthorization, redirectURI, codeChallenge, codeChallengeMethod string, authGeneration uint64, freshAuthAt time.Time, waitFor func(context.Context, events.StreamPosition) error) (string, error) {
	code, err := c.CreateAuthCodeForClientGeneration(ctx, authorization.UserID, authorization.ClientID, redirectURI, codeChallenge, codeChallengeMethod, authGeneration, freshAuthAt)
	if err != nil {
		return "", err
	}
