...
`core.secret_key` is not a message-encryption KEK. Rotating it invalidates bearer tokens, cookie-session handles, OAuth authorization codes, pending account-flow credentials, and existing invitation links, but it does not rotate or invalidate the KEK records in `ENCRYPTION_KEYS`. Invitation records remain in `EVT`, so authorised administrators can copy newly derived links after rotation.
