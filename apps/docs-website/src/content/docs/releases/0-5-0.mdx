...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="Bearer sessions renew without another login">
    ...
    fixed maximum requires a deliberate reconnect. The bundled same-origin client now keeps its
    safer HttpOnly cookie instead of persisting a duplicate origin bearer session.
  </ReleaseFeatureCard>
