...
Chatto uses opaque runtime credentials, not JWTs. Browser sessions use signed HTTP-only cookies for same-origin flows, including direct external-provider login, and provider redirects never contain bearer credentials. Registered cross-origin server connections use Chatto's OAuth PKCE flow to obtain a short-lived access token and rotating refresh credential. The client renews in the background until the session's fixed maximum lifetime, without reopening OAuth. Password resets, password changes, external identity disconnects, and account deletion revoke older credentials through the user's auth generation.
