...
| `push_subscription.{userId}.{endpointHash}` | Web Push subscription record (protobuf `PushSubscription`) for a user's browser/device and service-worker registration. The optional client host identifies the Chatto server that supplied the installed app; the sending server combines it with its own hostname to reconstruct the click route. Legacy records without it use the sending server's bundled-client route. The endpoint hash keeps multiple devices and per-server scoped subscriptions per user while deduplicating the same browser subscription. A record is deliverable only while its revision matches the endpoint's active owner claim. |
| `push_endpoint_owner.{sha256(endpoint)}` | JSON Web Push endpoint owner claim containing the active user ID and exact `push_subscription` KV revision. Legacy and client-host-aware subscriptions use separate additive user-ID slots; an older replica cannot recognize the latter and therefore skips delivery during a rolling upgrade. Saves transfer the claim with KV OCC; revision-matched deletes prevent stale logout, expiry cleanup, and subscription rotation races from releasing a newer claim. Legacy subscription records without a claim remain inert until the browser re-registers. |
