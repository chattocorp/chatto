...
| Permission                | Meaning                                                                                                                                                                         |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `message.read`            | Read message and thread content, attachments, message search results, typing indicators, and message-derived activity. Room membership is also required. Fresh servers allow this for `everyone`; existing servers are never backfilled. |
...
<Aside type="caution" title="Existing servers need an explicit read grant">
  ...
  that must keep message access. Grant it to `everyone` at server scope to
  preserve the previous membership-based behavior. Chatto does not migrate,
  backfill, or reconcile existing RBAC state.
</Aside>
...
Only participants with `message.read` can read a DM. Owners cannot read or
moderate a DM they are not part of.
