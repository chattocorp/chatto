...
test('continues an existing DM thread', async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal((posts[0] as { threadRootEventId: string }).threadRootEventId, 'root');
});
...
test('skips bot authors to prevent reply loops', async () => {
  ...
  assert.equal((await runWorkflow(workflow, { input })).output?.status, 'skipped');
  assert.equal(posts.length, 0);
});
...
test('repeated final delivery shares one POST even when it fails', async () => {
  for (const postStatus of [200, 503]) {
    ...
    assert.equal((await runWorkflow(workflow, { input })).ok, postStatus === 200);
    assert.equal(posts.length, 1);
