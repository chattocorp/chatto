...
const event = (payload: RunlingEventPayload, activityId?: string): RunlingEvent =>
  ({
    ...payload,
    activityId,
    timestamp: 0
...
test('renders activity as one semantic execution tree', () => {
  const dashboard = new RunlingDashboard('examples/review.ts', 0);
  dashboard.handle(event({ type: 'step.started', id: 'review', label: 'Review' }));
  dashboard.handle(
    event({ type: 'step.started', id: 'investigate', label: 'Investigate change' }, 'review')
  );
  dashboard.handle(
    event({ type: 'command.started', id: 'command', command: 'git status --short' }, 'investigate')
  );
  dashboard.handle(
    event(
      {
...
test('renders a Markdown result supplied as the workflow summary', () => {
  const dashboard = new RunlingDashboard('examples/joke.ts', 0);
...
test('shows a bounded, ANSI-free tail for failed commands', () => {
  ...
  dashboard.handle(event({ type: 'step.started', id: 'test', label: 'Run tests' }));
  dashboard.handle(
    event({ type: 'command.started', id: 'bun-test', command: 'pnpm test' }, 'test')
  );
  dashboard.handle(
    event(
      {
...
test('never renders beyond the available width', () => {
  ...
  expect(dashboard.render(20).every((line) => visibleWidth(line) <= 20)).toBe(true);
});
...
test('collects workflow input inline in the execution tree', async () => {
  ...
  expect(stripVTControlCharacters(terminal.output)).toContain('What should we call this release?');
...
test('renders timeout and cancellation reasons for questions', () => {
  ...
  for (const reason of ['timeout', 'cancelled'] as const) {
    ...
    dashboard.handle(
      event({ type: 'input.finished', id: reason, status: 'failed', reason, durationMs: 1000 })
    );
  }
