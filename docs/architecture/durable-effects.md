...
| Client-sync privacy cleanup after account deletion | Retained `client_sync.{userId}.deleted` marker in `RUNTIME_STATE` blocks later writes and identifies the two user records to purge | Account deletion creates the marker and purges preferences and the known-server directory | Every replica repeats marked purges at boot; a post-write marker check purges mutations that raced deletion | Recoverable and idempotent; markers are intentionally retained |
