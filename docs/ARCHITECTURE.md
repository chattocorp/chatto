...
| `mention_handle.{normalizedHandle}` | Atomic mention namespace claim shared by user logins and role names. Value is JSON identifying the owning user or role. No TTL; released when a user changes/deletes their login or a custom role is deleted. |
