...
New durable user events also encrypt login, display name, and verified email fields with the user's active content key epoch. Projections decrypt those fields while the key exists and derive the in-memory login/email indexes used by the API. Legacy plaintext user events remain readable for compatibility.
