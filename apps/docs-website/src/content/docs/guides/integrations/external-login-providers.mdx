...
- Removing a role in Server Admin clears every current source for that member. An enabled provider can grant it again at the next synchronization.
- Removing the provider configuration or its role-claim settings revokes its remaining managed sources during the next server startup.
