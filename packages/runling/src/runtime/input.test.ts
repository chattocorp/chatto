...
import { observeRunlingEvents, withRunlingActivity, type RunlingEvent } from './events.ts';
...
test('delegates input to the host and reports its lifecycle', async () => {
  ...
  const answer = await observeRunlingEvents(
    ...
    () =>
      withRunlingActivity('deploy', () => input('Which environment?', { defaultValue: 'staging' }))
  );
...
test('fails clearly when the host has no input handler', async () => {
  await expect(createInput()('Choose wisely')).rejects.toBeInstanceOf(InputUnavailableError);
});
...
test('parallel spread contexts route questions independently and permit composition', async () => {
  ...
  const child = {
    ...parent,
    onInput: async (request: Parameters<typeof previous>[0]) => 'child:' + (await previous(request))
  };
  expect(await Promise.all([input(parent, 'one'), input(child, 'two')])).toEqual([
    'parent:one',
    'child:parent:two'
  ]);
});
...
test('aborts pending input even if its handler ignores cancellation', async () => {
  ...
  expect(events.filter((e) => e.type === 'input.finished').map((e) => e.status)).toEqual([
    'failed'
  ]);
});
