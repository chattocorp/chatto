...
| Video derivative processing | `AssetProcessingStartedEvent`; `AssetProcessingSucceededEvent` or `AssetProcessingFailedEvent` is terminal. Failed events record the derivative IDs created by that attempt | A process-local callback starts bounded ffmpeg work, creates temporary MP4 renditions, uploads a thumbnail and HLS segments, then publishes the terminal manifest; animated GIF loops upload one MP4 derivative. Failure publication has an independent bounded context and precedes prompt cleanup | Each enabled video service scans the asset projection at boot for assets without a completed/failed outcome, including `STARTED`-only attempts, and resubmits them. Terminal event OCC prevents later manifest replacement. The elected asset-cleanup worker incrementally consumes failed outcomes and tombstones any recorded partial derivatives; normal deletion recovery then removes their bytes idempotently after timeout, restart, or lease handover | Recoverable scheduling and partial-derivative cleanup; processing ownership remains process-local, so duplicate derivative work is possible across replicas |
...
| Asset binary and transform-cache deletion | `AssetDeletedEvent` makes projected reads and signed asset resolution reject the asset; the asset ID locates the canonical aggregate's durable creation metadata | Message deletion, attachment removal, account cleanup, pending-upload expiry, and derivative cleanup delete NATS/S3 bytes and cached transforms after recording deletion | The elected `asset_cleanup` worker incrementally consumes canonical `asset_processing_failed` cleanup intents before canonical `asset_deleted` facts. It tombstones recorded failed-generation derivatives, loads each deletion's storage metadata from the matching `asset_created` fact, and retries idempotent binary/cache deletion after failure, restart, or lease handover. A source-video tombstone also re-reads its durable HLS manifest and tombstones any still-live HLS children, repairing deletion by an older HLS-unaware replica; beta room-scoped facts without a canonical creation aggregate are skipped | Recoverable for canonical message-owned asset deletion facts, failed video generations, and mixed-version HLS cleanup; beta room-scoped cleanup remains best-effort |
