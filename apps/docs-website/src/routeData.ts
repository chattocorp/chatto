...
const upsertMeta = (head: Head, attribute: 'name' | 'property', key: string, content: string) => {
  const entry = {
    tag: 'meta' as const,
    attrs: { [attribute]: key, content }
  };
  const index = head.findIndex(({ tag, attrs }) => tag === 'meta' && attrs?.[attribute] === key);
...
export const onRequest = defineRouteMiddleware(({ locals, site }) => {
  ...
  const description = route.entry.data.description ?? 'Self-hosting documentation for Chatto.';
