...
commits `RoomCreatedEvent` with `RoomAddedToGroupEvent`. A source-bound operator
creation also records its type-scoped source identity and original request
identity in `RoomCreatedEvent`; replay rebuilds the durable retry claim from
that fact. Channel-room deletion
