...
| Bot API-key runtime materialization | An authorization-fenced `BotAPIKeyRotatedEvent` or `BotAPIKeyRevokedEvent` identifies the latest acceptable credential state | Rotation writes the matching HMAC verifier and intent sequence to `RUNTIME_STATE`; revocation deletes the runtime record | Validation waits for the intent projection and rejects missing, revoked, or mismatched runtime records. A failed rotation write is recoverable by another rotation; a failed revocation delete leaves inert cleanup debt | Authentication safety is durable; runtime materialization is request-retried rather than worker-reconciled |
