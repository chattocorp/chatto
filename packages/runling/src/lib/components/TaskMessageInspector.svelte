    ...
    <header class="flex items-start justify-between gap-4 border-b border-base-300 px-6 py-4">
      <div>
        ...
        <p class="mt-1 text-xs text-base-content/60">
          {selected.status} · {duration(selected.timestamp)}
        </p>
      </div>
      <button
        class="btn btn-ghost btn-sm btn-square"
        onclick={() => dialog.close()}
        aria-label="Close task message"
      >
    ...
    <div class="min-h-0 space-y-4 overflow-auto px-6 py-5">
      {#if messages.length > 1}
        <label class="block text-xs text-base-content/60">
          Message
          <select
            class="select select-sm mt-1 w-full text-base-content"
            aria-label="Select task message"
            value={selected.id}
            onchange={(event) => onselect(event.currentTarget.value)}
          >
            {#each messages as item (item.id)}
              <option value={item.id}
                >{duration(item.timestamp)} · {item.payload.slice(0, 100)}</option
              >
            {/each}
          </select>
        </label>
      ...
      <dl class="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
        ...
        <dd class="min-w-0 wrap-anywhere">{selected.from}</dd>
        <dt class="text-base-content/60">To</dt>
        <dd class="min-w-0 wrap-anywhere">{selected.to}</dd>
      </dl>
      <pre
        class="m-0 whitespace-pre-wrap font-mono text-sm leading-relaxed wrap-anywhere">{selected.payload}</pre>
    </div>
  </div>
