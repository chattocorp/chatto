...
| Client-sync privacy cleanup after account deletion | `client_sync.{userId}.deletion_preparing` exclusively fences the command before commit; post-commit `deletion_pending` identifies retry work; retained `deleted` records committed deletion | The command creates the preparation, commits the account event, creates the retry and retained markers, then purges preferences and the known-server directory | After projection catch-up, every replica scans committed retry markers and promotes preparations whose account event already committed; it revision-purges only abandoned active-account preparations | Recoverable across every request crash boundary; pending is removed after success and deleted is retained |
