...
func TestLogOperatorPolicy(t *testing.T) {
	...
	require.NoError(t, toml.Unmarshal([]byte("[log]\nretention='2d'\n"), &cfg))
	require.Equal(t, 48*time.Hour, cfg.Log.RetentionOrDefault())
	require.NoError(t, cfg.Log.Validate())
	...
	t.Setenv("CHATTO_LOG_RETENTION", "3d")
	loaded, err := ReadConfig("")
	require.NoError(t, err)
	require.Equal(t, 72*time.Hour, loaded.Log.RetentionOrDefault())
}
