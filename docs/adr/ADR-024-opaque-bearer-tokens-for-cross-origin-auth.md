...
- **No reverse index**: account deletion cleanup scans `session.*` and stable `renewable_session.*` authorities and matches the stored user ID. Password changes and resets do not scan, because each scan reads every credential on the server. The revocation guarantee comes from the credential's stored auth generation being compared to the current user auth generation, so concurrent issuance cannot survive by missing either scan. A secondary index can be added later if credential counts make scans too expensive.
