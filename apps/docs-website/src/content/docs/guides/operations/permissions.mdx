...
- **Owners are entitled to every permission.** A user is an owner when they have the `owner` role or a verified email listed in `owners.emails`. They must activate privileged mode before elevation-required permissions become effective.
...
1. Owners are entitled to every permission. Elevation-required permissions also need active privileged mode. DM membership rules still apply, so owners cannot read DMs they are not part of.
...
- **Owners** are always entitled to every permission. You cannot lock them out by changing roles or permission settings. Their permission rows appear as read-only green checks in the UI. They must activate privileged mode to use elevation-required permissions.
