...
describe('BotCredentialsDialog', () => {
  ...
  it('generates a replacement after the previous key was revoked', async () => {
    const updated = bot('2026-07-22T13:00:00.000Z');
    mocks.rotateAPIKey.mockResolvedValue({ bot: updated, apiKey: 'cht_BK-new-secret' });
    const onupdated = vi.fn();
    const { container } = render(BotCredentialsDialog, {
      props: {
        bot: bot(),
    ...
    expect(container.textContent).toContain('Generate API key?');
    expect(container.textContent).not.toContain('Reset API key?');
