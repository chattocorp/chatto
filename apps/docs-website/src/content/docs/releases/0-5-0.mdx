...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="More useful user profiles">
    ...
    the local time that a user chooses to share. Bios can contain up to 1,000 characters. Local
    times use your preferred 12-hour or 24-hour format. Bots use the same self-profile API when
    authenticated with their API key.
  </ReleaseFeatureCard>
...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="Delete accounts from member administration" size="small">
    Admins with `user.delete` can permanently delete another human member from the
  ...
  <ReleaseFeatureCard title="Bots activate through normal notifications" size="large">
    ...
    thread for later activity when notification policy permits it.
  </ReleaseFeatureCard>
...
- Replace every legacy permission identifier in custom RBAC, bots, and clients:
  `message.post-in-thread` becomes `message.post.replies`, `role.assign`
  becomes `role.manage.assignments`, `room.ban-member` becomes
  `room.manage.bans`, `user.manage-accounts` becomes `user.manage`,
  `user.manage-permissions` becomes `user.manage.permissions`,
  `user.delete-any` becomes `user.delete`, `user.delete-self` becomes
  `user.delete.self`, `admin.view-users` becomes `user.read`, and
  `admin.view-audit` becomes `audit.read`. Chatto does not add aliases, rewrite
  stored events, or backfill RBAC decisions. Old decisions stay in event
  history but have no effect. Existing grants for `message.post`,
  `role.manage`, `room.manage`, `user.manage`, and `user.delete` now include
  their registered descendants. Replace every replica and client together,
  then review RBAC. The configured owner is the recovery path.
