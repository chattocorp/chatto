...
| `/api/connect/chatto.api.v1.AdminUserManagementService/SetUserPassword`                      | `AdminUserManagementService`     | `SetUserPassword`               | Authenticated; requires `role.assign`; self-target is rejected.                | Set another user's password as an admin action and revoke their runtime credentials.          |
