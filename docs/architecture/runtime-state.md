...
| `archive.dm.{userId}.{roomId}` | Stable event ID of the latest root message when this viewer archived the DM. The conversation is effectively archived only while the value still equals the room projection's current latest root event ID; a later root message makes the marker stale and restores the sidebar row without another write. Explicit unarchive deletes the marker. One process-wide filtered watcher indexes these keys, supplies revision barriers and bounded compact-reset change fences, and account deletion removes the viewer's keys. |
...
`DMArchiveIndex` independently mirrors `archive.dm.*` through one filtered KV
watcher per process. Startup waits for its initial snapshot, archive/unarchive
writes use KV revision OCC and wait for the observed revision, and a bounded
generation log identifies markers changed during compact realtime snapshot
assembly. Effective archive state compares the indexed marker with the room
projection's current latest root-message event ID; root-message delivery does
not mutate `RUNTIME_STATE`.
