...
Where the toolchain supports it cleanly, locale payloads should be split into separate bundles and loaded lazily. Chatto should not compromise message type-safety or the normal SvelteKit/Vite build in order to force this; if Paraglide's current integration cannot produce separate lazy locale bundles for Chatto's setup, the implementation should document that finding and rely on Paraglide's compile-time tree-shaking until a clean lazy-loading path exists.
...
Message keys should be semantic and stable rather than English-as-key. The default convention is lower snake case grouped by feature or UI surface, such as `settings_preferences_title`, `auth_login_submit`, or `room_event_user_joined`. If Paraglide later provides a stronger Svelte-specific namespacing convention, Chatto may adopt it, but new code must still keep keys stable across wording changes.
