...
vi.mock("@chatto/client", async importOriginal => {
  ...
  return { ...actual, createChattoClient: () => ({
    ...actual.createChattoClient({ serverUrl: "https://chat.example", apiKey: "key",
...
test("source retries failed registration through Runling dispatch and ignores accepted replay", async () => {
  ...
  const start = vi.fn().mockRejectedValueOnce(new Error("Temporary journal failure"))
    .mockResolvedValue({ id: "run" });
  const event = new RealtimeEvent({ id: "message", actorId: "human", event: {
  ...
  await chattoSource({ signal: new AbortController().signal, state: new Map(),
