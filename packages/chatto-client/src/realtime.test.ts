...
function fixture(onEvent = vi.fn(async () => {}), maxPendingFrames?: number, cursor?: string) {
  ...
  const client = createChattoClient({
    serverUrl: 'https://chat.example/path?secret=no',
  ...
  const result = client.consumeRealtime({
  ...
  new RealtimeServerFrame({
    frame: {
      ...
      value: {
        id,
        cursor: id,
        event: { case: 'messagePosted', value: { bodyPlaintext: 'hello' } }
      }
    }
  });
...
test.each([RealtimeCloseCode.AUTHENTICATION_REQUIRED, RealtimeCloseCode.UNSUPPORTED_PROTOCOL])(
  'stops on terminal close %s without leaking the remote message',
  async (code) => {
    const f = fixture();
    f.sockets[0]!.frame(
      new RealtimeServerFrame({
...
test.each([new Uint8Array([255]), new Uint8Array([0x9a, 0x06, 0])])(
  'does not cross undecodable or unknown top-level frames',
  async (bytes) => {
    const f = fixture();
