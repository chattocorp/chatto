...
const cancellation = (error: unknown) => {
  if (error instanceof Error && error.message === 'Cancelled from Chatto')
...
const report = (
  ...
  summary: string,
  details?: string
): AgentResult => ({ outcome, summary, details, usage: emptyTokenUsage() });
...
function setup(reports: AgentResult[], options: { timeout?: number; typing?: ChattoTyping } = {}) {
  ...
  const waitQuestion = (text: string) =>
    vi.waitFor(() => expect(post.mock.calls.some((call) => call[1].includes(text))).toBe(true));
...
test('interviews, revises a plan, and enters the stub only on an exact command', async () => {
  ...
  await vi.waitFor(() =>
    expect(f.post.mock.calls.filter((call) => call[1].includes('Which users'))).toHaveLength(2)
  );
  ...
  for (const [destination] of f.post.mock.calls)
    expect(destination).toEqual({ roomId: 'dm', threadRootId: 'root' });
});
...
test('holds the checkout across input waits and releases it on cancellation', async () => {
  ...
  const other = reloaded({
    directory: process.cwd(),
    post: f.post,
    refresh: f.refresh,
    createPlanner: f.createPlanner
  });
  expect(await other(createWorkflowContext(), delivery('Another feature', 'other'))).toMatchObject({
    status: 'busy'
  });
  expect(f.refresh).toHaveBeenCalledTimes(1);
...
test('fails visibly and releases the checkout when git or planning fails', async () => {
  ...
  await expect(f.bot(createWorkflowContext(), delivery('Feature', 'git-fail'))).rejects.toThrow(
    'dirty checkout'
  );
  expect(f.createPlanner).not.toHaveBeenCalled();
  await expect(f.bot(createWorkflowContext(), delivery('Feature', 'agent-fail'))).rejects.toThrow(
    'Cannot plan'
  );
  expect(f.planner.dispose).toHaveBeenCalledOnce();
...
test('times out an interview, disposes the planner, and permits a new conversation', async () => {
  ...
  await vi.waitFor(() =>
    expect(f.post.mock.calls.some((call) => call[1].includes('Question?'))).toBe(true)
  );
...
test('incorporates busy messages before publishing a plan and never queues approval', async () => {
  ...
  expect(f.planner.runOutcome.mock.calls[1]).toEqual(
    expect.arrayContaining([expect.stringContaining('Include tests')])
  );
...
test('includes feedback received while sending a rejected-approval notice', async () => {
  ...
  expect(f.planner.runOutcome.mock.calls[1]).toEqual(
    expect.arrayContaining([expect.stringContaining('Use the existing API')])
  );
  await f.answer('/implement');
  await run;
});
...
test('posts mid-interaction text immediately and finishes posts before asking a question', async () => {
  ...
  expect(bodies.indexOf('I am still investigating.')).toBeGreaterThan(
    bodies.indexOf('Here is the joke.')
  );
  await f.answer('/implement');
  await run;
});
...
test('reports a failed intermediate post without presenting the final question', async () => {
  ...
  await expect(f.bot(createWorkflowContext(), delivery('Feature', 'root'))).rejects.toThrow(
    'Chatto offline'
  );
