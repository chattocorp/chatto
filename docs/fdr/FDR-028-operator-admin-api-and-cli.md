...
**Decision:** AdminService only runs on the dedicated HTTP listener controlled by `[admin_api]` / `CHATTO_ADMIN_API_*`; it is never mounted on the public web listener.
...
**Decision:** Admin API writes use the system actor rather than impersonating a user or requiring an owner/admin login. The authenticated token name is recorded in the Admin API caller context and emitted in HTTP-edge admin-auth logs, but token secrets are never logged.
