...
test('run failure settles cleanup and keeps the original error', async () => {
  ...
  const child = createWorkflowContext().spawn(() => {
    throw error;
  });
...
const accumulator = task(async (ctx: WorkflowContext<Command, Update>, initial: number) => {
  let total = initial;
...
test('sends data while the task runs, streams updates, then returns its typed result', async () => {
  ...
  await expect(handle.send({ add: 1 })).rejects.toBeInstanceOf(ChannelClosedError);
});
...
test.each([false, true])(
  ...
  async (asynchronous) => {
    ...
    await expect(handle.updates[Symbol.asyncIterator]().next()).rejects.toBe(failure);
  }
);
...
test('supports variadic, optional, synchronous, and bound task functions', async () => {
  ...
  expect(await spawn(createWorkflowContext(), work.bind({ base: 10 }), 2).result).toBe(13);
});
...
test('schema tasks still validate inputs and outputs and retain timeline nesting', async () => {
  ...
  expect(events.filter((e) => e.type === 'step.started').map((e) => e.label)).toEqual([
    'Task',
    'Child'
  ]);
...
test('journals message identities, reads, receipts, and task links without changing values', async () => {
  ...
  await observeRunlingEvents(
    ...
    async () => {
      ...
      await vi.waitFor(() =>
        expect(events.filter((e) => e.type === 'message.receipt')).toHaveLength(3)
      );
      finish();
      const updates = [];
      for await (const update of handle.updates) updates.push(update);
  ...
  expect(events.filter((e) => e.type === 'message.receipt').map((e) => e.consumed)).toEqual([
    true,
    true,
    false
  ]);
  ...
  for (const message of sent) {
    expect(events.findIndex((e) => e === message)).toBeLessThan(
      events.findIndex((e) => e.type === 'message.read' && e.id === message.id)
    );
  }
});
...
test('message previews tolerate circular values and do not record rejected sends', async () => {
  ...
  await observeRunlingEvents(
    ...
    async () => {
      const handle = spawn(
        createWorkflowContext(),
        task(async (_ctx: WorkflowContext<object>) => {
          await new Promise(() => {});
        })
      );
