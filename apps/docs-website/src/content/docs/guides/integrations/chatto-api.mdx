...
Authenticated realtime clients connect to `/api/realtime` and exchange protobuf `chatto.realtime.v1` frames. Protocol v1 is a live-only compatibility mode. Protocol v2 sends one ordered server-scoped projection stream for initial state, bounded replay after reconnect, and later live mutations. Check for `chatto.realtime.projection.v1` in server discovery before requesting protocol v2.
