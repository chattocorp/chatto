...
An amber network icon on a participant card means a poor connection; a red
disconnected icon means the connection is lost. Click the icon to read its status.
Your current-user card shows the same warning for your connection, even outside
the call sidebar. These warnings describe network quality, not microphone volume.

## Adjust another participant's volume

During a call, open the three-dot menu in a participant card's header, or
right-click the card. On touch devices, long-press the card. Adjust
**Voice volume** for their microphone. Open the screen-share card's menu to
adjust **Stream volume** for shared audio.
Microphone volume is also available on camera cards and from that
participant's entry in the members list.
Each control ranges from 0% to 200%; 100% keeps the original level. Higher
levels amplify quiet audio and can distort audio that is already loud.

These settings affect only what you hear. This browser remembers them per
server and participant. **Mute locally** silences both sources and keeps your
levels. Sliders move in 5% steps, with a mark at 100% to help restore the original level.
The volume curve approximates perceived loudness: 50% applies -10 dB and 200% applies
+10 dB. Existing saved positions use this curve; 100% stays unchanged.

Boost requires browser Web Audio support. If boost is unavailable, the controls
show a 100% limit. Browsers without Web Audio output selection use the system
output for calls. Select **Enable call audio** if playback is blocked.
...
- **Audio level visualization** — A soft accent-coloured glow behind participant names grows brighter and broader with speaking volume, then fades away in silence. Screen-share tiles use their own shared audio, not the microphone. Video stays clear. Reduced motion keeps the glow stationary.

## Device preferences and microphone test

If your microphone input stays nearly silent for ten seconds, a warning icon
appears on your user card and your call participant card. Select it for an
explanation. Use the call toolbar's gear to open Voice & video preferences and
check your input device. The hint clears when input sound returns, you mute, change the
microphone, or leave the call. Short pauses do not trigger it. A long pause in
a very quiet room can also show the hint; it does not prove a microphone fault.
...
Changing a device here during a call also switches that call's device.
...
Under **Microphone processing**, **Voice Boosting** enhances your microphone
audio and is enabled by default. Clear the checkbox if it causes audio problems.
Use the microphone test to compare the sound with Voice Boosting on and off.
Your choice is saved for this browser and server.

Use **Noise gate threshold** to suppress quiet background sounds. Set the marker
above the background noise level but below your speech level. Select **Off** to
disable the gate. You can also adjust this threshold from the call's device menu.
