...
export const refreshCheckout = task(async function refreshChattoCheckout(ctx, directory: string) {
  ...
  if ((await realpath(root)) !== (await realpath(directory)))
    throw new Error('CHATTO_WORKING_COPY must be the repository root');
  if ((await git('branch', '--show-current')).stdout !== 'main')
    throw new Error('Chatto working copy must be on main');
  if ((await git('status', '--porcelain')).stdout)
    throw new Error('Chatto working copy must be clean');
