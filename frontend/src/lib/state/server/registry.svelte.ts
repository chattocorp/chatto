...
export interface RegisteredServer {
  ...
  /** Experimental Chatto live WebSocket endpoint advertised by /api/server */
  live?: LiveInfo | null;
...
class ServerRegistry {
  ...
  probeOrigin(knownServer = false): void {
    ...
    if (this.originServer) {
      if (knownServer && !this.originServer.live) {
        void this.#refreshOriginMetadata(this.originServer.id, this.originServer.url);
      }
    ...
    if (knownServer) {
      ...
      void this.#refreshOriginMetadata(id, origin);
      this.originProbed = true;
      return;
    }
  ...
  authenticateOrigin(token: string, user: AuthenticatedUserSummary | null = null): void {
    ...
    if (!origin) {
      ...
      void this.#refreshOriginMetadata(id, originUrl);
      this.originProbed = true;
      return;
    }
    ...
    if (!origin.live) {
      void this.#refreshOriginMetadata(origin.id, origin.url);
    }
    this.originProbed = true;
  }

  async #refreshOriginMetadata(id: string, origin: string): Promise<void> {
    try {
      ...
      if (!server) return;
      const previousLive = server.live ?? null;
      ...
      const nextLive = this.getServer(id)?.live ?? null;
      if (store?.isAuthenticated && !sameLiveInfo(previousLive, nextLive)) {
        eventBusManager.stopBus(id);
        eventBusManager.startBus(id, graphqlClientManager.getClient(id));
      }
    } catch {
      // Origin metadata refresh is best-effort. The existing GraphQL
      // session remains usable if /api/server is temporarily unavailable.
    }
  }
...
function sameLiveInfo(a: LiveInfo | null, b: LiveInfo | null): boolean {
  if (a === b) return true;
  if (!a || !b) return false;
  return a.url === b.url && a.tokenUrl === b.tokenUrl && a.protocol === b.protocol;
}
