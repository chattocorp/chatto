...
| Network MCP | `/mcp` on the public HTTP listener; `GET /.well-known/oauth-protected-resource/mcp` publishes RFC 9728 metadata when `[mcp].enabled = true` | MCP `2026-07-28` stateless Streamable HTTP with the read-only `get_server_info` tool and bounded `list_rooms` tool; static instructions direct hosts to the identity tool, server metadata uses the effective Chatto server name as its display title, and the canonical resource uses the `webserver.url` origin | Resource-bound OAuth bearer with `chatto:rooms:read`, or a current bot API key; every tool call also uses the normal operation authorization model |
