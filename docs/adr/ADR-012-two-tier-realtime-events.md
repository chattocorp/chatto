...
2. **Live-only signals** (typing indicators, presence, notification sync, session/user/config invalidations): Originally published to `live.space.{id}.>` subjects via bare NATS Core pub/sub; currently published as canonical `evtv1.Event` messages under `live.sync.>`. Not stored. Consumed via plain NATS subscriptions.
