...
vi.mock("./implement.ts", async importOriginal => ({
  ...
  implementationExtension: (_ctx: unknown, _settings: unknown, _announce: unknown, _tasks: unknown,
    callbacks: { onStopped: (message: string) => Promise<void> }) =>
    (pi: AgentExtensionAPI) => pi.registerTool({ name: "fakeImplement", execute: async () =>
      callbacks.onStopped("The implementation stopped: unfinished edits. The worktree was kept for review. Please tell me how you want to proceed.") } as never),
}));
...
test("a stopped implementation reaches Chatto even when the supervisor says nothing", async () => {
  ...
  const bot = createChattoBot({
    acknowledge: async () => {}, post, typing: async () => {}, readThread: async () => [], timeout: 0,
    createAgent, implementation: { directory: "/unused", repository: "example/chatto" },
  });
  await bot(createWorkflowContext(), {
    version: 1, id: "delivery", type: "message.created", triggers: ["direct_message"], occurred_at: "now",
    bot_id: "bot", room_id: "room", thread_root_id: null,
    message: { id: "message", author_id: "human", body: "Implement the fix" },
  });
  expect(post).toHaveBeenCalledOnce();
  expect(post).toHaveBeenCalledWith({ roomId: "room", threadRootId: "message" },
    "The implementation stopped: unfinished edits. The worktree was kept for review. Please tell me how you want to proceed.", expect.any(AbortSignal));
});
