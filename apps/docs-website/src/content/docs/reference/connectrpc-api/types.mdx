...
| `is_dm_archived` | `bool` | True when the current user has archived this DM conversation and no newer root message has arrived. Always false for channel rooms. Archiving affects navigation presentation only; membership, access, unread state, and notification policy are unchanged. |
