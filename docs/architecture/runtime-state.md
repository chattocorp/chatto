...
| `notification_v2.read_fence` | Big-endian captured EVT stream sequence written by list and realtime summary paths after the sole durable notification worker acknowledges that boundary. Its KV revision fences the serving replica's process-local occurrence watcher through every earlier occurrence/lifecycle mutation in the same KV stream. |
