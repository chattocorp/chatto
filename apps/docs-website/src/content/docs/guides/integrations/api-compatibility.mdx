...
field: capabilities such as individual RPCs or frontend implementation details
are not a stable, user-facing compatibility model. Replace checks against that
list with an explicit table of the Chatto releases your integration supports.

Regenerate protobuf clients from the final 0.5 schema. The
`minimum_web_client_version` field now uses protobuf tag 1, which the discarded
development schema used for `protocol_capabilities`; mixing generated clients
and servers from opposite sides of that development-only change is not
wire-compatible.
