...
public:
  ...
  [[nodiscard]] FARPROC function(const char *name) const {
    ...
    if (address == nullptr) {
      throw NvencError(std::string("The NVIDIA driver does not export ") + name);
    }
    return address;
  }
...
[[nodiscard]] std::string nvenc_status_message(
    const NVENCSTATUS status, const NV_ENCODE_API_FUNCTION_LIST &functions,
    void *encoder) {
...
public:
  DirectNvencH264Encoder(const std::uint32_t width,
      ...
      : width_(width), height_(height), frames_per_second_(frames_per_second),
        ...
        module_(L"nvEncodeAPI64.dll") {
    if (width == 0 || height == 0 || (width % 2) != 0 ||
        (height % 2) != 0 || frames_per_second == 0 ||
  ...
  DirectNvencH264Encoder &
  operator=(const DirectNvencH264Encoder &) = delete;
  ...
  encode(const std::span<const std::uint8_t> bgra,
    ...
    context_->UpdateSubresource(surface.texture.Get(), 0, nullptr, nv12.data(),
                                width_, static_cast<UINT>(nv12.size()));
    ...
    try {
      ...
      if (result == NV_ENC_ERR_NEED_MORE_INPUT) {
        static_cast<void>(functions_.nvEncUnmapInputResource(
      ...
      if (bitstream_locked) {
        static_cast<void>(functions_.nvEncUnlockBitstream(encoder_,
                                                          surface.bitstream));
      }
  ...
  void set_target_bitrate(
      const std::uint32_t target_bitrate_bps) override {
    ...
    if (encoder_ != nullptr) {
      for (auto &surface : surfaces_) {
        ...
        if (surface.registered != nullptr &&
          ...
          static_cast<void>(functions_.nvEncUnregisterResource(
              encoder_, surface.registered));
