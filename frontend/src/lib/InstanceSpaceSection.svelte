<script lang="ts">
  ...
  async function handleSpaceNotificationClick(spaceId: string) {
    ...
    // mention notifications we route through /m/[messageId], and that
    // resolver sets the pending highlight itself once it has fetched the
    // event and detected thread membership.
