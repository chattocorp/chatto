...
- `vitest-setup-client.ts` imports `$lib/query/client` before each browser
  spec. Thus `vi.mock` of `$lib/query/client` or `@tanstack/svelte-query` in a
  browser spec has no effect. Test against the real `queryClient`: control the
  reads through the mocked API, and let the setup file clear the cache after
  each test.
