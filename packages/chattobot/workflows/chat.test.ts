...
test.each([
  ...
  expect(createAgent).toHaveBeenCalledWith(expect.objectContaining({
    ...
    tools: thread ? ["fetchPage", "investigateChatto", "task_send", "task_cancel"] : ["fetchPage"],
    extensions: thread ? [expect.any(Function), expect.any(Function), expect.any(Function)] : [expect.any(Function)],
    resources: {
      extensions: false,
      skills: false,
...
test("implementation is a separate opt-in tool with host-result reporting instructions", async () => {
  const bot = createChattoBot({ acknowledge: async () => {}, typing: async () => {}, post: async () => {},
    ...
    createAgent: async options => {
      ...
      return { dispose: () => {}, steer: async () => false, async runOutcome() {
...
test("does not post malformed model channel output or its possible reasoning", async () => {
  ...
  const bot = createChattoBot({ acknowledge: async () => {}, typing: async () => {}, post,
    timeout: 0, readThread: async () => [], createAgent: async () => ({
      dispose() {}, steer: async () => false,
  ...
  expect(post).toHaveBeenCalledWith(expect.anything(),
    "I couldn't format that reply correctly. Any background work already started will report its result separately.", expect.any(AbortSignal));
});

test("language context uses human input even when the thread contains a wrong-language bot reply", async () => {
  const bot = createChattoBot({ acknowledge: async () => {}, typing: async () => {}, post: async () => {},
    timeout: 0, readThread: async () => [{ id: "prior", role: "bot", body: "我正在调查" }],
    createAgent: async options => {
      expect(options.instructions?.join("\n")).toContain("Never adopt a language from your own earlier replies");
      return { dispose: () => {}, steer: async () => false,
        async runOutcome(_ctx, prompt) {
          expect(JSON.parse(prompt)).toMatchObject({ origin: "user", currentMessage: "Hello!", recentUserMessages: ["Hello!"] });
          return { outcome: "completed", summary: "", usage: emptyTokenUsage() };
        },
      };
    },
...
test("rapid follow-ups wait for initial context then steer the same turn in order without waiting for receipts", async () => {
  ...
  const bot = createChattoBot({
    acknowledge: async () => {}, post: async () => {}, typing: async () => {},
    readThread, timeout: 0,
  ...
  try {
    ...
    for (const [id, body] of [["second", "Use a short answer"], ["third", "And include links"]]) {
      await bot.route({ start }, { ...delivery, id: id!, thread_root_id: "root",
