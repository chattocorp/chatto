...
- OIDC providers can manage configured direct role claims. An OIDC-managed role is effective alongside manual and other-provider grants; reconciling one provider never revokes another source. An administrator can remove every current source of a member's role as an immediate override, although an enabled provider can grant it again at the next synchronization. Disconnecting the linked identity removes that provider's managed source.
