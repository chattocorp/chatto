...
test('streams input and restores commands and missed messages in original order', async () => {
  ...
  f.agent.steer.mockImplementation(async (text) => (text === 'late' ? late.promise : true));
  f.inbox.push('initial');
  const run = f.run();
  f.inbox.push(' /approve ');
  f.inbox.push('late');
  await vi.waitFor(() => expect(f.agent.steer.mock.calls).toEqual([['initial'], ['late']]));
  f.finish();
  late.resolve(false);
  await run;
...
test('overflow remains in the host backlog and can be delivered on a later turn', async () => {
  ...
  expect(retained).toEqual(Array.from({ length: 35 }, (_, i) => String(i + 65)));
