...
vi.mock('./lib/server/source-host.ts', () => ({
  startSourceHost: vi.fn(),
  stopSourceHost: vi.fn()
}));
...
it('logs unexpected request errors', () => {
  ...
  handleError({
    error,
    event: { request: { method: 'GET' }, route: { id: '/' } } as RequestEvent,
    status: 500,
    message: 'Internal Error'
  });
  expect(serverLog).toHaveBeenCalledWith('error', 'http.error', {
    method: 'GET',
    route: '/',
    status: 500,
    error
