...
function createChattoAgentDemo({
  post,
  typing = async () => {},
  ...settings
...
function fixture(timeout = 0.03) {
  ...
  const createAgent = vi.fn(async () => ({
    ...bot,
    async runOutcome(
      ctx: Parameters<typeof bot.runOutcome>[0],
      prompt: string,
...
test('runs one web-only agent, sends typing and posts one final thread reply', async () => {
  ...
  expect(f.createAgent).toHaveBeenCalledWith(
...
test('reuses the same agent for messages that steering did not consume', async () => {
  ...
  f.bot.runOutcome
    .mockImplementationOnce(() => f.turn.promise)
...
test('concurrent conversations use separate agents and retain their thread destinations', async () => {
  ...
  await workflow.route(
    ...
    delivery('For first', 'reply', 'first')
  );
  ...
  expect(post).toHaveBeenCalledWith(
    { roomId: 'dm', threadRootId: 'first' },
    'First answer',
    expect.any(AbortSignal)
  );
  expect(post).toHaveBeenCalledWith(
    { roomId: 'dm', threadRootId: 'second' },
    'Second answer',
    expect.any(AbortSignal)
  );
...
test('keeps an idle conversation open for the next reply without another question', async () => {
  const f = fixture(900);
  f.bot.runOutcome
    .mockResolvedValueOnce(report('Hey! How can I help?'))
  ...
  expect(f.createAgent).toHaveBeenCalledOnce();
  expect(f.start).not.toHaveBeenCalled();
  expect(f.post.mock.calls.map((call) => call[1])).toEqual([
    'Hey! How can I help?',
    'I am TestBot.'
  ]);
...
test('identical replies in different interactions are still delivered', async () => {
  const f = fixture();
  f.bot.steer.mockResolvedValue(false);
  f.bot.runOutcome.mockResolvedValueOnce(report('Yes.')).mockResolvedValueOnce(report('Yes.'));
  f.post.mockImplementationOnce(async () => {
...
test.each([-1, Infinity, NaN, 2147484])(
  'rejects timeout %s before creating an agent',
  async (timeout) => {
