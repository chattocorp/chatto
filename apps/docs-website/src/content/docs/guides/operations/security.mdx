...
ordinary validation remains read-only. A separate fixed TTL marker drives
...
credential, and a renewable session window (90 days by default). The
...
session window and recreates its TTL marker through the public KV API. Access
token validation does not extend either expiry. Raw access and refresh
