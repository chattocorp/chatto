...
| ------- | ----- | -------- | --------------- |
| Public ConnectRPC | `/api/connect/chatto.{auth,discovery,api,admin}.v1.*` and `/api/connect/chatto.personaldata.api.v1.*` | Unary Connect, gRPC, and gRPC-Web services | Explicit per-service public or authenticated-user policy; method-level authorization remains inside operation models |
...
| `chatto.personaldata.api.v1` | `PersonalDataService` | Authenticated user; resources are always scoped to the caller |
