...
func (s *botService) CreateBot(ctx context.Context, req *connect.Request[apiv1.CreateBotRequest]) (*connect.Response[apiv1.CreateBotResponse], error) {
	...
	bot, apiKey, status, err := s.api.core.CreateBotWithAPIKeyAs(ctx, caller.UserID, req.Msg.GetLogin(), req.Msg.GetDisplayName(), req.Msg.GetDescription())
	if err != nil {
		return nil, connectError(err)
	}
	item := botMutationResponse(bot, status)
...
func (s *botService) RotateBotAPIKey(ctx context.Context, req *connect.Request[apiv1.RotateBotAPIKeyRequest]) (*connect.Response[apiv1.RotateBotAPIKeyResponse], error) {
	...
	// Hydrate all fallible response data before committing the show-once key.
	// After rotation succeeds, response construction must be infallible or the
	// caller could lose the only copy of the newly active secret.
	bot, err := s.manageableBot(ctx, caller.UserID, req.Msg.GetBotId())
	if err != nil {
		return nil, err
	}
	apiKey, status, err := s.api.core.RotateBotAPIKey(ctx, caller.UserID, req.Msg.GetBotId())
	if err != nil {
		return nil, connectError(err)
	}
	item := botMutationResponse(bot, status)
...
// botMutationResponse intentionally performs no I/O. Show-once credential
// mutations must not fail after the server has committed the new secret.
func botMutationResponse(bot *corev1.User, status *core.BotAPIKeyStatus) *apiv1.Bot {
	user := &apiv1.User{
		Id: bot.GetId(), Login: bot.GetLogin(), DisplayName: bot.GetDisplayName(), Deleted: bot.GetDeleted(),
	}
	setAPIUserAccountProfile(user, bot.GetBot())
	item := &apiv1.Bot{User: user, CreatedAt: bot.GetCreatedAt()}
	if status != nil {
		item.ApiKey = &apiv1.BotAPIKey{CreatedAt: timestamppb.New(status.CreatedAt)}
	}
	return item
