...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="A notification inbox you can actually triage">
    Notifications now stay in one chronological list after you read them. Mark an item Done,
    restore it to the inbox, or delete it permanently; per-cause controls distinguish mentions,
...
- Notifications 2.0 replaces the previous notification service and stored records. Existing
  ...
  regenerate their clients and move to the new group, occurrence, and policy methods on
...
- App badges stay synchronized with the authoritative unread notification groups across
