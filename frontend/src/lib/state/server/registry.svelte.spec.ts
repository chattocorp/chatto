...
describe('ServerRegistry', () => {
  ...
  describe('probeOrigin', () => {
    it('refreshes live metadata for known authenticated same-origin instances', async () => {
      const registry = await createRegistry();
      registry.servers = [];
      vi.stubGlobal(
        'fetch',
      ...
      registry.probeOrigin(true);

      const origin = registry.originServer;
      expect(origin).toBeDefined();
      expect(origin?.live).toBeNull();

      await vi.waitFor(() => {
        expect(registry.originServer?.live?.url).toBe('/api/live');
      });
      expect(registry.originServer?.name).toBe('Local Chatto');
      expect(registry.originServer?.iconUrl).toBe('/icon.png');
    });
  });
