...
**Admin API** — Opt-in operator-only ConnectRPC service for user administration and bootstrap automation. It is authenticated by named `[admin_api.tokens]` bearer tokens plus per-token direct-source CIDR allow-lists, not by a user's session or RBAC role. It can run on a dedicated `[admin_api.listener]` instead of the public web listener. See [FDR-028](fdr/FDR-028-operator-admin-api-and-cli.md).
