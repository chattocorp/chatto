...
The Bleve directory contains plaintext-derived message content. Keep it on a protected local volume and exclude it from normal backups. Chatto rebuilds it from retained `EVT` history if it is missing or incompatible. See [Bundled Search Provider](/guides/operations/bundled-search-provider/) for language configuration, standalone deployment, index lifecycle, and troubleshooting.
...
<CardGrid>
  ...
  <LinkCard title="Bundled Search Provider" href="/guides/operations/bundled-search-provider/" description="Configure local message indexing and language analyzers." />
