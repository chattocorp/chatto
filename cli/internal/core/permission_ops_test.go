...
func TestInitServerDefaults(t *testing.T) {
	...
	t.Run("admin has expected server permissions", func(t *testing.T) {
		...
		// and message management. Parent paths do not receive grants from their
		// descendants. Ordinary posting defaults come from everyone.
		for _, perm := range DefaultAdminPermissions() {
			if got := core.rbacModel.decision(ScopeServer, "", RoleAdmin, perm); got != DecisionAllow {
				t.Errorf("admin decision for %s = %s, want %s", perm, got, DecisionAllow)
			}
		}
		for _, perm := range []Permission{PermServer, PermRoom, PermMessage, PermRole, PermAdmin, PermUser, PermBot, PermMessagePost, PermMessagePostInThread, PermMessageReact, PermMessageEcho, PermBotCreate} {
