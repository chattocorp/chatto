...
func TestRealtimeMapperMapsOfflinePresence(t *testing.T) {
	...
	if presence.Status != realtimev1.RealtimePresenceStatus_REALTIME_PRESENCE_STATUS_OFFLINE {
...
func TestRealtimeMapperMapsUnspecifiedNotificationLevelToDefault(t *testing.T) {
	...
	if notificationLevel.Level != appv1.NotificationLevel_NOTIFICATION_LEVEL_DEFAULT {
		t.Fatalf("level = %v, want DEFAULT", notificationLevel.Level)
	}
	if notificationLevel.EffectiveLevel != appv1.NotificationLevel_NOTIFICATION_LEVEL_DEFAULT {
