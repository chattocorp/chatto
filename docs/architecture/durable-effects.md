...
| Video derivative processing | `AssetProcessingStartedEvent`; `AssetProcessingSucceededEvent` or `AssetProcessingFailedEvent` is terminal. Failed events record the derivative IDs created by that attempt; losing terminal attempts use a dedicated cleanup fact; each success attempt is preceded by an exact-event reconciliation guard | A process-local callback starts bounded ffmpeg work, creates temporary MP4 renditions, uploads a thumbnail and HLS segments, then publishes the terminal manifest; animated GIF loops upload one MP4 derivative. Failure publication has an independent bounded context and precedes prompt cleanup | Each enabled video service scans the asset projection at boot for assets without a completed/failed outcome, including `STARTED`-only attempts, and resubmits them. Terminal event OCC prevents later manifest replacement. The elected asset-cleanup worker consumes failed outcomes and losing-generation cleanup requests. A success guard waits through a grace period, retains output if its exact event ID committed, or records failure and cleans the output otherwise. Normal deletion recovery removes bytes idempotently after timeout, restart, or lease handover | Recoverable scheduling, ambiguous success commits, losing generations, and partial-derivative cleanup; an uncommitted derivative creation relies on bounded prompt compensation because no canonical storage fact exists; processing ownership remains process-local, so duplicate work remains possible across replicas |
