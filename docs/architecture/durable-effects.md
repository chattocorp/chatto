...
| Runtime credential cleanup after security changes | Revoking password and account-deletion events advance durable user/auth state. New external-identity disconnects preserve sessions. | Account deletion scans and deletes matching `RUNTIME_STATE` credentials. Password changes and resets do not scan, because the scan reads every credential on the server. All revoking changes publish a pubsub session-termination control | Credential generation prevents stale credentials from authenticating new requests or reconnects; stale records remain cleanup debt, and an already-open realtime connection depends on best-effort session termination | New authentication is durably revoked; physical cleanup and immediate live disconnect are best-effort |
