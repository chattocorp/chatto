...
- **External provider login** — operators configure repeated `[[auth.providers]]` entries or the equivalent `CHATTO_AUTH_PROVIDERS` JSON array. Supported provider types are `openid-connect`, `github`, `gitlab`, `google`, and `discord`. The login page renders buttons from `Server.authProviders`, and provider flows use `/auth/providers/{providerID}` plus `/auth/providers/{providerID}/callback`.
