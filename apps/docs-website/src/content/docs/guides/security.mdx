...
The dedicated Admin API listener is disabled by default. Operators can enable it with `[admin_api]` / `CHATTO_ADMIN_API_*` configuration for trusted automation such as `chatto admin user create` and password/bootstrap workflows. Operator-token requests must use a configured bearer token and originate from one of that token's configured CIDR ranges; by default each token is loopback-only.

Operator tokens are only accepted on the dedicated listener:
...
Chatto serves the same ConnectRPC API surface on the public and dedicated listeners, but the public web listener never accepts `[admin_api]` operator tokens. This is intentional defense in depth: normal public reverse-proxy routing for Chatto's web UI and public APIs cannot accidentally expose token-based bootstrap access. The `chatto admin ...` CLI also prefers the dedicated listener URL from `chatto.toml` or `CHATTO_ADMIN_API_BIND_ADDRESS` / `CHATTO_ADMIN_API_PORT`, so local operator commands keep using the private endpoint automatically.
