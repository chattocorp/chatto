...
describe('Chatto input demo', () => {
  it.each([-1, NaN, Infinity, 2_147_483.648])(
    'rejects invalid timeout %s at creation',
    (timeout) => {
      expect(() => createChattoInputDemo({ post: async () => {}, timeout })).toThrow(RangeError);
    }
  );
  ...
  it('starts from any root DM and resumes only in its thread', async () => {
    ...
    expect(
      await bot(createWorkflowContext(), delivery('wrong person', 'bob', 'wrong-person', thread))
    ).toBe('ignored');
    expect(
      await bot(
        createWorkflowContext(),
        delivery('unrelated thread', 'alice', 'unrelated', 'another-thread')
      )
    ).toBe('ignored');
    ...
    for (const call of post.mock.calls)
      expect(call[0]).toEqual({ roomId: 'dm', threadRootId: thread });
  ...
  it('cleans up when a workflow aborts or posting fails', async () => {
    ...
    expect(
      await bot(createWorkflowContext(), delivery('late answer', 'alice', 'late', 'root-message'))
    ).toBe('ignored');
  });

  it.each([false, true])(
    'reports the input timeout in its thread (first answered: %s)',
    async (firstAnswered) => {
      vi.useFakeTimers();
      try {
        const post = vi.fn<Parameters<typeof createChattoInputDemo>[0]['post']>(async () => {});
        ...
        expect(events.filter((event) => event.type === 'input.finished').at(-1)).toMatchObject({
          status: 'failed',
          reason: 'timeout'
        });
        expect(
          await bot(createWorkflowContext(), delivery('late', 'alice', 'late', 'timeout-root'))
        ).toBe('ignored');
      } finally {
        vi.useRealTimers();
      }
  ...
  it('posts through ConnectRPC without retrying a failed request', async () => {
    ...
    try {
      ...
      await expect(
        post({ roomId: 'room', threadRootId: 'thread' }, 'Question?', new AbortController().signal)
      ).rejects.toThrow('403');
