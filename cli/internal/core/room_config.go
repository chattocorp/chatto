...
func roomConfigLayerProto(layer RoomConfigLayer) *corev1.RoomConfigLayer {
	...
	if layer.AuthorEditWindowSeconds != nil {
		value := *layer.AuthorEditWindowSeconds
...
func (c *ChattoCore) roomConfigLayer(scope RoomConfigScope) RoomConfigLayer {
	...
	return RoomConfigLayer{AuthorEditWindowSeconds: state.authorEditWindowSeconds}
}

func resolveAuthorEditWindow(candidates ...struct {
	scope RoomConfigScope
	value *int32
}) (int32, RoomConfigSource) {
	for _, candidate := range candidates {
		if candidate.value != nil {
			return *candidate.value, RoomConfigSource{Kind: candidate.scope.Kind, ID: candidate.scope.ID}
		}
	}
	return int32(DefaultAuthorEditWindow / time.Second), RoomConfigSource{ProductDefault: true}
}
...
func (c *ChattoCore) effectiveRoomConfigForScope(scope RoomConfigScope) (RoomConfig, RoomConfigSources) {
	...
	if scope.Kind != RoomConfigScopeServer {
		value := c.roomConfigLayer(scope).AuthorEditWindowSeconds
		candidates = append(candidates, struct {
			scope RoomConfigScope
			value *int32
		}{scope, value})
	}
	serverValue := c.roomConfigLayer(server).AuthorEditWindowSeconds
	candidates = append(candidates, struct {
		scope RoomConfigScope
		value *int32
	}{server, serverValue})
	seconds, source := resolveAuthorEditWindow(candidates...)
	return RoomConfig{AuthorEditWindowSeconds: seconds}, RoomConfigSources{AuthorEditWindow: source}
}
...
func (c *ChattoCore) EffectiveRoomConfig(room *corev1.Room) (RoomConfig, RoomConfigSources) {
	...
	if room != nil && KindOfRoom(room) == KindChannel {
		...
		roomValue := c.roomConfigLayer(roomScope).AuthorEditWindowSeconds
		candidates = append(candidates, struct {
			scope RoomConfigScope
			value *int32
		}{roomScope, roomValue})
		if groupID := c.roomModel.roomGroupForRoom(room.GetId()); groupID != "" {
			...
			groupValue := c.roomConfigLayer(groupScope).AuthorEditWindowSeconds
			candidates = append(candidates, struct {
				scope RoomConfigScope
				value *int32
...
func validateRoomConfigUpdate(layer RoomConfigLayer, mask RoomConfigUpdateMask) error {
	...
	if *layer.AuthorEditWindowSeconds < 0 || *layer.AuthorEditWindowSeconds > max {
...
func roomConfigUpdatePaths(mask RoomConfigUpdateMask) []string {
	...
	if mask.AuthorEditWindow {
		paths = append(paths, "author_edit_window_seconds")
	}
	return paths
}
...
func allRoomConfigLayerPaths() []string {
	return []string{"author_edit_window_seconds"}
}

func roomConfigLayerMatchesUpdate(current, requested RoomConfigLayer, mask RoomConfigUpdateMask) bool {
	return !mask.AuthorEditWindow || equalOptionalInt32(current.AuthorEditWindowSeconds, requested.AuthorEditWindowSeconds)
}
