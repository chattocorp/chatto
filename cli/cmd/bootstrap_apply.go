...
func applyBootstrapServer(ctx context.Context, logger *log.Logger, c *core.ChattoCore, inst config.BootstrapServer, ownerID string) bool {
	...
	for _, r := range rooms {
		room, err := c.CreateRoom(ctx, ownerID, "channel", r.Name, r.Description)
		if err != nil {
		...
		if _, err := c.SetRoomAutoJoin(ctx, ownerID, "channel", room.Id, true); err != nil {
			logger.Warn("Failed to set auto_join on [bootstrap] room", "room", r.Name, "error", err)
		}
		if _, err := c.JoinRoom(ctx, ownerID, "channel", ownerID, room.Id); err != nil {
