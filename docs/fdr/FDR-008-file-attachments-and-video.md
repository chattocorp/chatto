...
- Processed video dimensions are display dimensions used for layout, not necessarily raw encoded storage pixels. Non-square-pixel and rotated sources should render in their intended orientation and aspect ratio. The room timeline preserves every posted video's measured aspect ratio, including unusual near-square dimensions and converted animated GIF loops, while bounding the player to the available timeline width and a maximum height.
