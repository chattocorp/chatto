...
**Server Content View** — Process-local, sequence-consistent view of client-readable state derived from `EVT`. It coordinates focused room, timeline, call, asset, thread, reaction, configuration, user-profile, content-key, RBAC, and mentionable components behind one apply barrier. Authentication, presence, read markers, notifications, invitations, OAuth clients, and search remain outside the view. See [ADR-089](adr/ADR-089-server-content-view.md).

**Component snapshot cohort** — Complete set of separately stored projection-component payloads that one manifest binds to the same event-log identity and cutoff. Chatto restores all required components or cold-replays the complete coordinator. See [ADR-088](adr/ADR-088-componentized-projections-behind-one-apply-barrier.md).
