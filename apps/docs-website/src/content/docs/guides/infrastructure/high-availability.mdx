...
<Aside type="caution">
  Replication multiplies storage proportionally. With `replicas=3`, every byte is stored three times across the cluster. Consider using [S3 storage](/guides/infrastructure/s3-storage/) for file attachments to keep replication costs manageable.
</Aside>
...
- **Authentication state** — cookie credential verifiers, short-lived bearer access verifiers, and renewable-session authorities; raw credentials are never stored
...
| All NATS nodes down | Complete outage | Restore cluster or [restore from backup](/guides/operations/backup-restore/) |
...
<CardGrid>
  ...
  <LinkCard title="S3 Storage" href="/guides/infrastructure/s3-storage/" description="Offload file storage to reduce replication costs." />
  <LinkCard title="Backup & Restore" href="/guides/operations/backup-restore/" description="Snapshot and restore your NATS data." />
</CardGrid>
