...
test("shows recorded logs and task milestones in order", () => {
  const events: RunlingEvent[] = [
    ...
    { type: "task.activity", channelId: "channel", message: "Validation failed", level: "error", timestamp: 50 },
    { type: "step.finished", id: "task", status: "failed", durationMs: 1234, timestamp: 60 },
  ];
  expect(runLogRows(events)).toEqual([
    { id: 0, event: { type: "log", level: "info", message: "Started task-1", depth: 0, color: "dodgerblue", source: "step", timestamp: 0 } },
    { id: 1, event: events[1] },
    { id: 3, event: events[3] },
    { id: 5, event: { type: "log", level: "error", message: "task-1 · Validation failed", depth: 0, color: "dodgerblue", source: "step", timestamp: 50 } },
    { id: 6, event: { type: "log", level: "error", message: "task-1 failed · 1 s", depth: 0, color: "dodgerblue", source: "step", timestamp: 60 } },
  ]);
