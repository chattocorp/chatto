...
import { createWorktree, describePullRequest, postReview } from './make-pr.ts';
...
describe('make-pr workflow', () => {
  test('updates the remote default branch and branches the worktree from it', async () => {
    ...
    expect(commands).toEqual([
      {
        template: 'gh repo view --json defaultBranchRef --jq .defaultBranchRef.name',
        values: []
      },
      {
        template: 'git fetch origin +refs/heads/_argument_:refs/remotes/origin/_argument_',
        values: ['master', 'master']
      },
      {
        template: 'git worktree add -b _argument_ _argument_ origin/_argument_',
        values: ['runling/bright-otters-2468', '/worktrees/bright-otters-2468', 'master']
      }
    ]);
  });
  ...
  test('posts a workflow review as a pull request comment', async () => {
    ...
    expect(command?.join('_argument_')).toBe('gh pr comment _argument_ --body _argument_');
