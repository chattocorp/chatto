...
- **Manage members** — `room.manage` holders can list, inspect, add, or remove human members of channel rooms, including when they are not themselves members or eligible to join. Adding can bring a user into a private room even when that user could not self-join through `room.join`. Bot accounts cannot yet be added through this command; explicit bot room approval belongs to a later capability slice. Active room bans still block adding; the user must be unbanned first. DM membership remains visible only to its participants.
