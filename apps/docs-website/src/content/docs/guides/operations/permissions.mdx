...
- **Admins** receive broad default grants, but they are still ordinary non-owner users for permission resolution. A direct-user deny or a deny from another named role can restrict an admin. An `everyone` deny also restricts an admin when it is nearer than the admin's named allow, or when the admin role has no allow for that permission.
