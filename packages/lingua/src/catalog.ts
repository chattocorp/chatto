...
function isPluralObject(value: Record<string, unknown>): value is PluralTranslation {
  const keys = Object.keys(value);
  return (
    keys.length > 0 &&
    keys.every((key) => pluralCategorySet.has(key) && typeof value[key] === 'string')
  );
}
...
function validatePlural(
  ...
  if (typeof value.other !== 'string') {
    throw new CatalogValidationError(`Plural translation "${key}" must define an "other" value`);
  }
...
function validateNode(value: unknown, key: string): void {
  if (typeof value === 'string') {
    if (isCountTranslationKey(key)) {
      throw new CatalogValidationError(`Plural translation "${key}" must be an object`);
    }
    return;
  }
  if (!isObject(value)) {
    throw new CatalogValidationError(`Translation "${key}" must be a string or object`);
  }
  if (isCountTranslationKey(key)) {
...
export function extractSection(loaded: TranslationModule, section: string): TranslationObject {
...
function assertMatchingPlaceholders(base: string, translated: string, key: string): void {
...
function validateOverlayNode(base: unknown, translated: unknown, key: string): void {
...
export function findTranslation(
  ...
  if (typeof current === 'string' || (isObject(current) && isPluralObject(current))) {
    return current as string | PluralTranslation;
  }
  return undefined;
}

export function selectPlural(plural: PluralTranslation, category: PluralCategory): string {
