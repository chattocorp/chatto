...
| Client-sync privacy cleanup after account deletion | `client_sync.{userId}.deletion_pending` exclusively fences the command before commit; retained `client_sync.{userId}.deleted` records committed deletion | The command creates the preparation, commits the account event, creates the retained fence, then purges preferences and the known-server directory | After projection catch-up, every replica scans pending markers, completes confirmed deletions, and revision-purges active-account preparations stale for one hour; a post-write fence check handles racing mutations | Recoverable and idempotent; the pending marker is removed after success, abandoned preparations recover without erasing active data, and the committed fence is retained |
