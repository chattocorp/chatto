...
Authenticated realtime clients connect to `/api/realtime` and exchange protobuf `chatto.realtime.v1` frames. The package name is the protobuf namespace; the only accepted behavioural protocol version is 2. It sends one ordered server-scoped projection stream for initial state, bounded replay after reconnect, and later live mutations. Check for `chatto.realtime.projection.v1` in server discovery before connecting. Resume and timeline cursors are opaque capabilities: clients must store and return them unchanged and must not infer persistence coordinates from them.
