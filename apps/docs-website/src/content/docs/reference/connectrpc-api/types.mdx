...
| Field | Type | Description |
| --- | --- | --- |
| `id` | `string` | Configuration generation, used to cancel earlier pending work. |
...
| `latest_delivery` | [`BotWebhookDelivery`](#chatto-api-v1-BotWebhookDelivery) | Latest recorded failure for this configuration, when available. Later successes and intentional skips do not clear this failure. |

<a id="chatto-api-v1-BotWebhookDelivery"></a>
...
| `status` | [`BotWebhookDeliveryStatus`](#chatto-api-v1-BotWebhookDeliveryStatus) | Final result of this delivery. |
| `reason` | `string` | Safe failure or skip category. Empty for successful delivery. |
| `attempts` | `uint32` | Queue deliveries consumed, up to the attempt limit. A delivery can fail before HTTP starts, so this is not an exact HTTP request count. |
| `http_status` | `uint32` | Zero if no HTTP response was received. |
| `completed_at` | `google.protobuf.Timestamp` | Time the terminal outcome was recorded. |
...
| `oldest_message_age_seconds` | `optional double` | Age in seconds of the oldest retained message at sampling time. Absent when empty. |
| `max_age_seconds` | `optional double` | Configured retention in seconds. Zero means unlimited; absent on older servers. |
...
<a id="chatto-api-v1-BotWebhookDeliveryStatus"></a>
...
| Name | Number | Description |
| --- | --- | --- |
| `BOT_WEBHOOK_DELIVERY_STATUS_UNSPECIFIED` | `0` | No recognised terminal state was supplied. |
| `BOT_WEBHOOK_DELIVERY_STATUS_DELIVERED` | `1` | The endpoint accepted the request with an HTTP 2xx response. |
| `BOT_WEBHOOK_DELIVERY_STATUS_FAILED` | `2` | Attempts or delivery lifetime were exhausted. |
| `BOT_WEBHOOK_DELIVERY_STATUS_SKIPPED` | `3` | Access, message availability, or endpoint settings prevented delivery. |
