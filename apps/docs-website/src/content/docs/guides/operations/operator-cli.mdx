...
For an import, supply both `--source` and `--source-id`:

```sh
chatto operator room create \
  --name "imported-chat" \
  --source discord \
...
Save the returned Chatto room ID. If the command loses its response, repeat it with the same flags. Chatto returns the original room and ID, even after a restart. A change to the name, description, or `--group-id` with the same source key returns a conflict. An omitted group ID and an explicit group ID count as different input. Source keys remain used after room deletion. `--source` is limited to 256 bytes and `--source-id` to 1024 bytes.
