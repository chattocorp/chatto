...
Public user-shaped payloads should reuse the narrowest canonical shape that represents their semantics. `User` is the lightweight identity shape for embeds and include maps. `UserProfile` adds live presence and custom status for member, notification, and call surfaces. Member rows, `ViewerUser`, and `AdminMember` remain separate because they carry membership, self-service, and admin-only fields with different visibility rules. Membership row services should be named by their scope, for example server members versus room members, rather than by the implementation concept of a directory.
