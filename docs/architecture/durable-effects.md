...
| Video derivative processing | `AssetProcessingStartedEvent`; `AssetProcessingSucceededEvent` or `AssetProcessingFailedEvent` is terminal. Failed events record the derivative IDs created by that attempt | A process-local callback starts bounded ffmpeg work, creates temporary MP4 renditions, uploads a thumbnail and HLS segments, then publishes the terminal manifest; animated GIF loops upload one MP4 derivative. Failure publication has an independent bounded context and precedes prompt cleanup | Each enabled video service scans the asset projection at boot for assets without a completed/failed outcome, including `STARTED`-only attempts, and resubmits them. Terminal event OCC prevents later manifest replacement. The elected asset-cleanup worker consumes failed outcomes, while normal deletion recovery removes bytes idempotently after timeout, restart, or lease handover. A losing terminal attempt receives only bounded prompt cleanup. An ambiguous success append is checked by exact event ID; if confirmation also fails, its output is retained | Failed-generation cleanup is recoverable. Processing ownership remains process-local, so duplicate work is possible across replicas; interrupted loser cleanup, an unconfirmed success that did not commit, and an uncommitted derivative creation can leave orphaned storage |
...
| Asset binary and transform-cache deletion | `AssetDeletedEvent` makes projected reads and signed asset resolution reject the asset; the asset ID locates the canonical aggregate's durable creation metadata | Message deletion, attachment removal, account cleanup, pending-upload expiry, and derivative cleanup delete NATS/S3 bytes and cached transforms after recording deletion | The elected `asset_cleanup` worker consumes canonical failed-generation and deletion lanes. It validates failed-generation cleanup IDs against immutable child `asset_created` facts, appends tombstones, then loads storage metadata from those facts and retries idempotent binary/cache deletion. A source-video tombstone also re-reads its durable HLS manifest and tombstones any still-live HLS children, repairing deletion by an older HLS-unaware replica; beta room-scoped facts without a canonical creation aggregate are skipped | Recoverable for canonical message-owned asset deletion facts, durably failed video generations, and mixed-version HLS cleanup; beta room-scoped cleanup and derivatives without a durable cleanup or deletion fact remain best-effort |
...
after ambiguous terminal publication, terminal manifest races, and replayed
