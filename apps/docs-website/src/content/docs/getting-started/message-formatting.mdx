...
Use the toolbar's indent and outdent actions to nest list items. With the cursor in a list, Tab indents the current item and Shift+Tab outdents it in either editor. Tab continues to accept an active mention or emoji suggestion first, and it moves keyboard focus normally outside lists.

## Write and send

Open **Settings → Preferences → Send messages with** and choose:

- **Return** to send with Return. Use `Ctrl+Return` on Windows and Linux or `Cmd+Return` on macOS for the editor's normal Return action.
