...
privileged-mode deadline, and explicit expiry live in the server-side
`session.{hmac}` record. Chatto
...
Administrative and moderation permissions also require explicit privileged
mode on the current human session. The fixed activation lasts for 15 minutes.
It does not change roles or grants. Bot API keys do not use this mode.
...
operator-selected Chatto and LiveKit servers. It also permits HTTP and HTTPS
frames for HTML attachments from registered servers. HTML previews require
consent on each opening and use an iframe with no sandbox permissions and no
referrer. The notice before preview loading explains that external document
resources can reveal the viewer's IP address.
