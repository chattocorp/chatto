...
Chatto uses opaque runtime credentials, not JWTs. Browser sessions use signed HttpOnly cookies for same-origin flows, including direct external-provider login, and provider redirects never contain bearer credentials. An active same-origin browser rotates its session handle before the fixed cookie lifetime ends; validation does not write a new session revision on every request. Registered cross-origin server connections use Chatto's OAuth PKCE flow to obtain a short-lived access token and rotating refresh credential. The client renews the current session window in the background without reopening OAuth or asking the user to extend the session. A client that is inactive for one complete window must sign in again. Password resets, password changes, external identity disconnects, and account deletion revoke older credentials through the user's auth generation.
