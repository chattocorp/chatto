...
| Server/user config | Server Config        | `evt.config.>`, selected user cleanup facts     | `ConfigModel`; server config, branding refs, explicit per-signal-class server/room-group/room notification delivery modes, blocked usernames; group deletion leaves group override maps inert; historical user display-preference and coarse notification-level facts decode but have no projected effect |
