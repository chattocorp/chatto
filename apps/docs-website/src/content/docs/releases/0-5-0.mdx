...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="Version-aware compatibility">
    Public server discovery reports the running server version and optional bundled-client
    requirements. The multi-server client uses explicit release boundaries to show useful
    upgrade guidance.
  </ReleaseFeatureCard>
...
- Server discovery no longer publishes `compatibility.protocol_capabilities`, which appeared
  only in unreleased 0.5 development snapshots. Integrations built against one of those
  snapshots must regenerate from the final 0.5 protobufs and replace capability-list checks
  with an explicit supported-release table. Do not mix clients and servers across this
  development-only schema change: `minimum_web_client_version` now occupies protobuf tag 1.
