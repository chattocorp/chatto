...
import { agent, taskTool, defineAgentExtension } from 'runling/agents';
import type { ChattoAgentFactory, SpecialistUpdate } from './chatto/agent-text.ts';
...
import { createChattoWebhook, postToChatto, type ChattoPost } from './chatto/webhook.ts';
...
export function createChattoCoordinatorDemo({
  ...
  return createChattoWebhook({
    ...
    async run(rootCtx, delivery, destination, inbox) {
      // Check before creating agents or tasks that can access a checkout.
      if (!directory.trim()) {
        throw new Error(
      ...
      async function delegate<T>(child: TaskHandle<string, SpecialistUpdate, T>): Promise<T> {
        children.add(child);
