...
const question = (signal?: AbortSignal): Parameters<InputHandler>[0] => ({
  id: 'question',
  message: 'Question?',
  signal
});
...
test('questions follow queued replies and suppress typing until all questions settle', async () => {
  ...
  const { ctx, say, agentOptions } = createChattoConversation(
    { ...createWorkflowContext(), onInput },
    config
  );
...
test('input failure restores typing and separate conversations remain independent', async () => {
  ...
  const config = options();
  const a = createChattoConversation(
    { ...createWorkflowContext(), onInput: () => failed.promise },
    config
  );
