...
| ------------ | ---------- |
| Same-origin browser app | HttpOnly cookie session. The bundled client persists the direct-login bearer only when its cookie probe cannot authenticate, then removes that fallback after a later successful cookie probe. |
...
`refresh_request_id`. The request ID must contain 16–128 ASCII letters, digits,
underscores, or hyphens (`[A-Za-z0-9_-]`). Persist it before sending and reuse
it until the complete response has been persisted. An exact retry can recover
only the immediately previous refresh generation, and only during the access
token lifetime created by that rotation. A retry outside that boundary is
stale reuse and revokes the renewable session. Serialize refresh attempts. An
`invalid_grant` response requires explicit authentication, while network
failures and `5xx` responses may be retried promptly with the same request ID.
