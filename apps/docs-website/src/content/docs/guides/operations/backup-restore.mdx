...
| [Neighborhood](/guides/operations/neighbors/) directory and images | Regeneratable. The server discovers them again after a restore. |
...
The Docker Compose example mounts its host backup directory at `/backups` in
the Chatto container. The default host directory is `./backups`. You can set
`CHATTO_BACKUP_DIR` in `.env` to select a different directory.

Run a one-off container from the existing `chatto` service. Chatto can continue
to run while this command creates the backup:
...
The default output is a timestamped `.tar.gz.age` file in the host backup
directory. Chatto prompts you to enter and confirm the passphrase. The
passphrase does not appear in the command arguments.
...
backup outside the server. For unattended backups, use the
[`--passphrase-file` or `--passphrase-stdin` options](#encrypted-backups).
...
<Aside type="tip">
  Keep the same `core.secret_key` in `chatto.toml` when restoring if you want existing renewable bearer sessions, cookie-session handles, OAuth codes, and pending account-flow credentials to remain valid. Using a new value is a clean way to invalidate them during disaster recovery.
</Aside>
...
Stop all Chatto replicas, but keep the NATS service running. Then run the
restore in a one-off container:
...
Do not use `docker compose down` for this procedure because it stops NATS. If
you previously scaled Chatto, start it with the required scale, for example
`docker compose up -d --scale chatto=5`.

Chatto detects whether an archive is encrypted and prompts for its passphrase.
An unencrypted archive does not require a passphrase. For an unattended
restore, use `--passphrase-file` or `--passphrase-stdin`.
