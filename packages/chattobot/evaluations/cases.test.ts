...
test("detects the observed language, citation, uncertainty, and stale-update regressions", () => {
  ...
  expect(checkReply("source-evidence", "The handler may allow bubbling (src/Link.ts:12-14). Tests were not run.")).toEqual([]);
...
test("implementation replies need the verified PR link, actual checks, and limitations", () => {
  ...
  expect(checkReply("implementation-completed", "[PR](https://github.com/example/chatto/pull/42) fixes the menu. pnpm test menu passed. Browser behavior was not checked.")).toEqual([]);
  expect(checkReply("publication-unknown", "Done: https://github.com/example/chatto/pull/99")).toHaveLength(2);
  expect(checkReply("publication-unknown", "Publication could not be verified; a PR may already exist.")).toEqual([]);
});

test("missing evidence is a stopped report handoff, not an ongoing search", () => {
  expect(checkReply("missing-evidence", "The worker did not submit checked findings. The investigation has stopped.")).toEqual([]);
  expect(checkReply("missing-evidence", "I couldn't find the code. I'm still working and will try again.")).not.toEqual([]);
});
