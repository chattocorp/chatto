...
test('agent supervision observes an ordinary run without replacing its identity or typed result', async () => {
  ...
  expect(notice).toMatchObject({
    type: 'task.completed',
    task: { id: run.id, state: { phase: 'done' }, result: '{"answer":"Steering"}' }
  });
  ...
  await observeRunlingEvents(
    ...
    async () => {
      ...
      const run = ctx.spawn(async (ctx: WorkflowContext<string, AgentTaskUpdate>) => {
        await ctx.emit({
          type: 'state',
          value: { phase: 'validating', private: 'secret' },
          activity: 'Validating change',
      ...
      try {
        await vi.waitFor(() =>
          expect(events.some((event) => event.type === 'task.activity')).toBe(true)
        );
        expect(events.filter((event) => event.type === 'task.activity')).toEqual([
          expect.objectContaining({
            channelId: run.id,
            message: 'Validating change',
            level: 'success'
          })
        ]);
        expect(
          JSON.stringify(events.filter((event) => event.type === 'task.activity'))
  ...
  await observeRunlingEvents(
    ...
    async () => {
      ...
      try {
        ...
        expect(events.find((event) => event.type === 'task.activity')).toMatchObject({
...
test('cancelling an observed run directly reports cancellation and awaits its cleanup', async () => {
  ...
  const run = ctx.spawn(async (child: WorkflowContext<string, AgentTaskUpdate>) => {
    try {
      await child.inbox[Symbol.asyncIterator]().next();
    } finally {
  ...
  const notice = reader.next().then((value) => {
    notified();
    return value;
  });
  run.cancel();
...
test('task context keeps bounded output and fresh state without waking the owner', async () => {
  ...
  const handle = tasks.start(
    ...
    async (ctx) => {
      ...
      for (let i = 0; i < 20; i++)
        await ctx.emit({ type: 'output', text: `${i}:` + 'x'.repeat(5000) });
      await ctx.emit({
        type: 'state',
        value: { phase: 'validating', completed: ['check'], pending: ['test'] }
      });
      ready.resolve();
  ...
  try {
    ...
    expect(copy.output[0]).toMatchObject({
      sequence: 5,
      kind: 'output',
      text: expect.stringMatching(/^4:/)
    });
...
test('provider trouble bypasses progress throttle and snapshots retain the age of findings', async () => {
  ...
  try {
    ...
    expect(JSON.parse((await reader.next()).value!)).toMatchObject({
      type: 'task.retrying',
      task: {
        progress: 'Found the relevant handler',
...
test('returns handles immediately, forwards input, and reports completion', async () => {
  ...
  expect(event).toMatchObject({
    type: 'task.completed',
    task: { id: handle.id, result: 'Received: A clarification' }
  });
...
test('tool failures supersede stale prose and stop the child after the configured limit', async () => {
  ...
  try {
    ...
    expect(first).toMatchObject({
      ...
      task: { activity: { operation: 'edit', phase: 'failed' } }
    });
    ...
    expect(JSON.parse((await reader.next()).value!)).toMatchObject({
      type: 'task.failed',
      task: { failureReason: 'tool_failure_limit' }
    });
...
test('provider recovery and repeated retries do not wake the owner with stale findings', async () => {
  ...
  try {
    ...
    await vi.waitFor(() => expect(tasks.get(handle.id).provider).toEqual({ type: 'working' }), {
      interval: 1
    });
...
test('explicit findings survive tool activity and are announced only once', async () => {
  ...
  try {
    ...
    expect(JSON.parse((await reader.next()).value!)).toMatchObject({
      type: 'task.progress',
      progress: 'Checked source evidence'
    });
    nextTool.resolve();
    await vi.advanceTimersByTimeAsync(100);
...
test('disposal waits for cooperative cleanup and cancels all tasks', async () => {
  ...
  let closed = false;
  const closing = tasks.dispose().then(() => {
    closed = true;
  });
