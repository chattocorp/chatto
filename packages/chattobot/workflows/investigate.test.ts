...
vi.mock("runling/agents", async importOriginal => ({
  ...await importOriginal<typeof import("runling/agents")>(), agent: mocks.agent,
}));
...
async function fixture() {
  ...
  await exec("git", ["-c", "user.name=Test", "-c", "user.email=test@example.invalid", "-c", "commit.gpgsign=false", "commit", "-m", "fixture"], { cwd: directory });
...
test("parallel investigations use detached worktrees and expose only read-only tools", async () => {
  ...
  const investigate = createInvestigation(settings, async options => {
    ...
    await factory({ registerTool(tool: { execute: typeof record }) { record = tool.execute; } } as unknown as AgentExtensionAPI);
    return { dispose, async runOutcome() {
      await record("evidence", { claim: "The file contains the original value.", kind: "observation", evidence: [{ path: "example.txt", startLine: 1, endLine: 1, quote: "original" }] });
      return { outcome: "completed", summary: "Inspected", details: "Evidence: example.txt:1. Tests not run: read-only investigation.", usage: emptyTokenUsage() };
    } };
  });
  const results = await Promise.all(["first", "second"].map(question => investigate(createWorkflowContext(), { question })));
  expect(new Set(paths).size).toBe(2);
  expect(dispose).toHaveBeenCalledTimes(2);
  for (const result of results) {
    expect(result.outcome).toBe("completed");
    expect(result.findings).toHaveLength(1);
    expect(result.validation).toEqual({ citationsChecked: true, reproduced: false, testsRun: false, changesApplied: false });
    ...
    expect((await exec("git", ["rev-parse", "--abbrev-ref", "HEAD"], { cwd: result.worktree })).stdout.trim()).toBe("HEAD");
  }
  expect(await readFile(join(settings.directory, "example.txt"), "utf8")).toBe("host dirty change\n");
});

test.each(["cancel", "timeout", "failure"])("disposes the agent and retains evidence on %s", async mode => {
  const settings = await fixture();
  const controller = new AbortController();
  const dispose = vi.fn();
  const investigate = createInvestigation({ ...settings, timeoutMs: mode === "timeout" ? 200 : 10_000 }, async () => ({
    dispose, async runOutcome(ctx) {
      if (mode === "failure") throw new Error("Model failed");
      if (mode === "cancel") controller.abort(new Error("Cancelled"));
      ctx.signal.throwIfAborted();
      return new Promise((_resolve, reject) => ctx.signal.addEventListener("abort", () => reject(ctx.signal.reason), { once: true }));
    },
  }));
  await expect(investigate({ ...createWorkflowContext(), signal: controller.signal }, { question: "Investigate" })).rejects.toThrow();
  expect(dispose).toHaveBeenCalledOnce();
  const [folder] = await readdir(settings.artifactsDirectory);
  expect(await readFile(join(settings.artifactsDirectory, folder!, "changes.patch"), "utf8")).toBe("");
});

test("invalid refs fail before creating an agent", async () => {
  const settings = await fixture();
  const factory = vi.fn();
  await expect(createInvestigation({ ...settings, baseRef: "--help" }, factory)(createWorkflowContext(), { question: "test" })).rejects.toThrow();
...
test("repairs the loud-waves handoff in the same worker before the owner receives completion", async () => {
  ...
  const createAgent = vi.fn(async (options: AgentOptions) => {
    ...
    return { dispose, async runOutcome(_ctx: unknown, prompt: string) {
      prompts.push(prompt);
      expect(await readFile(join(options.cwd, "example.txt"), "utf8")).toBe("original\n");
      if (prompts.length === 2) await record("repaired", { claim: "The fixture contains the original value.", kind: "observation", evidence: [{ path: "example.txt", startLine: 1, endLine: 1, quote: "original" }] });
      return { outcome: "completed" as const, summary: "I found the answer", details: "Unvalidated prose", usage: emptyTokenUsage() };
    } };
  });
  try {
    const investigate = createInvestigation(settings, createAgent);
    tasks.start("Investigate", async ctx => JSON.stringify(await investigate(ctx, { question: "What does the fixture contain?" })), undefined);
    ...
    expect(result).toMatchObject({ outcome: "completed", findings: [{ evidence: [{ path: "example.txt", startLine: 1 }] }] });
...
test.each([
  ...
  { outcome: "failed" as const, reason: "missing_outcome" as const, expected: "missing_outcome", calls: 2 },
  { outcome: "failed" as const, reason: "provider_error" as const, expected: "provider_error", calls: 1 },
  { outcome: "blocked" as const, reason: undefined, expected: "worker_blocked", calls: 1 },
])("reports $expected without an unbounded repair loop", async ({ outcome, reason, expected, calls }) => {
  const settings = await fixture();
  const runOutcome = vi.fn(async () => ({ outcome, failureReason: reason, summary: "Worker result", usage: emptyTokenUsage() }));
  const result = await createInvestigation(settings, async () => ({ dispose: () => {}, runOutcome }))(createWorkflowContext(), { question: "Inspect" });
...
test("cancellation during report repair disposes the same worker", async () => {
  ...
  const dispose = vi.fn();
  let turns = 0;
  await expect(createInvestigation(settings, async () => ({ dispose, async runOutcome() {
    if (++turns === 2) controller.abort();
    return { outcome: "completed", summary: "No evidence", usage: emptyTokenUsage() };
  } }))({ ...createWorkflowContext(), signal: controller.signal }, { question: "Inspect" })).rejects.toThrow();
  expect(turns).toBe(2);
  expect(dispose).toHaveBeenCalledOnce();
});

// Opt-in model evaluation of real file tools, citations, completion, and owner reply.
test.skipIf(!process.env.CHATTO_EVAL_MODEL)("live worker hands checked evidence to the owner", async () => {
  const settings = await fixture();
  const real = await vi.importActual<typeof import("runling/agents")>("runling/agents");
  const model = process.env.CHATTO_EVAL_MODEL!;
  const ctx = createWorkflowContext();
  const result = await createInvestigation({ ...settings, model, timeoutMs: 90_000 }, options => real.agent({ ...options,
    onEvent: event => {
      ...
      if (event.type === "tool_execution_end" && event.toolName === "recordFinding" && event.isError) console.info("Synthetic citation validation:", JSON.stringify(event.result));
    },
  }))(ctx, {
  ...
  let reply = "";
  try {
    await owner.runOutcome(ctx, JSON.stringify({ origin: "notification", recentUserMessages: ["What does example.txt contain?"],
      backgroundTasks: [{ status: "completed", result }], notification: { type: "task.completed" } }),
    { signal: AbortSignal.timeout(60_000), onText: text => { reply += text; } });
...
test("background investigations retain findings without chat notifications and accept supervisor steering", async () => {
  ...
  const investigate = createInvestigation(settings, async options => {
    ...
    async runOutcome(_ctx, _prompt, options) {
      ...
      await record("finding", { claim: "The fixture contains the original value.", kind: "observation", evidence: [{ path: "example.txt", startLine: 1, endLine: 1, quote: "original" }] });
      started.resolve();
  ...
  try {
    ...
    expect(JSON.parse((await reader.next()).value!)).toMatchObject({ type: "task.tool_failed", task: {
...
test("the registered tool runs a child workflow and returns its evidence", async () => {
  const settings = await fixture();
  const dispose = vi.fn();
  mocks.agent.mockResolvedValue({ dispose, runOutcome: async () => ({ outcome: "completed",
    summary: "Checked the report", details: "Evidence: example.txt:1", usage: emptyTokenUsage() }) });
  let execute!: (id: string, input: { question: string; announcement: string }, signal?: AbortSignal) => Promise<{ content: Array<{ text: string }> }>;
  let release!: () => void;
  ...
  const extension = investigationExtension({ ...createWorkflowContext(),
    inbox: (async function* () { yield "unused follow-up"; })(), emit: async (_text: string) => {},
  }, settings, announce, tasks);
  const factory = typeof extension === "function" ? extension : extension.factory;
  await factory({ registerTool(tool: { name: string; execute: typeof execute }) {
    expect(tool.name).toBe("investigateChatto");
    execute = tool.execute;
  } } as unknown as AgentExtensionAPI);
  const pending = execute("call", { question: "Is this a bug?", announcement: "I'll trace the notification code." });
  await vi.waitFor(() => expect(announce).toHaveBeenCalledOnce());
  expect(announce).toHaveBeenCalledWith("I'll trace the notification code.", expect.any(AbortSignal));
  ...
  expect(JSON.parse(tasks.get(handle.id).result!)).toMatchObject({ outcome: "blocked", findings: [], validation: { citationsChecked: false, testsRun: false, changesApplied: false } });
  expect(tasks.get(handle.id).result).not.toContain("Evidence: example.txt:1");
  expect(dispose).toHaveBeenCalledOnce();
  const aborted = new AbortController();
  aborted.abort(new Error("cancel tool"));
  await expect(execute("cancelled", { question: "test", announcement: "Checking." }, aborted.signal)).rejects.toThrow("cancel tool");
  announce.mockRejectedValueOnce(new Error("Posting failed"));
  await expect(execute("failed", { question: "test", announcement: "Checking." })).rejects.toThrow("Posting failed");
  await expect(execute("blank", { question: "test", announcement: "   " })).rejects.toThrow("announcement is required");
