<script lang="ts">
  ...
  async function selectRun(id: string) {
    if (
      ...
      !detailError &&
      (loadingRun === id ||
        (detail?.id === id && (connection === 'Live' || connection === 'Saved')))
    )
      return;
    detailRequest?.abort();
    runStream?.close();
    ...
    try {
      ...
      if (detail) connection = 'Could not refresh. Showing cached run.';
        ...
        detailError = request.signal.aborted
    ...
    stream.onopen = () => {
      if (runStream === stream) connection = 'Live';
    };
    stream.onerror = () => {
      if (runStream === stream) connection = 'Connection lost. Retrying…';
    };
  }
  ...
  onMount(() => {
    try {
      const saved =
        localStorage.getItem('runling-runs-sidebar') ?? localStorage.getItem('runling-sidebars');
...
<div class="flex h-dvh min-h-0 flex-col overflow-hidden bg-base-100 text-base-content">
  {#if configError}
    <div class="alert alert-error shrink-0 rounded-none" role="alert">
  ...
  <header class="navbar shrink-0 gap-3 border-b border-base-300 bg-base-200 px-3 sm:px-4">
    <div class="hidden md:block">
      <SidebarToggle expanded={runsExpanded} onclick={toggleRuns} />
    </div>
    <a
      href="/"
    ...
    <div class="ml-auto flex items-center gap-2 sm:gap-3">
      ...
      <button
        ...
        onclick={() => (composer = true)}>+ New run</button
      >
    </div>
  </header>
  ...
  <div
    class={[
      ...
      runsExpanded ? 'md:grid-cols-[var(--runs-width)_minmax(0,1fr)]' : 'md:grid-cols-1'
    ]}
  ...
  >
    ...
    >
      <div class="shrink-0 border-b border-base-300 px-3 py-3">
        ...
        <div class="flex items-center gap-1">
          ...
          <select
            id="desktop-webhook-filter"
            class="select select-sm min-w-0 flex-1"
            bind:value={filter}
          >
            <option value="">All webhooks</option>
            {#each data.webhooks as webhook (webhook.name)}<option value={webhook.name}
                >{webhook.name}</option
              >{/each}
          </select>
          {#if selectedHook}
            <button
              ...
              onclick={() => (hookInfo = selectedHook)}
              aria-label={`Information about ${selectedHook.name}`}
      ...
      <div class="min-h-0 flex-1 overflow-y-auto">
        <ul class="m-0 list-none p-1.5">
          {#each visibleRuns as run (run.id)}
            <li>
              ...
              >
                ...
                <span
                  class="mt-1 flex min-w-0 items-center justify-between gap-2 pl-1 text-xs text-base-content/50"
                >
          ...
          {:else}
            <li class="px-3 py-5 text-sm text-base-content/60">
              {filter
                ? 'No runs for this webhook.'
                : 'No runs yet. Start a run to see its log here.'}
            </li>
          {/each}
        </ul>
      </div>
      <ColumnResizer
        bind:width={runsWidth}
        storageKey="runling-width-runs"
        label="Runs column width"
        minWidth={180}
    ...
    <div
      class="flex shrink-0 items-center gap-2 border-b border-base-300 bg-base-200 px-3 py-2 md:hidden"
    >
      ...
      <select
        id="mobile-run-picker"
        class="select select-sm min-w-0 flex-1"
        value={visibleRuns.some((run) => run.id === selected) ? selected : ''}
        onchange={(event) => {
          if (event.currentTarget.value) void selectRun(event.currentTarget.value);
        }}
      >
        <option value="">Select a run</option>
        {#each visibleRuns as run (run.id)}<option value={run.id}
            >{run.workflow} · {run.status} · {run.reference ?? run.id}</option
          >{/each}
      </select>
      {#if selectedHook}
        <button
        ...
        <section class="flex h-full min-h-0 flex-col" aria-label="Run details">
          <header class="shrink-0 border-b border-base-300 px-4 py-4 sm:px-6">
            ...
            <p class="mt-1 text-xs text-base-content/60">
              {selectedSummary?.reference ?? selected}
            </p>
          </header>
          <div class="flex min-h-0 flex-1">
            <div
              class="flex min-w-0 flex-1 flex-col items-center justify-center gap-3 p-8 text-center"
            >
              {#if !detailError}<span class="loading loading-spinner loading-md text-primary"
                ></span>{/if}
              ...
              {#if detailError}<button class="btn btn-sm" onclick={() => selectRun(selected)}
                  >Retry</button
                >{/if}
            </div>
            <aside
              class="hidden w-[40%] min-w-[25rem] max-w-[48rem] flex-col border-l border-base-300 2xl:flex"
              aria-label="Details pane"
            >
              <h2 class="border-b border-base-300 px-5 py-4 text-lg font-medium">Run details</h2>
              <p class="px-5 py-5 text-sm text-base-content/60">
                {detailError ? 'Details are unavailable.' : 'Loading details…'}
              </p>
            </aside>
          </div>
        </section>
        ...
        <div class="hero h-full p-8">
          <div class="hero-content flex-col text-center">
            ...
            <p class="max-w-sm text-base-content/60">
              Select a run to read its log, or start a new run.
            </p>
            <button
              class="btn btn-primary"
              disabled={!data.webhooks.length}
              onclick={() => (composer = true)}>Start a run</button
            >
          </div>
        </div>
      {/if}
