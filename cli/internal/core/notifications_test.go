...
func TestGetNotificationCounts(t *testing.T) {
	core, _ := setupTestCore(t)
	ctx := context.Background()

	userID := "count-by-room-user"

	t.Run("returns empty counts when no notifications", func(t *testing.T) {
		counts, err := core.GetNotificationCounts(ctx, userID)
		if err != nil {
			t.Fatalf("GetNotificationCounts error: %v", err)
		}
		if counts.Total != 0 {
			t.Errorf("Expected total 0, got %d", counts.Total)
		}
		if len(counts.ByRoom) != 0 {
			t.Errorf("Expected no room counts, got %v", counts.ByRoom)
		}
	})

	t.Run("groups all notification payload types by room", func(t *testing.T) {
		if _, err := core.CreateNotification(ctx, userID, "actor", &corev1.Notification{
			Notification: &corev1.Notification_DmMessage{
				DmMessage: &corev1.DMMessageNotification{RoomId: "dm-room"},
			},
		}); err != nil {
			t.Fatalf("CreateNotification DM error: %v", err)
		}
		if _, err := core.CreateNotification(ctx, userID, "actor", &corev1.Notification{
			Notification: &corev1.Notification_Mention{
				Mention: &corev1.MentionNotification{RoomId: "channel-room", EventId: "mention-event"},
			},
		}); err != nil {
			t.Fatalf("CreateNotification mention error: %v", err)
		}
		if _, err := core.CreateNotification(ctx, userID, "actor", &corev1.Notification{
			Notification: &corev1.Notification_Reply{
				Reply: &corev1.ReplyNotification{
			...
			t.Fatalf("CreateNotification reply error: %v", err)
		}
		if _, err := core.CreateNotification(ctx, userID, "actor", &corev1.Notification{
			Notification: &corev1.Notification_RoomMessage{
				RoomMessage: &corev1.RoomMessageNotification{
					RoomId:  "channel-room",
					EventId: "room-message-event",
				},
			},
		}); err != nil {
			t.Fatalf("CreateNotification room message error: %v", err)
		}
		...
		if got := counts.ByRoom["channel-room"]; got != 3 {
			t.Errorf("Expected channel-room count 3, got %d", got)
		}
		if got := counts.ByRoom["dm-room"]; got != 1 {
			t.Errorf("Expected dm-room count 1, got %d", got)
		}
	})

	t.Run("does not include other users notifications", func(t *testing.T) {
		if _, err := core.CreateNotification(ctx, "other-count-user", "actor", &corev1.Notification{
			Notification: &corev1.Notification_DmMessage{
				DmMessage: &corev1.DMMessageNotification{RoomId: "channel-room"},
			},
		}); err != nil {
			t.Fatalf("CreateNotification other user error: %v", err)
		}
		...
		if counts.Total != 4 {
			t.Errorf("Expected total 4 after other user notification, got %d", counts.Total)
		}
	})
}
