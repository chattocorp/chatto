...
- **Experimental NATS-backed projection snapshots** — opaque encrypted accelerators when `experimental.projection_snapshots` is enabled
...
| S3-backed assets and experimental snapshots | Stored outside NATS; protect them through the deployment's S3 backup policy |
