...
test('maps Electron locale names to supported frontend locales', () => {
  ...
  assert.equal(isElectronLocaleSupported('de_FEMININE.lproj', supportedLocales), true);
  assert.equal(isElectronLocaleSupported('en-US.pak', supportedLocales), true);
  assert.equal(isElectronLocaleSupported('es_419.lproj', supportedLocales), true);
  assert.equal(isElectronLocaleSupported('zh_TW_NEUTER.lproj', supportedLocales), true);
...
test('prunes unsupported macOS locale bundles', async (t) => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), 'chatto-desktop-locales-'));
  ...
  const result = await pruneElectronLocales(temporaryRoot, 'darwin', supportedLocales);
...
test('prunes unsupported Windows and Linux locale packs', async (t) => {
  ...
  await Promise.all(
    ['de.pak', 'en-US.pak', 'fr.pak'].map((locale) => writeFile(path.join(locales, locale), locale))
  );

  const result = await pruneElectronLocales(temporaryRoot, 'linux', supportedLocales);
