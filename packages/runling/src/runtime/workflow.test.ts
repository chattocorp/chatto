...
describe('task', () => {
  ...
  test('nested tasks preserve inputs and activity parentage', async () => {
    const events: RunlingEvent[] = [];
    const child = task(function child(ctx, value: string) {
      return `${value} child`;
    });
    const parent = task(async function parent(ctx, value: string) {
      return child(ctx, value);
    });
  ...
  test('validates sync and async outputs', async () => {
    ...
    await expect(asyncTask(createWorkflowContext(), 'input')).rejects.toThrow(
      'Task "Broken" output is invalid'
    );
  });
  ...
  test('preserves thrown errors and records failure', async () => {
    ...
    expect(events.filter((e) => e.type === 'step.finished').map((e) => e.status)).toEqual([
      'failed',
      'failed'
    ]);
  });
  ...
  test('preserves null input', () => {
    const run = task(
      { name: 'Null', input: Type.Null(), output: Type.Null() },
      (ctx, value) => value
    );
