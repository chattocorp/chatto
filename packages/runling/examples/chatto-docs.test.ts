...
describe('chatto-docs workflow', () => {
  test('answers the CLI question using only the web tool', async () => {
    ...
    await expect(
      chattoDocs(createWorkflowContext(), {
    ...
    expect(steps).toEqual(['Consulting Chatto documentation']);
    expect(prompts[0]).toContain('What is Chatto?');
    expect(prompts[0]).toContain('https://docs.chatto.run/getting-started/introduction/');
