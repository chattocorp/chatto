...
Room-relevant permissions such as `message.read`, `message.read.interactions`, `message.post`, `message.attach`, `message.react`, `message.manage`, `room.join`, and `room.manage.bans` can be configured at room/group/server scope. Server-only permissions such as `server.manage`, `role.manage`, and `audit.read` are configured only at server scope.

Permission identifiers contain two or more dot-separated components. Each
registered prefix includes its registered descendants. For example,
`message.read` includes `message.read.interactions`, and `room.manage` includes
`room.manage.bans`. Inclusion works only from parent to child. A child grant
does not grant its parent. A child deny cannot restrict an effective parent
allow, and a parent deny cannot restrict a separate child allow.

## Important permissions

| Permission                | Meaning                                                                                                                                                                         |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `message.read`            | Read channel-room message and thread content, attachments, message search results, typing indicators, and message-derived activity. It includes `message.read.interactions`. Channel-room membership is also required. DM membership authorizes DM reads. Fresh servers allow this for `everyone`; existing servers are never backfilled. |
| `message.read.interactions` | Read a complete channel-room thread when the account authored its root or another account directly mentioned it in that thread. This includes messages from before the relationship started. Role mentions, broadcast mentions, self-mentions, and authored replies do not create access. Channel-room membership is also required. Fresh servers get this authority through the `message.read` grant; existing servers are never backfilled. |
| `message.post`            | Post root messages and replies. A human with this permission can also start DMs. A bot cannot start a DM. It includes `message.post.replies`. |
| `message.post.replies`    | Post messages inside channel-room threads and, together with `message.post`, explicitly establish a root as a thread. This grant does not allow root posts. |
...
| `room.manage`             | Configure, rename, archive, or delete rooms and room groups at the effective scope, including explicit channel-room membership and bans. A server-scope grant also permits global room-group creation and ordering. It includes `room.manage.bans`. |
| `room.manage.bans`        | Ban members from channel rooms without granting the other room-management capabilities. |
| `role.manage`             | Configure role definitions and role permission decisions, and assign roles. It includes `role.manage.assignments`. |
| `role.manage.assignments` | Assign or revoke roles for users without granting the other role-management capabilities. Non-owners can only manage roles whose explicit scoped decisions are within their own authority; the `owner` role is owner-only. |
| `user.invite`             | List, create, copy, and revoke invite links. This exposes every active invite-link capability to the holder.                                                              |
| `user.manage`             | Create users, edit account identity, reset passwords, attach verified emails, clear login cooldowns, and edit direct permissions. It includes `user.manage.permissions`. |
| `user.manage.permissions` | Edit direct per-user permission overrides without granting other user-management capabilities. |
| `user.delete`             | Delete any account, including your own. It includes `user.delete.self`. |
| `user.delete.self`        | Delete only your own account. |
...
<Aside type="caution" title="Existing servers need explicit read grants">
  ...
  that must keep channel-room message access. Grant it to `everyone` at server
  ...
  state. These permissions do not control DM reads.
</Aside>
...
`message.post.replies` still decide whether the user may post at all. Edits,
...
If you also want to prevent thread replies or file uploads in announcements, deny `message.post.replies` or `message.attach` for `everyone` in that room.
...
3. Deny `message.post.replies` too if replies must stop. Deny
   `message.attach` and `message.react` if those actions must also stop.
...
4. Deny `message.post.replies` too if replies must stop. Deny
   `message.attach`, `message.react`, and `message.echo` if those actions must
   also stop.
...
DMs use the same permission names where they make sense, but private conversations have extra protection. For non-owners, Chatto always blocks channel management actions in DMs, including `message.manage`, `message.echo`, `room.manage`, `room.manage.bans`, `room.create`, and `message.post.replies`. Role grants cannot enable them.

Each participant can read the complete DM. `message.read` and
`message.read.interactions` grants and denials do not change DM access. Owners
cannot read or moderate a DM that they are not part of.
