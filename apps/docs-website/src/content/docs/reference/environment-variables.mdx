...
<EnvVar name="CHATTO_CORE_PROJECTION_SNAPSHOTS" toml="core.projection_snapshots" default="false">
  Persist encrypted projection snapshots and replay only the later `EVT` delta for eligible projections at startup. Chatto treats `EVT` as authoritative and automatically replays it when a snapshot is missing, incompatible, corrupt, or unavailable. User account/authentication state still cold-replays from its focused event subjects because it contains sensitive credential material that is not snapshot-eligible. While enabled, one elected replica periodically removes unreferenced snapshot objects after a 24-hour grace period. S3 credentials therefore require permission to list the configured bucket prefix in addition to reading, writing, and deleting objects.
</EnvVar>
