...
The generated production configuration sets `rtc.use_external_ip: true`. LiveKit
then makes outbound STUN requests to discover the public IP it should advertise
to clients. Allow UDP egress from the LiveKit container to its configured or
default STUN servers. This is separate from UDP 3478 in the table, which is an
inbound listener for LiveKit's embedded TURN/UDP relay.
...
For an internet-facing deployment with a stable, known public IP, you can also
avoid public STUN discovery by configuring the advertised address explicitly:
...
Use this only when the advertised address and the required media port
forwarding are stable. Otherwise, keep `use_external_ip: true` so LiveKit can
discover the address automatically.
...
The Compose example uses LiveKit's built-in TURN/UDP server, so you do not need
to run a separate coturn service for a standard deployment:
...
A separate TURN server such as [coturn](https://github.com/coturn/coturn) is
useful when you need tighter network separation, a shared TURN service for
multiple LiveKit deployments, or TURN/TLS on port 443 without moving LiveKit
itself. Configure static credentials on the TURN server and list it under
LiveKit's `rtc.turn_servers` configuration.
