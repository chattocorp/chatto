...
- Each content key is wrapped through Chatto's KMS boundary with the author's per-user key encryption key (KEK).
...
The built-in KMS runs in-process and stores per-user KEKs in a dedicated `ENCRYPTION_KEYS` NATS KV bucket. Authentication tags are verified on every read; tampered ciphertext or ciphertext copied into the wrong event context is rejected with `ErrDecryptionFailed`.
...
2. **Deletes the user's KEK**, rendering remaining encrypted body bytes and wrapped content key epochs permanently unreadable.
...
| ----------------------------------- | ------------------------------------------- | -------------- |
| User message-body KEKs              | `ENCRYPTION_KEYS` (NATS KV)                 | No (export separately) |
