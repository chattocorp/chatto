...
- Reconnect catch-up in the web client refreshes the currently viewed room window from `RoomTimelineProjection` after wake/reconnect. When the user is at the bottom it fetches the latest page; when scrolled up it uses `eventsAround` for the visible anchor event and preserves scroll by event ID. The server still supports `Subscription.myEvents(after:)` for callers that need durable replay, but the browser event bus no longer keeps a replay cursor.
