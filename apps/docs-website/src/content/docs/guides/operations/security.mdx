...
handles use typed `RUNTIME_STATE` `session.{hmac}` records with explicit fixed
expiry. The browser handle rotates in the final quarter of its lifetime, while
ordinary validation remains read-only. A separate immutable TTL marker drives
physical cleanup without making that cleanup the authorization boundary.

Human bearer authentication uses renewable sessions. Each login returns a
short-lived access token (15 minutes by default), a rotating refresh
credential, and a fixed maximum session lifetime (90 days by default). The
...
checks serialize its updates, while a separate immutable TTL marker drives
physical cleanup. Successful access-token validation never extends either
expiry. Raw access and refresh credentials are not stored.

Refresh rotation uses JetStream KV revision checks across replicas. An exact
retry with the same client-persisted request ID can recover a lost response;
reuse of an older refresh generation under another request ID revokes the
whole renewable session. Deleting the stable record rejects every access
generation on its next API request or realtime connection, even before its
short-lived verifier record expires. A realtime socket that already
authenticated can continue only until that access token's fixed expiry unless
a separate logout or account-lifecycle signal closes it sooner.
...
| Runtime credential verifiers | `RUNTIME_STATE` (`session.{hmac}` and `renewable_session.{hmac}`, HMAC-keyed, with explicit expiry and TTL markers) | Yes (not raw access tokens, refresh credentials, or cookie handles) |
