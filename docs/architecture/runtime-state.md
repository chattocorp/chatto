...
| `grant.{hmac}` | OAuth authorization code JSON bound to user ID, optional client ID, optional exact OAuth resource and scopes, exact redirect URI, PKCE challenge, and the user auth generation it was issued against. The generation key is always written. Uses per-key 5-minute TTL and is claimed with a revision-matched delete before exchange validation and token issuance. |
