...
| `client_sync.{userId}.deleted` | Retained no-TTL committed deletion fence created after the account-deletion event and before client-sync privacy cleanup. Mutations check it before and after their OCC write so an authenticated request racing deletion cannot recreate either user record. |
| `client_sync.{userId}.deletion_preparing` | No-TTL exclusive pre-commit fence containing its creation time. It blocks mutations before the account event. After projection catch-up, every replica promotes preparations for deleted accounts into committed cleanup and revision-purges active-account preparations older than one hour. |
| `client_sync.{userId}.deletion_pending` | No-TTL post-commit retry marker created after the account event and purged only after both client-sync user records have been purged. Every replica periodically scans this filtered committed-work family. |
