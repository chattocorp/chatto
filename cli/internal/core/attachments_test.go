...
func TestChattoCore_ServerAssetAnyBackendLookupOrder(t *testing.T) {
	core, _ := setupTestCore(t)
	ctx := testContext(t)
	assetID := NewAssetID()
	publicKey := PublicServerAssetObjectKey(assetID)
	...
	if _, err := core.storage.serverAssets.Put(ctx, jetstream.ObjectMeta{
		Name:    assetID,
		Headers: map[string][]string{"Content-Type": {"image/legacy"}},
	}, bytes.NewReader([]byte("legacy"))); err != nil {
		t.Fatalf("store legacy fixture: %v", err)
	}
	if _, err := core.storage.serverAssets.Put(ctx, jetstream.ObjectMeta{
		Name:    publicKey,
		Headers: map[string][]string{"Content-Type": {"image/current"}},
	}, bytes.NewReader([]byte("current"))); err != nil {
		t.Fatalf("store current fixture: %v", err)
	}

	record, err := core.ServerAssetRecordFromAnyBackend(ctx, assetID, "preview")
	if err != nil {
		t.Fatalf("resolve current record: %v", err)
	}
	if record.GetNats().GetKey() != publicKey || record.GetContentType() != "image/current" {
		t.Fatalf("current record = %+v, want namespaced object", record)
	}
	reader, info, err := core.GetServerAssetFromAnyBackend(ctx, assetID)
	if err != nil {
	...
	if err != nil || string(current) != "current" || info.ContentType != "image/current" {
		t.Fatalf("current object = %q, %+v, %v", current, info, err)
	}

	if err := core.storage.serverAssets.Delete(ctx, publicKey); err != nil {
		t.Fatalf("delete current fixture: %v", err)
	}
	record, err = core.ServerAssetRecordFromAnyBackend(ctx, assetID, "preview")
	if err != nil {
		t.Fatalf("resolve legacy record: %v", err)
	}
	if record.GetNats().GetKey() != assetID || record.GetContentType() != "image/legacy" {
		t.Fatalf("legacy record = %+v, want flat object", record)
	}
	reader, _, err = core.GetServerAssetFromAnyBackend(ctx, assetID)
	if err != nil {
		t.Fatalf("open legacy object: %v", err)
	}
	legacy, err := io.ReadAll(reader)
	if closer, ok := reader.(io.Closer); ok {
		_ = closer.Close()
	}
	if err != nil || string(legacy) != "legacy" {
		t.Fatalf("legacy object = %q, %v", legacy, err)
	}

	if _, err := core.ServerAssetRecordFromAnyBackend(ctx, publicKey, "preview"); err == nil {
		t.Fatal("explicit namespaced record fell back to flat object")
	}
	if _, _, err := core.GetServerAssetFromAnyBackend(ctx, publicKey); err == nil {
		t.Fatal("explicit namespaced read fell back to flat object")
	}
}
