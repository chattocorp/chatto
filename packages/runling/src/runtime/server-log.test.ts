...
describe('server logging', () => {
  ...
  it('prints readable diagnostics and writes structured private file records beside the config', () => {
    ...
    expect(stripVTControlCharacters(consoleLog.mock.calls[0]![0])).toMatch(
      /^\d{2}:\d{2}:\d{2} \[test\] ✗ run error · broken$/
    );
...
it('prints the ready URL once on one colored line and honors NO_COLOR', () => {
  ...
  expect(stripVTControlCharacters(colored)).toMatch(
    /^\d{2}:\d{2}:\d{2} ✓ Runling ready → http:\/\/localhost:5173$/
  );
...
it('keeps task identities distinct under compact agent role prefixes', () => {
  ...
  vi.stubEnv('NO_COLOR', '1');
  const output = vi.spyOn(console, 'info').mockImplementation(() => {});
  for (const taskReference of ['task-3', 'task-4'])
    serverLog('info', 'run.activity', {
      runReference: 'sunny-poems-5431',
      taskReference,
...
it('marks successful task activity in the terminal', () => {
  ...
  const output = vi.spyOn(console, 'info').mockImplementation(() => {});
  serverLog('info', 'run.activity', {
    runReference: 'quiet-run-1234',
    activity: 'Validation passed · check',
