...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="A more practical permission baseline">
    ...
    separates channel-room membership from access to channel-room message content.
  </ReleaseFeatureCard>
...
- Before or during the upgrade, grant `message.read` to every existing role or
  account that must keep channel-room message access. A server-level `everyone`
  grant preserves the previous channel-room membership-based behavior. Chatto
  never migrates, backfills, or reconciles existing RBAC state, while fresh
  servers receive the grant during empty-stream bootstrap. DM membership
