...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="Bearer sessions renew without another login">
    Human clients use short-lived access tokens and rotating refresh credentials with a fixed
    maximum session lifetime. The bundled frontend renews in the background and reconnects
    realtime in place without reopening OAuth or losing the current route.
  </ReleaseFeatureCard>
  ...
  <ReleaseFeatureCard title="A more practical permission baseline">
    ...
    give admins an explicit room-level posting grant. The new `message.read` permission
    separates room membership from access to message content.
  </ReleaseFeatureCard>
...
- Browser sessions that still use the retired pre-typed cookie format are not accepted by
  0.5. Affected users are signed out once and must sign in again; current typed browser
  sessions stored under `session.*` remain valid.
- Pre-renewal bearer records are not accepted by 0.5 servers. Affected users sign in once to
  ...
  serialize `/oauth/token` refresh grants, including the same `refresh_request_id` across a
  lost-response retry.
- Before or during the upgrade, grant `message.read` to every existing role or
  account that must keep message access. A server-level `everyone` grant
  preserves the previous membership-based behavior. Chatto never migrates,
  backfills, or reconciles existing RBAC state, while fresh servers receive the
  grant during empty-stream bootstrap. Upgrade every replica before you rely
  on a deny or missing grant because older replicas enforce membership only.
