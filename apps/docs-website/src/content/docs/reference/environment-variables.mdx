...
Set `request_email = false` to avoid requesting provider email scopes. OIDC providers can run with only `openid profile`; Chatto links and signs in accounts by issuer and subject even when no email claim is returned.

Set `auto_provision = true` only for providers whose users should be allowed to create Chatto accounts. Unlinked identities from providers without auto-provisioning can still be linked by existing signed-in users from account settings.
