...
function fixture() {
  ...
  const worker = {
    runOutcome: vi.fn((_ctx, _prompt, _options?: AgentRunOptions) => done.promise),
...
test('keeps context and usage explicit; runs without I/O and does not dispose the agent', async () => {
  ...
  expect(f.worker.runOutcome).toHaveBeenCalledWith(f.ctx, 'Hello', expect.any(Object));
  expectTypeOf(result).toEqualTypeOf<Promise<import('../agent.ts').AgentResult>>();
...
test.each(['false', 'reject', 'throw', 'absent'] as const)(
  ...
  async (mode) => {
    ...
    const steer =
      mode === 'absent'
        ...
        : vi.fn(() => {
            ...
            return mode === 'reject' ? Promise.reject(new Error('async')) : Promise.resolve(false);
          });
    const inbox = createChannel<string>();
    ...
    await vi.waitFor(() => expect(f.onDelivery).toHaveBeenCalledWith('message', false));
    f.done.resolve(report);
    await run;
  }
);
...
test('uses one iterator across turns and reports messages received between turns as missed', async () => {
  ...
  await vi.waitFor(() => expect(f.onDelivery).toHaveBeenCalledWith('between', false));
  ...
  await vi.waitFor(() => expect(f.onDelivery).toHaveBeenCalledWith('repair', true));
...
test('separate connections do not mix context, input, output, or cancellation', async () => {
  ...
  await vi.waitFor(() => expect(b.onDelivery).toHaveBeenCalledWith('only B', true));
  await first.dispose();
  await failed;
...
test('input claims a single consumer and cleanup tolerates a throwing return', async () => {
  const f = fixture();
  expect(() => connectAgent(f.ctx, f.worker, { inbox: f.inbox })).toThrow('consumer');
...
test('waits for a pending delivery callback before completing the turn', async () => {
  ...
  void run.then(settled);
  await f.inbox.send('message');
  await vi.waitFor(() => expect(f.onDelivery).toHaveBeenCalledWith('message', true));
  f.done.resolve(report);
  await Promise.resolve();
  expect(settled).not.toHaveBeenCalled();
