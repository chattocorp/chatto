...
# Omit languages to enable every bundled analyzer.
languages = ["en", "de", "fr", "cjk"]
```

The Bleve directory contains plaintext-derived message content. Keep it on a protected local volume and exclude it from normal backups. Chatto rebuilds it from retained `EVT` history if it is missing or incompatible.

`search_provider.languages` controls which language-specific analyzers index
each message and analyze queries. Supported codes are `ar`, `cjk`, `ckb`, `da`,
`de`, `en`, `es`, `fa`, `fi`, `fr`, `hi`, `hr`, `hu`, `it`, `nl`, `no`, `pl`,
`pt`, `ro`, `ru`, `sv`, and `tr`. Omit the setting to use all bundled analyzers,
or set `languages = []` to keep only language-neutral literal and typo-tolerant
matching. Changing the list automatically rebuilds the disposable index.
