...
test('service worker caches the shell while leaving private requests on the network', async ({ page }) => {
  await page.goto('/');
  ...
  expect((await cacheSnapshot(page)).cacheNames.some((name) => name.startsWith('chatto-shell-'))).toBe(true);
  ...
  const cachedRequests = await page.evaluate(async () => {
    const cache = await caches.open((await caches.keys()).find((name) => name.startsWith('chatto-shell-'))!);
    return (await cache.keys()).map((request) => new URL(request.url).pathname);
  });
  expect(cachedRequests).toContain('/login');
  expect(cachedRequests.some((path) => path.startsWith('/api/') || path.startsWith('/assets/'))).toBe(false);
...
test('offline reload restores saved text in the normal chat view', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(async (body) => {
    ...
    try {
      const views = await new Promise<Array<{ rooms?: Array<{ messages?: Array<{ body?: string }> }> }>>((resolve, reject) => {
        const request = db.transaction('views').objectStore('views').getAll();
        request.onsuccess = () => resolve(request.result);
        request.onerror = () => reject(request.error);
      });
      return views.some((view) => view.rooms?.some((room) => room.messages?.some((entry) => entry.body === body)));
    } finally {
      db.close();
    }
  }, message)).toBe(true);
