...
export type ImplementationProcess = (command: string, args: string[], options: {
  cwd: string;
  signal: AbortSignal;
  timeoutMs?: number;
...
export const implementationProcess: ImplementationProcess = async (command, args, options) => {
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    const kill = (signal: NodeJS.Signals) => {
      ...
      } catch { /* The process can exit before cancellation arrives. */ }
    };
    const stop = () => {
    ...
    child.on("error", () => { failure = new Error("Command could not start"); });
    child.on("close", code => {
      ...
      else if (code !== 0) reject(options.captureDiagnostics
        ? new ImplementationCommandError(`Command failed (exit ${code ?? "signal"})`, `${output}\n${errors}`)
