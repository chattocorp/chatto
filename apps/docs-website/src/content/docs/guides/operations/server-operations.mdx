...
`chatto_users_total` reports all non-deleted accounts, including bots, by verified-email status. `chatto_bot_users_total` reports the bot-account subset as a separate gauge.
