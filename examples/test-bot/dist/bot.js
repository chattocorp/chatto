...
import { RealtimeInitialState, RealtimeServerFrame, RealtimeSubscribe, } from "@chatto/api-types/realtime/v1/realtime_pb";
import { readFile } from "node:fs/promises";
import { loadTestBotState, rememberProcessedEvent, saveTestBotState, } from "./state.js";
...
function realtimeUrl(serverUrl) {
    const url = new URL("/api/realtime", serverUrl);
    url.protocol = url.protocol === "https:" ? "wss:" : "ws:";
    return url.toString();
}
async function messageDataToBytes(data) {
...
function retryAfterMilliseconds(seconds, nanos) {
...
async function readAPIKey(apiKeyFile) {
    ...
    if (!apiKey)
        throw new Error("bot API key file is empty");
    return apiKey;
}
async function verifyPublicAPI(config, apiKey) {
    const authorization = (next) => async (request) => {
    ...
    if (!viewerId)
        throw new Error("viewer response did not contain a user ID");
...
async function commitState(stateFile, state, resumeCursor) {
    if (resumeCursor)
        state.resumeCursor = resumeCursor;
    await saveTestBotState(stateFile, state);
}
async function runRealtimeSession(config, apiKey, state, signal) {
    ...
    const result = await new Promise((resolve) => {
        ...
        socket.addEventListener("message", (message) => {
            processing = processing
                .then(async () => {
                const frame = RealtimeServerFrame.fromBinary(await messageDataToBytes(message.data));
                switch (frame.frame.case) {
                    ...
                    case "heartbeat":
                        if (frame.frame.value.resumeCursor) {
                            await commitState(config.stateFile, state, frame.frame.value.resumeCursor);
                        }
                        return;
                    case "caughtUp":
                        await commitState(config.stateFile, state, frame.frame.value.cursor);
                        caughtUp = true;
                    ...
                    case "close": {
                        ...
                        requestedResult = {
                            ...
                            ...(close.retryAfter
                                ? {
                                    retryAfterMs: retryAfterMilliseconds(close.retryAfter.seconds, close.retryAfter.nanos),
                                }
                                : {}),
                        };
                        log({
...
function reconnectDelay(attempt, requestedDelay) {
    if (requestedDelay !== undefined) {
        return Math.min(MAXIMUM_RECONNECT_DELAY_MS, Math.max(MINIMUM_RECONNECT_DELAY_MS, requestedDelay));
    }
    return Math.min(MAXIMUM_RECONNECT_DELAY_MS, MINIMUM_RECONNECT_DELAY_MS * 2 ** attempt);
}
...
export async function runTestBot(config, signal) {
    ...
    while (!signal.aborted) {
        try {
            ...
            if (session.caughtUp)
                attempt = 0;
            if (!session.reconnect || signal.aborted)
                return;
