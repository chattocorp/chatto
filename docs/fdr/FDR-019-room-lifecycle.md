...
- **Settings access** — a joined room's context menu links `room.manage` holders directly to that room's management page. The page exposes general room settings and room-level permission overrides without granting access to other rooms or server-wide administration.
