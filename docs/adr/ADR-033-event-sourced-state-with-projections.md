...
- **Projections are derived state.** Each aggregate type has one or more projections rebuilt from the event stream that serve reads. Most are in-memory Go data structures maintained independently by every Chatto process. ADR-054 additionally permits disposable locally checkpointed projections, such as a disk-backed search index, while preserving `EVT` as the reconstruction authority.
...
- **Restore mechanisms are optional capabilities.** The base projection contract owns logical subjects and ordered event application. [ADR-050](ADR-050-ephemeral-encrypted-projection-snapshots.md) defines optional portable encrypted snapshots for eligible in-memory projections, while [ADR-054](ADR-054-locally-checkpointed-projections.md) defines projection-owned local checkpoints for disposable disk-backed read models. Neither changes the authority of `EVT`.
...
- Snapshot persistence and projection bootstrapping — [ADR-050](ADR-050-ephemeral-encrypted-projection-snapshots.md) defines optional encrypted snapshots, while [ADR-054](ADR-054-locally-checkpointed-projections.md) defines local checkpointed read models and their replay frontiers.
