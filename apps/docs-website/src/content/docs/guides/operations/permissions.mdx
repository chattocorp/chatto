...
`message.post-interactions` (reply only in readable threads with an interaction
...
and `message.post-interactions` for `everyone` in that room. An effective broad
...
3. Deny `message.post-in-thread` and `message.post-interactions` to prevent replies. Optionally deny `message.attach` and `message.react` as well.
...
4. Deny `message.post-in-thread` and `message.post-interactions` to prevent replies. Optionally deny `message.attach`, `message.react`, and `message.echo`.
