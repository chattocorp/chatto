...
func TestPublicBotOwnerProfile(t *testing.T) {
	...
	require.Equal(t, env.viewer.Id, response.Msg.Users[0].User.GetBotOwnerUserId())
	require.Nil(t, response.Msg.Users[1].User.BotOwnerUserId)
	...
	require.NoError(t, err)
	require.Equal(t, env.viewer.Id, snapshot.GetBotOwnerUserId())
	...
	require.Equal(t, env.viewer.Id, adminSummary.GetBotOwnerUserId())
	humanSnapshot, err := env.api.realtimeSnapshotUser(env.ctx, &core.HydratedUserContent{User: reader})
	require.NoError(t, err)
	require.Nil(t, humanSnapshot.BotOwnerUserId)
	...
	response, err = service.BatchGetUsers(withCaller(env.ctx, reader), request)
	require.NoError(t, err)
	require.Equal(t, reader.Id, response.Msg.Users[0].User.GetBotOwnerUserId())
	deleted, err := userSummary(env.ctx, env.api, core.DeletedUserReference(bot.User.Id), nil)
	require.NoError(t, err)
	require.Nil(t, deleted.BotOwnerUserId)
}
