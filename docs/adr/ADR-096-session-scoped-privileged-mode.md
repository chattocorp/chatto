# ADR-096: Require Session-Scoped Privileged Mode
...
client reads current viewer and room resources through ConnectRPC before it
completes catch-up. A normal cursor resume contains only later EVT changes and
cannot represent this runtime-state mutation. These resource updates change
