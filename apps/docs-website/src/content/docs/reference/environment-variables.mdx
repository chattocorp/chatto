...
<EnvVar name="CHATTO_SHIELDS_ENABLED" toml="shields.enabled" default="false">
  Expose public PNG shield URLs for aggregate community counts. When enabled, `/shields/online.png` shows users with any live presence record and `/shields/registered.png` shows verified accounts. Leave disabled for private communities that should not publish size or activity counts.
</EnvVar>
