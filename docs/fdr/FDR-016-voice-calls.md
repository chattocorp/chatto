...
**Decision:** `CallStartedEvent`, `CallParticipantJoinedEvent`, `CallParticipantLeftEvent`, and `CallEndedEvent` are persisted in the room EVT aggregate keyed by room ID, on `evt.room.{roomId}.call_started`, `evt.room.{roomId}.call_joined`, `evt.room.{roomId}.call_left`, and `evt.room.{roomId}.call_ended`. Explicit frontend join/leave writes use source `USER`; LiveKit webhook writes use source `LIVEKIT`; reconciliation writes use source `RECONCILIATION`. Public GraphQL and protobuf-wire views expose the same room/call identity without the internal source or E2EE key ref. Room history shows only call start/end notices; participant join/leave facts drive active call state and live indicators.
...
**Decision:** Active participant snapshots and the active call session come from a call-state service/projection over durable call facts, not from `MEMORY_CACHE`. User joins can create pending/optimistic state; LiveKit and reconciliation facts confirm or correct it. On startup and periodically, Chatto compares active LiveKit rooms/participants to the projection and appends reconciliation facts for mismatches. If LiveKit repeatedly cannot list rooms/participants, Chatto ends projected active calls with reconciliation facts after the configured failure threshold instead of immediately on the first transient listing failure.
...
**Tradeoff:** The projection can briefly show optimistic state before LiveKit or reconciliation corrects it. If LiveKit reports the same already-active transition, the duplicate report is skipped instead of appending another public call event. A sustained LiveKit listing outage can still end projected active calls, favoring eventual UI recovery and unblocking new sessions over preserving stale call state forever. Multiple replicas may reconcile concurrently; call transition facts are OCC-gated on the room aggregate and rechecked after conflicts.
...
**Why:** LiveKit delivers audio data over WebRTC, but the browser doesn't autoplay it without an attached element. Without explicit attach, the UI looks like everything works — participant rings even animate — but nobody hears anything. The pattern lives in `frontend/src/lib/state/server/voiceCall.svelte.ts`; any refactor that touches LiveKit subscription handling needs to keep the `track.attach()` / `track.detach()` calls intact.
...
- Call token lookup (`voiceCallToken` in GraphQL, `GetVoiceCallToken` over wire) — requires room membership.
- Call participant reads (`callParticipants` in GraphQL, `GetCallParticipants` over wire) — require room membership.
- Active call room reads (`activeCallRoomIds` in GraphQL, `ListActiveCalls` over wire) — require server authentication; the bundled wire client receives participant previews only for visible rooms.
- Join/leave intents (`joinVoiceCall` / `leaveVoiceCall` in GraphQL, `JoinVoiceCall` / `LeaveVoiceCall` over wire) — require room membership.
