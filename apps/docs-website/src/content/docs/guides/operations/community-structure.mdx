...
Fresh servers start with a `Lobby` group containing `#announcements` and `#general`. If you have the required permission, use the group and room action menus in the sidebar to create rooms and links, change settings, archive rooms, or remove empty groups. Use the drag handles to reorder groups or to move rooms and links within or between groups. Create another group with **New Group** after the last room group.
