...
| Server/user/room config | Server Config   | `evt.config.>`, `evt.room_config.>`, selected user preference/deletion facts, room and room-group deletion facts | `ConfigModel`; server config, branding refs, user preferences, notification levels, blocked usernames, and sparse room-configuration layers |
