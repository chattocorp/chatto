/** Write safe Runling server diagnostics to the terminal and rotating log. */
...
export function serverLog(level: Level, event: string, fields: Record<string, unknown> = {}): void {
  let line: string;
  try {
    line = JSON.stringify(
