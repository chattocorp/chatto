...
{#if run.status === 'running' || run.output === null}
  <section
    class="flex items-start gap-3 rounded-lg border border-dashed border-base-300 p-6"
    aria-label="Workflow output"
  >
    <span
      class={run.status === 'running'
        ? 'loading loading-spinner loading-sm text-base-content/50'
        : 'icon-[lucide--file-text] size-5 shrink-0 text-base-content/50'}
      aria-hidden="true"
    ></span>
    <div class="space-y-1">
      <h2 class="text-sm font-medium">
        {run.status === 'running' ? 'Waiting for output' : 'No output returned'}
      </h2>
