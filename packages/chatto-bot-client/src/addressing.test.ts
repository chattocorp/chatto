...
test("disabled reply recognition makes no lookup", async () => {
  ...
  expect(await addressedMessage(client, event(), { viewerId: "viewer", reasons: ["mention"] })).toBeUndefined();
...
test("DM and viewer mention recognition needs no lookup and preserves empty text", async () => {
  ...
  expect(await addressedMessage(client, incoming, { viewerId: "viewer" })).toMatchObject({
  ...
  expect((await addressedMessage(client, incoming, { viewerId: "viewer" }))?.reasons).toEqual(["mention"]);
  expect(request).not.toHaveBeenCalled();
});
...
test.each(["self", "missing-text", "missing-id", "missing-actor", "other-event", "unaddressed"])("ignores %s without lookup", async kind => {
  ...
  expect(await addressedMessage(client, incoming, { viewerId: "viewer" })).toBeUndefined();
  expect(request).not.toHaveBeenCalled();
});
...
test("replies to a viewer-authored thread root are recognized", async () => {
  ...
  expect((await addressedMessage(client, incoming, { viewerId: "viewer" }))?.reasons).toEqual(["reply"]);
});
...
test("lookup failures propagate sanitized errors; cancellation does not become an ignored message", async () => {
  ...
  await expect(addressedMessage(client, event(), { viewerId: "viewer" })).rejects.toThrow(/^Chatto API request did not complete$/);
  const controller = new AbortController();
  controller.abort(new Error("cancelled"));
  await expect(addressedMessage(client, event(), { viewerId: "viewer", signal: controller.signal })).rejects.toThrow("cancelled");
  expect(request).toHaveBeenCalledOnce();
});
