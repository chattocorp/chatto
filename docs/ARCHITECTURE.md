...
The ConnectRPC API is an early protobuf-first public API mounted under `/api/connect`. Generated service paths are prefixed by that mount, for example `/api/connect/chatto.api.v1.ServerService/GetServer`. `cli/internal/connectapi` owns service implementations and API/core protobuf mapping; `cli/internal/http_server/connect.go` owns Gin route mounting and existing bearer-token-then-cookie auth injection. Registered Connect handlers carry an explicit auth policy: `ServerService.GetServer` is public, normal private services are wrapped with user auth middleware, and the opt-in `AdminService` is wrapped with a separate admin-token middleware. The HTTP edge resolves the full user through existing auth mechanisms for normal private services, stores a narrow `connectapi.Caller` in Connect authn context, and private Connect handlers pass only the actor ID into core operation models. AdminService requests never resolve a user session; when `[admin_api].enabled` is true, the edge accepts `Authorization: Bearer ...` only when the token matches one configured `[[admin_api.tokens]]` entry and the direct TCP peer IP is inside that token entry's `allowed_cidrs` (default loopback only), then stores a separate `connectapi.AdminCaller` with the matched token name. Admin handlers pass `core.SystemActorID` into domain writes, while HTTP auth logs include the token name as non-secret operation context. When `[admin_api.listener].enabled` is true, AdminService is served on that dedicated listener and is not mounted on the public web listener; otherwise it remains mounted under public `/api/connect` and deployments must block those routes at any public reverse proxy. Those models own room membership authorization, reaction/thread/root/anchor validation, and lower-level core helper calls. The ConnectRPC layer keeps protobuf request/response shaping, schema validation, cursor parsing/formatting, hydrated public timeline DTOs, and Connect status mapping.
...
| -------------------------------------------------------------------------------------------- | -------------------------------- | ------------------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `/api/connect/chatto.api.v1.AdminService/ListUsers`                                          | `AdminService`                   | `ListUsers`                     | Admin API token from that token entry's allowed CIDRs; service mounted only when `[admin_api].enabled = true`, either on the public listener or on `[admin_api.listener]`. | Operator-only paginated user listing for CLI/bootstrap automation.                            |
| `/api/connect/chatto.api.v1.AdminService/GetUser`                                            | `AdminService`                   | `GetUser`                       | Admin API token from that token entry's allowed CIDRs; service mounted only when `[admin_api].enabled = true`, either on the public listener or on `[admin_api.listener]`. | Operator-only user lookup by ID or login.                                                     |
| `/api/connect/chatto.api.v1.AdminService/CreateUser`                                         | `AdminService`                   | `CreateUser`                    | Admin API token from that token entry's allowed CIDRs; service mounted only when `[admin_api].enabled = true`, either on the public listener or on `[admin_api.listener]`. | Create a user as the system actor, optionally with password, verified email, and roles.       |
| `/api/connect/chatto.api.v1.AdminService/UpdateUser`                                         | `AdminService`                   | `UpdateUser`                    | Admin API token from that token entry's allowed CIDRs; service mounted only when `[admin_api].enabled = true`, either on the public listener or on `[admin_api.listener]`. | Update a user's login and/or display name as the system actor.                                |
| `/api/connect/chatto.api.v1.AdminService/SetUserPassword`                                    | `AdminService`                   | `SetUserPassword`               | Admin API token from that token entry's allowed CIDRs; service mounted only when `[admin_api].enabled = true`, either on the public listener or on `[admin_api.listener]`. | Set a user's password as the system actor and advance their auth generation.                  |
| `/api/connect/chatto.api.v1.AdminService/DeleteUser`                                         | `AdminService`                   | `DeleteUser`                    | Admin API token from that token entry's allowed CIDRs; service mounted only when `[admin_api].enabled = true`, either on the public listener or on `[admin_api.listener]`. | Permanently delete a user through the normal account-deletion model.                          |
| `/api/connect/chatto.api.v1.AdminService/AddUserVerifiedEmail`                               | `AdminService`                   | `AddUserVerifiedEmail`          | Admin API token from that token entry's allowed CIDRs; service mounted only when `[admin_api].enabled = true`, either on the public listener or on `[admin_api.listener]`. | Add an already-verified email address to a user as the system actor.                          |
| `/api/connect/chatto.api.v1.AdminService/AssignUserRole`                                     | `AdminService`                   | `AssignUserRole`                | Admin API token from that token entry's allowed CIDRs; service mounted only when `[admin_api].enabled = true`, either on the public listener or on `[admin_api.listener]`. | Assign a server role to a user as the system actor.                                           |
| `/api/connect/chatto.api.v1.AdminService/RevokeUserRole`                                     | `AdminService`                   | `RevokeUserRole`                | Admin API token from that token entry's allowed CIDRs; service mounted only when `[admin_api].enabled = true`, either on the public listener or on `[admin_api.listener]`. | Revoke a server role from a user as the system actor.                                         |
