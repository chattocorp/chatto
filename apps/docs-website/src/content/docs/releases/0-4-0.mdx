...
- Message posting no longer has a server-side large-mention confirmation challenge.
  ...
  `MessageService.CreateMessage`. `CreateMessageResponse` now returns only the
  created message event; hydrate user/member details through `ServerService.GetMember`
  or `BatchGetMembers` when needed.
