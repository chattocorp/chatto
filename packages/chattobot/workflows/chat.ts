...
export const conversation = task(async (
  ...
  const bot = await createAgent({
    ...
    instructions: [
      ...
      ...(options.investigation ? ["Source investigation is enabled through investigateChatto. Pass relevant scope, observations, and reproduction steps."] : []),
  ...
  try {
    ...
    return await runAgentConversation({ ...ctx, emit: async text => {
      ...
      if (/<\|[^>\r\n]*\|>|<\|?(?:channel|im_start|im_end)\|>|<\/?(?:think|thought|analysis)>|^\s*Token usage\s*:/im.test(text)) {
      ...
      async prepareMessage(message, origin) {
        ...
        return JSON.stringify({ thread, origin, recentUserMessages: [...recentUserMessages],
          ...(origin === "user" ? { currentMessage: message } : { notification: taskNotification(message) }),
          backgroundTasks: taskContext(tasks.list()),
          savedImplementationPlans: [...plans].map(([investigationId, plan]) => ({ investigationId, plan })) });
      },
