...
Video processing is optional and disabled by default. When enabled, Chatto requires `ffmpeg` and `ffprobe`, stores the original upload, generates a thumbnail and web-friendly MP4 compatibility variants, then packages adaptive HLS playlists and six-second segments.
