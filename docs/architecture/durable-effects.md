...
[ADR-075](../adr/ADR-075-deterministic-notification-occurrences.md).
...
| Notification occurrence materialization and Alert delivery | Source-time policy evaluation replaces one exact `NotificationMaterializationWork` value in `RUNTIME_STATE` before the existing message/reaction fact commits. The source fact wakes the shared materializer; retraction, reaction removal, visibility loss, and account deletion remain existing domain facts. No notification-only event is added to `EVT`. The materializer appends rich lifecycle facts to the bounded `NOTIFICATIONS` stream | Every source-command OCC retry recomputes mention expansion, policy decisions, and the exact recipient set. The shared `chatto-notification-materializer-v2` EVT consumer applies prepared work idempotently and records visibility-loss boundaries. The `NotificationProjection` consumes and snapshots `NOTIFICATIONS`. The separate `chatto-notification-alert-delivery-v2` consumer filters `notifications.signalled` from the same stream through `events.DurableWorker`, waits for the projection at the delivered sequence, fences the materializer and current policy/authorization projections, revalidates current state, target visibility, subscription ownership, and DND, then appends a terminal alert outcome before acknowledging | Deterministic recipient/source/signal-class identity, replaceable prepared work, dismissal tombstones, read and visibility boundaries, and projection fences make cross-replica ordering explicit. `NOTIFICATIONS` and durable consumer state are backed up. Application expiry is 90 days; per-record broker TTL adds a 24-hour cleanup grace. An immutable two-minute Alert deadline and provider TTL bound delivery after contention, retry, or restore. Terminal projected state makes redelivery an ack-only no-op, but a crash after provider acceptance and before terminal persistence can duplicate a push | Occurrence creation/removal and Alert retry are recoverable and at least once. Materializer and Alert consumer state are included in owner-only durable-worker diagnostics |
