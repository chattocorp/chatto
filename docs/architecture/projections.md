...
| Server/user config | Server Config        | `evt.config.>`, selected user cleanup/preference facts     | `ConfigModel`; server config, branding refs, user preferences, per-signal-class server/room notification intensities, blocked usernames; historical coarse notification-level facts decode but have no projected effect |
