...
| `timezone` | `optional string` | IANA time zone the user shares on their profile (for example "Europe/Berlin"), when set. Absent means the user has not chosen a shareable time zone. |
...
| `has_unread` | `optional bool` | True when the thread has unread replies or neutral Badge attention for the current user, when known. |
...
<a id="chatto-admin-v1-PermissionDefinition"></a>
...
| --- | --- | --- |
| `permission` | `string` | Permission identifier. |
| `included_by_permission` | `optional string` | Immediate parent whose effective allow includes this permission. Absent when the permission has no parent. Dotted names do not imply inclusion. |
...
| `permission_definitions` | repeated [`PermissionDefinition`](#chatto-admin-v1-PermissionDefinition) | Catalog metadata for applicable permissions. |
