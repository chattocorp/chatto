...
func TestChattoCore_AddReaction(t *testing.T) {
	...
	room, _ := core.CreateRoom(ctx, user.Id, "channel", "test-room", "Test room")

	// Join room before posting
	core.JoinRoom(ctx, user.Id, "channel", user.Id, room.Id)

	// Post a message to react to
	event, err := core.PostMessage(ctx, "channel", room.Id, user.Id, "Hello world", nil, "", "", nil, false)
	if err != nil {
	...
	t.Run("add new reaction", func(t *testing.T) {
		added, err := core.AddReaction(ctx, "channel", room.Id, eventID, "thumbsup", user.Id)
		if err != nil {
	...
	t.Run("different users can add same emoji", func(t *testing.T) {
		added, err := core.AddReaction(ctx, "channel", room.Id, eventID, "thumbsup", "other-user")
		if err != nil {
			t.Fatalf("AddReaction failed: %v", err)
		}
		if !added {
	...
	t.Run("same user can add different emoji", func(t *testing.T) {
		added, err := core.AddReaction(ctx, "channel", room.Id, eventID, "heart", user.Id)
		if err != nil {
			t.Fatalf("AddReaction failed: %v", err)
		}
		if !added {
	...
	t.Run("add reaction with unicode emoji is rejected", func(t *testing.T) {
		_, err := core.AddReaction(ctx, "channel", room.Id, eventID, "🎉", user.Id)
		if err == nil {
	...
	t.Run("add reaction with invalid input", func(t *testing.T) {
		_, err := core.AddReaction(ctx, "channel", room.Id, eventID, "not_valid", user.Id)
		if err == nil {
...
func TestChattoCore_RemoveReaction(t *testing.T) {
	...
	_, err = core.AddReaction(ctx, "channel", room.Id, eventID, "thumbsup", user.Id)
	if err != nil {
		t.Fatalf("AddReaction failed: %v", err)
	}

	t.Run("remove existing reaction", func(t *testing.T) {
		removed, err := core.RemoveReaction(ctx, "channel", room.Id, eventID, "thumbsup", user.Id)
		if err != nil {
	...
	t.Run("remove reaction that was never added", func(t *testing.T) {
		removed, err := core.RemoveReaction(ctx, "channel", room.Id, eventID, "tada", user.Id)
		if err != nil {
			t.Fatalf("RemoveReaction failed: %v", err)
		}
		if removed {
	...
	t.Run("remove reaction with unicode emoji is rejected", func(t *testing.T) {
		_, err = core.RemoveReaction(ctx, "channel", room.Id, eventID, "🚀", user.Id)
		if err == nil {
...
func TestChattoCore_GetReactions(t *testing.T) {
	...
	// Add some reactions
	core.AddReaction(ctx, "channel", room.Id, eventID, "thumbsup", "user1")
	core.AddReaction(ctx, "channel", room.Id, eventID, "thumbsup", "user2")
	core.AddReaction(ctx, "channel", room.Id, eventID, "heart", "user1")
	...
	t.Run("reactions isolated to message", func(t *testing.T) {
		...
		event2, _ := core.PostMessage(ctx, "channel", room.Id, user.Id, "Another message", nil, "", "", nil, false)
		eventID2 := event2.Id
...
func TestChattoCore_GetReactionsBatch(t *testing.T) {
	...
	// Post two messages
	event1, _ := core.PostMessage(ctx, "channel", room.Id, user.Id, "Message 1", nil, "", "", nil, false)
	event2, _ := core.PostMessage(ctx, "channel", room.Id, user.Id, "Message 2", nil, "", "", nil, false)

	// Add reactions to message 1
	core.AddReaction(ctx, "channel", room.Id, event1.Id, "thumbsup", user.Id)
	core.AddReaction(ctx, "channel", room.Id, event1.Id, "heart", "user2")

	// Add reaction to message 2
	core.AddReaction(ctx, "channel", room.Id, event2.Id, "tada", user.Id)
	...
	t.Run("batch fetch with no reactions returns empty map", func(t *testing.T) {
		event3, _ := core.PostMessage(ctx, "channel", room.Id, user.Id, "Message 3", nil, "", "", nil, false)
...
func TestChattoCore_EchoReactionsShared(t *testing.T) {
	...
	rootEvent, err := core.PostMessage(ctx, "channel", room.Id, user.Id, "Root message", nil, "", "", nil, false)
	if err != nil {
		t.Fatalf("PostMessage (root) failed: %v", err)
	}

	replyEvent, err := core.PostMessage(ctx, "channel", room.Id, user.Id, "Thread reply", nil, rootEvent.Id, rootEvent.Id, nil, true)
	if err != nil {
		t.Fatalf("PostMessage (reply+echo) failed: %v", err)
	}

	// Find the echo event in the room events
	result, err := core.GetRoomEvents(ctx, "channel", room.Id, 50, nil)
	if err != nil {
	...
	t.Run("reaction on echo is stored against original", func(t *testing.T) {
		added, err := core.AddReaction(ctx, "channel", room.Id, echoEventID, "thumbsup", user.Id)
		if err != nil {
	...
	t.Run("reaction on original is visible via echo", func(t *testing.T) {
		added, err := core.AddReaction(ctx, "channel", room.Id, replyEvent.Id, "heart", user.Id)
		if err != nil {
	...
	t.Run("duplicate reaction via echo is idempotent", func(t *testing.T) {
		...
		added, err := core.AddReaction(ctx, "channel", room.Id, replyEvent.Id, "thumbsup", user.Id)
		if err != nil {
			t.Fatalf("AddReaction failed: %v", err)
		}
		if added {
	...
	t.Run("remove reaction via echo", func(t *testing.T) {
		removed, err := core.RemoveReaction(ctx, "channel", room.Id, echoEventID, "thumbsup", user.Id)
		if err != nil {
