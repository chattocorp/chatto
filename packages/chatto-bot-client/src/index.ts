import type { ChattoClient, ChattoMessage, Destination, RealtimeEvent, ThreadLocation, ThreadMessage } from "@chatto/client";
import { addressedMessage, type AddressingOptions } from "./addressing.js";
export { addressedMessage, type AddressedMessage, type AddressingReason, type AddressingOptions } from "./addressing.js";
...
export async function readBotThread(client: Pick<ChattoClient, "readThread">, viewerId: string,
  location: ThreadLocation, signal?: AbortSignal): Promise<BotThreadMessage[]> {
  return (await client.readThread(location, signal)).map(message => ({
    ...message, role: message.authorId === viewerId ? "bot" : "human",
  }));
}

/** Reply in the original thread and reference the message that prompted the reply. */
export function replyDestination(message: Pick<ChattoMessage, "id" | "roomId" | "threadRootId">): Destination {
  return { roomId: message.roomId, threadRootId: message.threadRootId ?? message.id, inReplyTo: message.id };
}
...
export function conversationKey(viewerId: string, message: Pick<ChattoMessage, "id" | "roomId" | "threadRootId" | "authorId">): string {
  return JSON.stringify([viewerId, message.roomId, message.threadRootId ?? message.id, message.authorId]);
}
...
export async function createBotClient(client: ChattoClient, { signal }: { signal?: AbortSignal } = {}) {
  ...
  return {
    ...
    readThread: (location: ThreadLocation, signal?: AbortSignal) => readBotThread(client, viewerId, location, signal),
  };
}
