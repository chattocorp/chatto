...
issuing `client_id` when applicable, and a client-generated
`refresh_request_id`. Generate a cryptographically random UUID version 4 for
each rotation. Treat it as a show-once recovery nonce. Persist it before you
send the request, and reuse it until you persist the complete response. An
