# ADR-067: Event-Sourced Hierarchical Room Configuration
...
Each setting has a compile-time product default and a typed value. Room
configuration resolves independently per field in this order:
...
values required for client behaviour. Realtime delivery replaces one room's
viewer state for room-scoped changes. Server- and room-group-scoped changes
request a compacted reconnect instead of expanding into an unbounded set of
per-room updates.
