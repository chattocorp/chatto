...
func (c *ChattoCore) MoveRoomToGroup(ctx context.Context, actorID, roomID, targetGroupID string) error {
	...
	// Wait for the highest seq we touched on either aggregate — the
	// single projector applies in stream-global order, so reaching
	// the later seq implies both events have landed.
...
func (c *ChattoCore) publishLayoutOrdering(ctx context.Context, actorID string, groupIDs []string) error {
	...
	seq, err := c.EventPublisher.Append(ctx, events.LayoutAggregate().Subject(), event)
	if err != nil {
		return fmt.Errorf("publish RoomGroupsReorderedEvent: %w", err)
	}
	if err := c.RoomLayoutProjector.WaitForSeq(ctx, seq); err != nil {
		return fmt.Errorf("wait for layout projection: %w", err)
	}
	return nil
}
