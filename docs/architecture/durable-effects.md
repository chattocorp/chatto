...
| Obsolete or retracted message-body erasure | Body masks, retraction, and hidden echo state select obsolete payloads | The mutation calls JetStream `SecureDeleteMsg` only for payloads with no current field or timestamp references; clear events count as current sources | After projections catch up at boot, every replica derives obsolete payload sequences and repeats idempotent secure deletion | Recoverable from EVT replay; shared payloads remain while any current field uses them; boot work is not lease-owned |
...
| Runtime credential cleanup after security changes | Password, account-deletion, and external-identity events advance durable user/auth state before stored sessions and tokens are deleted | The request scans and deletes matching `RUNTIME_STATE` credentials and publishes a pubsub session-termination control | Credential generation prevents stale credentials from authenticating new requests or reconnects; stale records remain cleanup debt, and an already-open realtime connection depends on best-effort session termination | New authentication is durably revoked; physical cleanup and immediate live disconnect are best-effort |
