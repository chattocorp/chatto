...
describe('responsive task dialogs', () => {
  it.each([320, 390, 767, 768])(
    'keeps a centred dialog usable with a mouse at %i px',
    async (width) => {
      await cdp().send('Emulation.setTouchEmulationEnabled', { enabled: false });
      ...
      await expect
        .element(container.querySelector<HTMLButtonElement>('footer button')!)
        .toBeVisible();
  ...
  it('uses the same frame surround as context-menu sheets', async () => {
    ...
    function surround(frame: Element, content: Element) {
      ...
      return [inner.left - outer.left, outer.right - inner.right, outer.bottom - inner.bottom].map(
  ...
  it.each(['long drag', 'fresh tap', 'keyboard'])(
    'ignores clicks after a short handle drag, then closes with %s',
    async (dismissal) => {
      await page.viewport(390, 844);
      const { container } = render(ResponsiveDialogHarness);
      const dialog = container.querySelector('dialog')!;
