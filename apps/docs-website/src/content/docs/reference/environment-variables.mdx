...
<EnvVar name="CHATTO_CORE_PROJECTION_SNAPSHOTS" toml="core.projection_snapshots" default="false">
  Persist encrypted `ThreadProjection` snapshots. Chatto still treats `EVT` as authoritative and automatically replays it when a snapshot is missing, incompatible, corrupt, or unavailable.
</EnvVar>
