...
**Decision:** Account deletion first appends `UserKeyShreddingRequestedEvent` with every DEK and wrapping-key ref needed for recovery. Capture is fenced by the exact user-aggregate tail and is repeated after any concurrent key mutation; account deletion aborts if the request cannot be made durable. Projections treat that request as the immediate fail-closed privacy boundary. The request path and a shared durable worker then idempotently delete the keys, after which `UserKeyShreddedEvent` records physical completion. Message-owned assets, including derivative children such as thumbnails and video variants, receive `AssetDeletedEvent` and have their backing bytes removed.
**Why:** Scanning every JetStream stream and KV bucket for a user's messages would be slow, error-prone, and leave fragments in backups and replicas. Recording the complete deletion intent before irreversible work removes the crash window where keys could be gone without a tombstone fact. Backups specifically exclude the encryption key bucket so that restoring a backup doesn't restore the ability to read deleted users' messages. See ADR-007.
