...
- **Call facts** — room-scoped call join/leave events used to rebuild active call participant state
...
| LiveKit E2EE room keys | Ephemeral (memory-only), regenerated when members reconnect to calls |
