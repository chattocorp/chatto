...
export type Task<
  ...
  ) => SchemaInput<OutputSchema> | Promise<SchemaInput<OutputSchema>>
> = ((
  ctx: Parameters<Run>[0],
...
export function task<Run extends (ctx: WorkflowContext, ...args: never[]) => unknown>(
  run: Run
): Parameters<Run> extends [unknown, ...unknown[]]
  ? Run
  : (
      ...
      ...args: Parameters<Run> extends []
        ? []
        : Parameters<Run> extends [unknown?, ...infer Args]
          ? Args
          : []
    ) => ReturnType<Run>;
...
export function task(
  ...
  const standard =
    definition && (isStandardSchema(definition.input) || isStandardSchema(definition.output));
