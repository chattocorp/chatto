...
- **Event types** are snake_case tokens derived from the protobuf oneof variant on `evtv1.Event` (`user_joined`, `room_created`, `server_name_changed`). The canonical mapping lives in `evtstream.EventTypeOf(*evtv1.Event)` — the payload is the single source of truth, the subject token is computed from it, never authored independently.
