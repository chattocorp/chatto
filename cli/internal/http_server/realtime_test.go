...
func TestRealtimeProjectionConfigChangeReplacesEffectiveRoomConfig(t *testing.T) {
	...
	for _, operation := range frame.GetProjectionEvent().GetOperations() {
		...
		found = true
		if got := replacement.GetViewerState().GetRoomConfig().GetAuthorEditWindow().AsDuration(); got != window {
			t.Fatalf("effective author edit window = %s, want %s", got, window)
		}
	}
	if !found {
...
func TestRealtimeRoomGroupMoveReplacesInheritedRoomConfig(t *testing.T) {
	...
	for _, operation := range frame.GetProjectionEvent().GetOperations() {
		...
		found = true
		if got := replacement.GetViewerState().GetRoomConfig().GetAuthorEditWindow().AsDuration(); got != valueB {
			t.Fatalf("moved room effective window = %s, want %s", got, valueB)
		}
	}
	if !found {
