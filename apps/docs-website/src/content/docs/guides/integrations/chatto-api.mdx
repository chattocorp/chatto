...
| User-authorized client | OAuth authorization code with PKCE, then opaque bearer token. |
| Local recovery automation | Operator socket, not user auth. |

Bearer tokens and bot API keys are opaque credentials. They are not JWTs. See
