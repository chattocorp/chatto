...
Reconnect catch-up is owned by the foreground web app, not the service worker. When a controlled PWA tab wakes or reconnects, the room UI refetches the currently viewed room window from projected GraphQL state. The worker must not cache or replay messages, API responses, or live-event traffic.
