# ADR-051: Home Server for Portable Personal Data
...
server**. The first authenticated server on a fresh client becomes home; an
upgraded client with existing registrations asks the user to choose. Home is
visible in the server gutter and cannot be removed until the user moves home or
explicitly disables sync.

The home server exposes an authenticated `PersonalDataService`. It stores two
no-TTL protobuf documents in the existing backed-up `RUNTIME_STATE` bucket:

- `personal_data.{userId}.preferences`
- `personal_data.{userId}.servers`
...
enter personal data; restored server entries require authentication on the new
...
- `chatto.personaldata.v1` for stored protobuf messages
- `chatto.personaldata.api.v1` for public ConnectRPC resources and services
...
vendor account. A server backup includes this personal data because it includes
`RUNTIME_STATE`.

The home server becomes a small availability dependency for sync. Clients must
continue using their local cache while it is offline or while talking to a 0.4
server that does not implement the API. Moving home copies current client state
to the new server; automated server-to-server transfer and identity federation
remain future work.
