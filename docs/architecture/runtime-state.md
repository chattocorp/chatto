...
| `client_sync.{userId}.deletion_pending` | No-TTL exclusive preparation fence containing its creation time. It blocks mutations before the account event, becomes the retry marker after commit, and is purged only after both client-sync user records have been purged. Every replica scans this filtered key family after projection catch-up, completes committed deletions, and revision-purges active-account preparations older than one hour. |
